...
class ImageWriter FINAL {
 ...
 private:
  ...
  void CopyAndRecordReference(DestType* dest, ObjPtr<mirror::Object> src, size_t oat_index)
      REQUIRES_SHARED(Locks::mutator_lock_);

  // Copy a reference without recording image relocation. Used when we do not have
  // the address for recording the relocation, namely for interned strings and the
  // class table. The relocation needs to be recorded some other way.
