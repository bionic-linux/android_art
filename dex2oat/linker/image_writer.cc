...
void ImageWriter::CreateHeader(size_t oat_index) {
  ...
  *bitmap_section = ImageSection(RoundUp(image_end, kPageSize), RoundUp(bitmap_bytes, kPageSize));
  ...
  // image.
  void* header_storage = image_info.image_->Begin();
  ImageHeader* header =
      new (header_storage) ImageHeader(PointerToLowMemUInt32(image_info.image_begin_),
                                       image_end,
                                       sections,
...
void ImageWriter::FixupPointerArray(mirror::Object* dst,
  ...
  CopyReference(dst->GetFieldObjectReferenceAddr<kVerifyNone>(Class::ClassOffset()),
                arr->GetClass(),
                oat_index);
...
void ImageWriter::FixupDexCacheArrayEntry(std::atomic<mirror::DexCachePair<T>>* orig_array,
  ...
  CopyReference(new_pair->object.AddressWithoutBarrier(), orig_pair->object.Read(), oat_index);
...
void ImageWriter::FixupDexCacheArrayEntry(GcRoot<mirror::CallSite>* orig_array,
                                          ...
                                          uint32_t array_index,
                                          size_t oat_index) {
  CopyReference(new_array[array_index].AddressWithoutBarrier(),
...
void ImageWriter::CopyAndFixupMethod(ArtMethod* orig,
  ...
  CopyReference(copy->GetDeclaringClassAddressWithoutBarrier(),
...
void ImageWriter::RecordImageRelocation(const void* dest ATTRIBUTE_UNUSED,
                                        size_t oat_index ATTRIBUTE_UNUSED,
                                        bool app_to_boot_image ATTRIBUTE_UNUSED /* = false */) {
}

template <typename DestType>
void ImageWriter::DoCopyReference(DestType* dest, ObjPtr<mirror::Object> src, size_t oat_index) {
...
void ImageWriter::CopyReference(mirror::HeapReference<mirror::Object>* dest,
                                ...
                                size_t oat_index) {
  DoCopyReference(dest, src, oat_index);
}
...
void ImageWriter::CopyReference(mirror::CompressedReference<mirror::Object>* dest,
                                ObjPtr<mirror::Object> src) {
  // This overload is used when we do not have the address for recording the relocation,
  // namely for interned string and the class table. The relocation still needs to be
