...
class RelativePatcherTest : public CommonCompilerTest {
 protected:
  ...
  void SetUp() OVERRIDE {
    // Override CommonCompilerTest's instruction_set_features_.
    std::string error_msg;
    instruction_set_features_ =
        InstructionSetFeatures::FromVariant(instruction_set_, variant_, &error_msg);
    CHECK(instruction_set_features_ != nullptr) << instruction_set_ << "/" << variant_
        << " " << error_msg;
