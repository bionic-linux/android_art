 ...
 public:
  ...
  bool VisitMethod(size_t class_def_method_index,
    ...
    if (HasCompiledCode(compiled_method)) {
      ...
      // Determine the "method hotness order", used to determine relative order
      ...
      if (profile_index_ != ProfileCompilationInfo::MaxProfileIndex()) {
        ...
        constexpr size_t kHotBit = 1u;
        constexpr size_t kStartupBit = 2u;
        constexpr size_t kPostStartupBit = 4u;
        hotness_bits =
        ...
        if (kIsDebugBuild) {
          ...
          if (hotness_bits == kHotBit) {
            ...
            std::string name = method_ref.PrettyMethod();
