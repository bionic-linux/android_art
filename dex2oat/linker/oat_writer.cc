...
size_t OatWriter::InitIndexBssMappings(size_t offset) {
  if (!(compiler_options_.IsBootImage() || compiler_options_.IsBootImageExtension())) {
    bcp_bss_info.resize(Runtime::Current()->GetClassLinker()->GetBootClassPath().size());
  }

  if (bss_method_entry_references_.empty() && bss_type_entry_references_.empty() &&
      bss_public_type_entry_references_.empty() && bss_package_type_entry_references_.empty() &&
  ...
  if (!(compiler_options_.IsBootImage() || compiler_options_.IsBootImageExtension())) {
    ...
    for (size_t i = 0, size = bcp_bss_info.size(); i != size; ++i) {
      ...
      offset = InitIndexBssMappingsHelper(offset,
                                          ...
                                          number_of_package_type_dex_files,
                                          number_of_string_dex_files,
                                          bcp_bss_info[i].method_bss_mapping_offset_,
                                          bcp_bss_info[i].type_bss_mapping_offset_,
                                          bcp_bss_info[i].public_type_bss_mapping_offset_,
                                          bcp_bss_info[i].package_type_bss_mapping_offset_,
                                          bcp_bss_info[i].string_bss_mapping_offset_);
    }
  }
...
size_t OatWriter::InitBcpBssInfo(size_t offset) {
  // We first increase the offset to make room to store the number of BCP DexFiles.
  offset += sizeof(size_t);
  if (compiler_options_.IsBootImage() || compiler_options_.IsBootImageExtension()) {
    return offset;
  }

  for (BssMappingInfo& info : bcp_bss_info) {
    info.offset_ = offset;
...
size_t OatWriter::WriteBcpBssInfo(OutputStream* out, size_t file_offset, size_t relative_offset) {
  ...
  const size_t number_of_BCP_dexfiles = bcp_bss_info.size();
  ...
  relative_offset += sizeof(number_of_BCP_dexfiles);

  const bool is_multi_image =
      compiler_options_.IsBootImage() || compiler_options_.IsBootImageExtension();
  if (is_multi_image) {
    // We do not add the bss info for BCP DexFiles with multi image since the current opitmizations
    // are only expecting single image.
    DCHECK_EQ(number_of_BCP_dexfiles, 0u);
  }

  for (size_t i = 0, size = number_of_BCP_dexfiles; i != size; ++i) {
    DCHECK_EQ(relative_offset, bcp_bss_info[i].offset_);
    DCHECK_OFFSET();
    if (!bcp_bss_info[i].Write(this, out)) {
      return 0u;
    }
...
bool OatWriter::BssMappingInfo::Write(OatWriter* oat_writer, OutputStream* out) const {
  ...
  oat_writer->size_bcp_bss_info_method_bss_mapping_offset_ += sizeof(method_bss_mapping_offset_);

  if (!out->WriteFully(&type_bss_mapping_offset_, sizeof(type_bss_mapping_offset_))) {
    PLOG(ERROR) << "Failed to write type bss mapping offset to " << out->GetLocation();
    return false;
  }
  oat_writer->size_bcp_bss_info_type_bss_mapping_offset_ += sizeof(type_bss_mapping_offset_);
  ...
  oat_writer->size_bcp_bss_info_string_bss_mapping_offset_ += sizeof(string_bss_mapping_offset_);
