...
void OatWriter::PrepareLayout(MultiOatRelativePatcher* relative_patcher) {
  ...
  }
  {
    ...
    oat_header_->SetBCPInfoOffset(offset);
...
size_t OatWriter::InitIndexBssMappings(size_t offset) {
  // TODO(solanes): Move to the correct place.
  LOG(INFO) << "InitIndexBssMappings Prelude";
  if (!(compiler_options_.IsBootImage() || compiler_options_.IsBootImageExtension())) {
    ...
    // TODO(solanes): Remove for after debugging.
  ...
  // }
    ...
    ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
    const std::vector<const DexFile*>& boot_class_path = class_linker->GetBootClassPath();
    for (size_t i = 0, size = BCP_info.size(); i != size; ++i) {
      const DexFile* dex_file = boot_class_path[i];
...
size_t OatWriter::InitBCPInfo(size_t offset) {
  ...
  // We have to make room to store the number of BCP DexFiles.
  for (BSSInfo& info : BCP_info) {
    LOG(INFO) << "InitBCPInfo offset " << offset;
    info.offset_ = offset;
...
size_t OatWriter::WriteBSSInfo(OutputStream* out, size_t file_offset, size_t relative_offset) {
  ...
  size_t number_of_BCP_dexfiles =
      is_multi_image ? 0 : Runtime::Current()->GetClassLinker()->GetBootClassPath().size();
  ...
  size_BCP_info_size_ += sizeof(number_of_BCP_dexfiles);
  relative_offset += sizeof(number_of_BCP_dexfiles);
  LOG(INFO) << "WriteBSSInfo wrote number_of_BCP_dexfiles";

  if (is_multi_image) {
    LOG(INFO) << "WriteBSSInfo SKIP IS MULTI IMAGE";
    return relative_offset;
  }

  // LOG(INFO) << "What";
  ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
  // LOG(INFO) << "got class_linker";
  const std::vector<const DexFile*>& boot_class_path = class_linker->GetBootClassPath();
  // LOG(INFO) << "got boot_class_path";
  LOG(INFO) << "BCP_info size " << BCP_info.size();
  LOG(INFO) << "WriteBSSInfo number_of_BCP_dexfiles " << number_of_BCP_dexfiles;

  for (size_t i = 0, size = BCP_info.size(); i != size; ++i) {
    LOG(INFO) << "BCP_info[i].offset_ " << BCP_info[i].offset_;
    ...
    // Write BCP_info.
    LOG(INFO) << "WriteBSSInfo writing " <<  boot_class_path[i]->GetLocation();
    if (!BCP_info[i].Write(this, out)) {
      return 0u;
    }
    LOG(INFO) << BCP_info[i].method_bss_mapping_offset_ << " " <<
    BCP_info[i].type_bss_mapping_offset_ << " " <<
    BCP_info[i].public_type_bss_mapping_offset_ << " " <<
    BCP_info[i].package_type_bss_mapping_offset_ << " " <<
    BCP_info[i].string_bss_mapping_offset_ << " ";
