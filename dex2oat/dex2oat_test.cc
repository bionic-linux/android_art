...
class Dex2oatTest : public Dex2oatEnvironmentTest {
 ...
 protected:
  ...
  AssertionResult GenerateOdexForTest(const std::string& dex_location,
    ...
    if (!status.ok()) {
      return AssertionFailure() << status.error();
    }

    bool success = status.value() == 0;
    if (expect_success) {
      if (!success) {
        return AssertionFailure() << "Failed to compile odex: " << output_;
      }
...
class Dex2oatReturnCodeTest : public Dex2oatTest {
 protected:
  Result<int> RunTest(const std::vector<std::string>& extra_args = {}) {
    std::string dex_location = GetScratchDir() + "/Dex2OatSwapTest.jar";
    std::string odex_location = GetOdexDir() + "/Dex2OatSwapTest.odex";
    ...
    return GenerateOdexForTestWithStatus(
        {dex_location}, odex_location, CompilerFilter::kSpeed, extra_args);
  }
...
TEST_F(Dex2oatTest, EmptyUncompressedDexTest) {
  std::string out_dir = GetScratchDir();
  const std::string base_oat_name = out_dir + "/base.oat";
  Result<int> status = GenerateOdexForTestWithStatus({GetTestDexFileName("MainEmptyUncompressed")},
                                                     base_oat_name,
                                                     CompilerFilter::Filter::kVerify,
                                                     /*extra_args*/ {},
                                                     /*use_fd*/ false);
...
TEST_F(Dex2oatTest, CompactDexInvalidSource) {
  ...
  Result<int> status = GenerateOdexForTestWithStatus(
      {dex_location}, odex_location, CompilerFilter::kVerify, /*extra_args*/ {});
  ASSERT_THAT(status, Ok());
  ASSERT_NE(status.value(), 0) << " " << output_;
}
