  ...
  bool DisallowAllFileCompression() {
    ...
    for (auto& files : {&vdex_files_, &oat_files_}) {
      for (size_t i = 0; i < files->size(); ++i) {
        ...
        LOG(INFO) << "ASDF ASDF HERE " << file->GetPath() << " " << oat_location_;

        std::string dir = file->GetPath();
        std::size_t slash = dir.find_last_of('/');
        if (slash == std::string::npos) {
          LOG(ERROR) << "Could not find path in " << dir;
          error = true;
          continue;
        }
        dir = dir.substr(0, slash);
        LOG(INFO) << "ASDF ASDF HERE " << file->GetPath() << " " << oat_location_ << " dir " << dir;

        struct statfs64 fsInfo;
        int result = statfs64(dir.c_str(), &fsInfo);
        if (result < 0) {
