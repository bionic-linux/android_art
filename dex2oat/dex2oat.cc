 ...
 public:
  ...
  dex2oat::ReturnCode Setup() {
    ...
    if (!IsBootImage() && IsVeryLarge(dex_files_)) {
      CompilerFilter::Filter filter = CompilerFilter::kVerify;
      if (!CompilerFilter::IsAsGoodAs(filter, compiler_options_->GetCompilerFilter())) {
        ...
        compiler_options_->SetCompilerFilter(filter);
      }
    }
