...
class Dex2Oat final {
 public:
  ...
  void ProcessOptions(ParserOptions* parser_options) {
    ...
    // Trim the boot image location to not include any specified profile.
    std::vector<std::string> boot_images =
        android::base::Split(boot_image_filename_, {ImageSpace::kComponentSeparator});
    bool boot_image_filename_trimmed = false;
    for (std::string& boot_image : boot_images) {
      ...
      if (profile_separator_pos != std::string::npos) {
        ...
        boot_image_filename_trimmed = true;
      }
    }
    if (boot_image_filename_trimmed) {
