...
TEST_F(Dex2oatVdexTest, VerifyPublicSdkStubs) {
  ...
  Result<std::unique_ptr<VerifierDeps>> res = GetVerifierDeps(GetVdex(dex_file), dex_file.get());
  ASSERT_THAT(res, Ok());
  std::unique_ptr<VerifierDeps> deps = std::move(res.value());
  ...
  res = GetVerifierDeps(GetVdex(dex_file), dex_file.get());
  ASSERT_THAT(res, Ok());
  std::unique_ptr<VerifierDeps> deps2 = std::move(res.value());
