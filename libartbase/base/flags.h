 ...
 public:
  ...
  static void ReloadAllFlags(const std::string& reason_for_dbg) {
    ...
    if (VLOG_IS_ON(startup)) {
      VLOG_STREAM(startup) << "Dumping flags because " << reason_for_dbg;
...
// Example:
//
//     Flag<bool> WriteMetricsToLog{"my-feature-test.flag", false};
//
...
struct Flags {
  ...
  Flag<int> MyFeatureTestFlag{"my-feature-test.flag", /*default*/ 42};
};
