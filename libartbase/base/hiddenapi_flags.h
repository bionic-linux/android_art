...
class ApiList {
 private:
  ...
  // Additional bit flags after the first kDexValueBitSize bits in dex flags.
 ...
 public:
  ...
  static ApiList FromName(const std::string& str) {
    ...
    for (uint32_t i = 0; i <= kDomainApiCount; ++i) {
      if (str == kDomainApiNames[i]) {
        return ApiList(static_cast<Value>(i + helper::ToUint(DomainApi::kMin)));
      }
    }
    return ApiList();
  }
  ...
  bool CanBeCombinedWith(const ApiList other) {
