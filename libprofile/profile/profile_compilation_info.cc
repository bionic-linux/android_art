...
void FlattenProfileData::ItemMetadata::ExtractInlineCacheInfo(
  ...
  for (const auto& [pc, ic_data] : *inline_caches) {
    ...
    InlineCacheInfo& val = inline_cache_.FindOrAdd(TypeReference(dex_file, target.class_idx_))->second;
    ...
    for (dex::TypeIndex type_index : ic_data.classes) {
      val.classes_.insert(TypeReference(dex_file, type_index));
    }
  }
}
...
void FlattenProfileData::ItemMetadata::MergeInlineCacheInfo(const SafeMap<TypeReference, InlineCacheInfo, TypeReferenceValueComparator>& other) {
  for (const auto& [target, inline_cache_data] : other) {
    ...
    for (TypeReference cls : inline_cache_data.classes_) {
