...
bool ProfileBootInfo::Load(int fd, const std::vector<const DexFile*>& dex_files) {
  ...
  do {
    ...
    if (it != dex_files.end()) {
      ...
      LOG(WARNING) << "Couldn't find " << std::string(data.get(), string_length);
    }
  } while (true);

  // Read methods.
  do {
    std::pair<uint32_t, uint32_t> method;
    int bytes_read = TEMP_FAILURE_RETRY(read(fd, &method, sizeof(method)));
    if (bytes_read <= 0) {
      break;
    }
