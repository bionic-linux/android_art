...
Result<void> BindMount(const std::string& source, const std::string& target) {
  ...
  OR_RETURN(Unmount(*kBindMountTmpDir, /*logging=*/false));
...
Result<void> DexoptChrootSetup::InitChroot() const {
  ...
  OR_RETURN(BindMount("/linkerconfig", PathInChroot("/linkerconfig")));
...
Result<void> DexoptChrootSetup::TearDownChroot() const {
  ...
  // Size being >0 means a bind-mounted /linkerconfig, unmount it so that apexd isn't run with the
  // wrong linker config; ==0 means an empty linker config generated by `InitChroot`; <0 means that
  // /linkerconfig doesn't exist.
  if (OS::GetFileSizeBytes(PathInChroot("/linkerconfig/ld.config.txt").c_str()) > 0) {
    OR_RETURN(Unmount(PathInChroot("/linkerconfig")));
  }
