...
Result<void> DexoptChrootSetup::SetUpChroot(const std::optional<std::string>& ota_slot,
  ...
  std::vector<std::tuple<std::string, std::string, bool>> additional_system_partitions = {
      {"system_ext", "/system_ext", /*is_optional=*/true},
      {"vendor", "/vendor", /*is_optional=*/true},
      {"product", "/product", /*is_optional=*/true},
  };
  ...
  for (std::string_view entry : partitions_from_sysprop_entries) {
    ...
    additional_system_partitions.emplace_back(
        std::string(pair[0]), std::string(pair[1]), /*is_optional=*/false);
  }
  ...
  if (!IsOtaUpdate(ota_slot)) {  // Mainline update
    ...
    for (const auto& [partition, mount_point, is_optional] : additional_system_partitions) {
    ...
    for (const auto& [partition, mount_point, is_optional] : additional_system_partitions) {
      OR_RETURN(Mount(
          GetBlockDeviceName(partition, ota_slot.value()), PathInChroot(mount_point), is_optional));
    }
  }
