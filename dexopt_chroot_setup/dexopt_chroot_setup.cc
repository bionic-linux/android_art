...
constexpr std::array<const char*, 4> kLibDirs = {
...
Result<void> DexoptChrootSetup::TearDownChroot() const {
  ...
  for (const FstabEntry entry : entries) {
    if (ContainsElement(kLibDirs, entry.mount_point.substr(strlen(CHROOT_DIR)))) {
