...
class OatDumpTest : public CommonRuntimeTest, public testing::WithParamInterface<Flavor> {
 protected:
  ...
  enum Args {
    ...
    kArgMethodAndOffsetOnly = 1 << 7,  // --dump-method-and-offset-only
  ...
  enum Expects {
    ...
    kExpectMethodAndOffsetOnly = 1 << 5,
  };
  ...
  ::testing::AssertionResult Exec(Flavor flavor,
    ...
    if ((expects & kExpectMethodAndOffsetOnly) != 0) {
      expected_prefixes.push_back(
          "method=void java.lang.Object.<init>(),offset=0x");  // actual offset may differ
                                                               // between dex files
    }
    ...
    if ((args & kArgMethodAndOffsetOnly) != 0) {
      exec_argv.push_back("--dump-method-and-offset-only");
    }
