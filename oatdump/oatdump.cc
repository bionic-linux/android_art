 ...
 public:
  OatDumperOptions(bool dump_vmap,
      ...
      : dump_vmap_(dump_vmap),
        ...
        dump_methods_as_json(dump_methods_as_json),
  ...
  const bool dump_methods_as_json;
...
class OatDumper {
 public:
  ...
  bool Dump(std::ostream& os) {
    if (options_.dump_methods_as_json) {
      DumpMethodsAsJson(os);
      return true;
    }
...
#undef DUMP_OAT_HEADER_OFFSET
  ...
  void DumpMethodsAsJson(std::ostream& os) {
    for (size_t i = 0; i < oat_dex_files_.size(); i++) {
      const OatDexFile* oat_dex_file = oat_dex_files_[i];
      CHECK(oat_dex_file != nullptr);
      ...
      for (ClassAccessor accessor : dex_file->GetClasses()) {
        ...
        for (const ClassAccessor::Method& method : accessor.GetMethods()) {
          ...
          std::string pretty_method = dex_file->PrettyMethod(dex_method_idx, false);
          entry["fqn"] = pretty_method.c_str();
          entry["offset"] = StringPrintf("0x%08x", code_offset);

          const dex::ProtoId* proto_id =
              &dex_file->GetProtoId(dex_file->GetMethodId(dex_method_idx).proto_idx_);
          std::string returns;
          AppendPrettyDescriptor(dex_file->StringByTypeIdx(proto_id->return_type_idx_), &returns);
          entry["signature"]["returns"] = returns;

          entry["signature"]["params"] = Json::arrayValue;
          const dex::TypeList* params = dex_file->GetProtoParameters(*proto_id);
          if (params != nullptr) {
            for (uint32_t param_index = 0u, size = params->Size(); param_index != size;
                 ++param_index) {
              std::string param;
              AppendPrettyDescriptor(
                  dex_file->StringByTypeIdx(params->GetTypeItem(param_index).type_idx_), &param);
              entry["signature"]["params"][param_index] = param;
            }
          }
          Json::FastWriter writer;
          vios.Stream() << writer.write(entry);
        }
      }
    }
  }
 ...
 protected:
  ...
  ParseStatus ParseCustom(const char* raw_option,
    ...
    if (StartsWith(option, "--oat-file=")) {
      ...
      imt_stat_dump_ = true;
    } else if (option == "--dump-methods-as-json") {
      dump_methods_as_json = true;
    } else {
...
Options:
 ...
 public:
  ...
  bool dump_methods_as_json = false;
