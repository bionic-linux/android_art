...
class OatDumperOptions {
 public:
  OatDumperOptions(bool dump_vmap,
                   ...
                   bool dump_method_and_offset_only,
      ...
      : dump_vmap_(dump_vmap),
        ...
        dump_method_and_offset_only(dump_method_and_offset_only),
  ...
  const bool dump_method_and_offset_only;
...
class OatDumper {
 public:
  ...
  bool Dump(std::ostream& os) {
    if (options_.dump_method_and_offset_only) {
      DumpMethodAndOffsetOnly(os);
      return true;
    }
...
#undef DUMP_OAT_HEADER_OFFSET
  ...
  void DumpMethodAndOffsetOnly(std::ostream& os) {
    for (size_t i = 0; i < oat_dex_files_.size(); i++) {
      const OatDexFile* oat_dex_file = oat_dex_files_[i];
      CHECK(oat_dex_file != nullptr);
      ...
      for (ClassAccessor accessor : dex_file->GetClasses()) {
        ...
        for (const ClassAccessor::Method& method : accessor.GetMethods()) {
          ...
          vios.Stream() << StringPrintf("method=%s", pretty_method.c_str())
 ...
 protected:
  ...
  ParseStatus ParseCustom(const char* raw_option,
    ...
    if (StartsWith(option, "--oat-file=")) {
      ...
      imt_stat_dump_ = true;
    } else if (option == "--dump-method-and-offset-only") {
      dump_method_and_offset_only = true;
    } else {
...
Options:
 ...
 public:
  ...
  bool dump_method_and_offset_only = false;
...
struct OatdumpMain : public CmdlineMain<OatdumpArgs> {
  bool NeedsRuntime() override {
    ...
    oat_dumper_options_.reset(new OatDumperOptions(args_->dump_vmap_,
                                                   ...
                                                   args_->dump_method_and_offset_only,
