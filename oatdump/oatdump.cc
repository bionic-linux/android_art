...
static int DumpOatWithRuntime(Runtime* runtime,
  ...
  std::vector<const DexFile*> class_path;
  jobject class_loader = InstallOatFile(runtime, oat_file, &class_path);
...
class IMTDumper {
 public:
  static bool Dump(Runtime* runtime,
    ...
    if (oat_filename != nullptr) {
      std::string error_msg;
      oat_file = std::unique_ptr<OatFile>(OatFile::Open(oat_filename,
                                                        oat_filename,
                                                        nullptr,
                                                        nullptr,
                                                        false,
      ...
      class_loader = soa.Decode<mirror::ClassLoader*>(
          InstallOatFile(runtime, oat_file.get(), &class_path));
    } else {
      class_loader = nullptr;  // Boot classloader.
 ...
 private:
  static bool DumpImt(Runtime* runtime,
    ...
    Handle<mirror::ClassLoader> h_loader(scope.NewHandle<mirror::ClassLoader>(class_loader));
    ...
    for (const std::string& line : lines) {
      ...
      if (first_space == std::string::npos) {
        DumpIMTForClass(runtime, line, h_loader, &prepared);
      } else {
  ...
  static bool DumpImtStats(Runtime* runtime,
    ...
    for (const DexFile* dex_file : dex_files) {
      for (uint32_t class_def_index = 0;
        ...
        const DexFile::ClassDef& class_def = dex_file->GetClassDef(class_def_index);
        const char* descriptor = dex_file->GetClassDescriptor(class_def);
        h_klass.Assign(class_linker->FindClass(self, descriptor, h_loader));
