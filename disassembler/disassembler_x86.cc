...
size_t DisassemblerX86::DumpInstruction(std::ostream& os, const uint8_t* instr) {
  ...
  uint8_t vex = (*instr == THREE_BYTE_VEX || *instr== TWO_BYTE_VEX) ? *instr : 0;
  uint8_t vex_byte_zero = 0;
  uint8_t vex_byte_one  = 0;
  uint8_t vex_byte_two  = 0;
  if (vex != 0) {
    /** Intializing VEX BYTE ZERO */
    vex_byte_zero = *instr;
    /** Intializing VEX BYTE ONE */
    vex_byte_one =  *(instr+1);
    if (vex == THREE_BYTE_VEX) {
      /** If it is 3 Byte Encoding Intializing VEX BYTE TWO */
...
#undef DISASSEMBLER_ENTRY
  ...
  case 0xC4: case 0xC5:
    /** Two Byte Encoding 0xC5 Three Byte Encoding 0xC4 */
    ...
    if (has_vex_none && has_vex_0f) {
      ...
      LOG(FATAL) << "Unimplemented VEX Instruction";
    }
    break;
  case 0xC6:
  ...
  if (has_modrm) {
    ...
    if (load) {
      ...
      if (vex != 0 && reg_is_vvvv) {
        uint8_t check_byte = (vex == THREE_BYTE_VEX) ? vex_byte_two : vex_byte_one;
