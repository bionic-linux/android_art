...
#undef DISASSEMBLER_ENTRY
  ...
  const uint8_t* adjustedInstr = ctxt.orig_instr_ + actual_bytes_read;
  os << FormatInstructionPointer(ctxt.orig_instr_)
     << StringPrintf(": %22s    \t%-7s%s%s%s%s%s ",
                     DumpCodeHex(ctxt.orig_instr_, adjustedInstr).c_str(),
                     prefix_str,
                     opcode0,
                     opcode1,
                     opcode2,
...
DisassemblerX86::InstructionContext::InstructionContext(DisassemblerX86* disass,
  ...
  if (vex_.ConvertToRex(
          instr, shadow_instr_buffer_, disassembler_->GetDisassemblerOptions()->end_address_)) {
