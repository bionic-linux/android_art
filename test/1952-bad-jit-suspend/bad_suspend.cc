...
static JNICALL void AgentThreadWait(jvmtiEnv* jvmti ATTRIBUTE_UNUSED,
  ...
  // Give us 30 seconds to deadlock.
  sleep(30);
...
// Exploit b/111394423 to pause the jit thread even though we aren't really supposed to be able to.
// In real world agents this would be caused by a deadlock with a thread holding a mutex we need but
// it is waiting for the jit thread to stop.
