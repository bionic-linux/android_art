...
# The build servers have an ancient version of bash so we cannot use @Q.
  ...
  if not HOST:
    ...
    push_files += [Path(args.stdout_file), Path(args.stderr_file)]
    push_files += [Path("profile"), Path("res")]
    push_files = [p for p in push_files if p.exists()]
    if ctx.is_first_pass:
      # Push files needed to run the test.
      adb.shell(f"rm -rf {CHROOT_DEX_LOCATION} && mkdir -p {CHROOT_DEX_LOCATION}")
      adb.push(" ".join(map(str, push_files)), CHROOT_DEX_LOCATION)
    else:
      # Clean-up any files generated by the previous run.
      adb.shell(f"rm -rf {CHROOT_DEX_LOCATION}/oat/")
      adb.shell(f"rm -rf {CHROOT_DEX_LOCATION}/dalvik-cache/")
      adb.shell(f"rm -rf {CHROOT_DEX_LOCATION}/cmdline.sh")
      adb.shell(f"rm -rf {CHROOT_DEX_LOCATION}/*.prof")
      files = adb.shell(f"ls {CHROOT_DEX_LOCATION}").stdout.strip().split("\n")
      extra_files = set(files) - set(p.name for p in push_files)
      assert len(extra_files) == 0, f"Files were not cleaned up: {extra_files}"
    ctx.is_first_pass = False
