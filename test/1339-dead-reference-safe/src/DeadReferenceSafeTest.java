...
public final class DeadReferenceSafeTest implements Runnable {
  ...
  Finalizable aFinalizable = new Finalizable();
  SensitiveFinalizable aSensitiveFinalizable = new SensitiveFinalizable();

  private void loop() {
    // Each loop allocates INNER_ITERS Finalizable objects, and calls doLittle()
    // INNER_ITERS - 1 times.
    Finalizable x = aFinalizable;
    for (int i = 0; i < INNER_ITERS; ++i) {
      if (i == 5 && realTest) {
        // At the start of iteration n, we've allocated n objects.
  ...
  private void sensitiveLoop() {
    SensitiveFinalizable x = aSensitiveFinalizable;
    for (int i = 0; i < INNER_ITERS; ++i) {
      if (i == 5 && realTest) {
        Runtime.getRuntime().gc();
        System.runFinalization();
  ...
  private void fencedLoop() {
    ...
    for (int i = 0; i < INNER_ITERS; ++i) {
      if (i == 5 && realTest) {
        ...
        // Thus all 5 prior objects should have been found unreachable and finalized.
        int finalizedCount = nFinalized.get();
        System.out.println("FencedLoop finalized " + finalizedCount + " objects");
        if (finalizedCount != 4) {
  ...
  public void run() {
    ...
    reset(3 * WARMUP_ITERS * INNER_ITERS);
    loop();
    ...
    if (n != 3 * (WARMUP_ITERS + 1) * INNER_ITERS) {
