...
extern "C" JNIEXPORT void JNICALL Java_art_Exceptions_disableExceptionTracing(JNIEnv* env, [[maybe_unused]] jclass klass, jthread thr) {
  ExceptionsData* data = nullptr;
  ...
  if (JvmtiErrorToException(
      env, jvmti_env, jvmti_env->SetEnvironmentLocalStorage(nullptr))) {
    return;
  }
}
