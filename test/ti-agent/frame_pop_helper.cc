...
extern "C" JNIEXPORT void JNICALL Java_art_FramePop_notifyFramePop(
    JNIEnv* env, jclass, jlong jvmti_env_ptr, jthread thr, jint depth) {
  jvmtiEnv* jvmti =
      jvmti_env_ptr == 0 ? jvmti_env
                         : reinterpret_cast<jvmtiEnv*>(static_cast<intptr_t>(jvmti_env_ptr));
  JvmtiErrorToException(env, jvmti, jvmti->NotifyFramePop(thr, depth));
}
