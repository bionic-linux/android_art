...
public class Main {
  ...
  private static long timeNotifications() {
    ...
    // Notify enough times to actually chack heap state twice.
    // Each iteration count must be >= Heap::kNotifyNativeInterval.
    for (int i = 0; i < 400; ++i) {
      vmr.notifyNativeAllocation();
    }
    try {
      // Allow a triggered GC to actually get started. This works around a known deficiency:
      // Native allocation doesn't block for a background GC until the GC has actually started.
      Thread.sleep(1);
