...
public class Monitors {
  ...
  public static class NamedLock {
    ...
    private AtomicInteger calledNotify;
    public NamedLock(String name) {
      this.name = name;
      calledNotify = new AtomicInteger(0);
    }
    ...
    public final void DoWait() throws Exception {
      final int v = calledNotify.get();
      while (v == calledNotify.get()) {
        wait();
      }
    }
    ...
    public final void DoWait(long t) throws Exception {
      ...
      while (v == calledNotify.get() && (t < 0 || System.currentTimeMillis() < target)) {
        wait(t);
      }
    }

    public final void DoNotifyAll() throws Exception {
      calledNotify.getAndIncrement();
      notifyAll();
    }
