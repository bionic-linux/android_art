...
public class Test2282 {
    ...
    public static void notifySingleStep(Thread thr, Executable e, long loc) {
        ...
        if (LAST_LINE_NUMBER == CATCH_LINE_NUMBER) {
          System.out.println("Single stepping was enabled in catch handler. We should not break on catch line");
        }
    }

    public static void ExceptionCatchHandler(Thread thr, Executable method, long location, Throwable exception) {
      CATCH_LINE_NUMBER = Breakpoint.locationToLine(method, location);
      System.out.println(thr.getName() + ": " + method + " @ line = " +
        CATCH_LINE_NUMBER + " caught " +
    ...
    public static void run() throws Exception {
      CATCH_HANDLER = Test2282.class.getDeclaredMethod("ExceptionCatchHandler",
          Thread.class, Executable.class, Long.TYPE, Throwable.class);
      SINGLE_STEP_HANDLER = Test2282.class.getDeclaredMethod("notifySingleStep", Thread.class,
          Executable.class, Long.TYPE);
      ...
      Test2282.class,
      TestException.class,
      /*ExceptionCaughtEventHandler=*/ null,
      CATCH_HANDLER);
