...
def run_tests(tests):
  ...
  def start_combination(config_tuple, address_size):
      ...
      # Honor ART_TEST_CHROOT, ART_TEST_ANDROID_ROOT,
      # ART_TEST_ANDROID_RUNTIME_ROOT, and ART_TEST_LD_CONFIG_FILE, but only for
      # target tests.
      if target == 'target':
        ...
        if env.ART_TEST_LD_CONFIG_FILE:
          options_test += ' --ld-config-file ' + env.ART_TEST_LD_CONFIG_FILE
