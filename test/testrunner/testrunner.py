...
def get_variant_type(variant):
  for k,v in VARIANT_TYPE_DICT.items():
    if variant in v:
      return k
...
def parse_option():
  ...
  global_group.add_argument('--logcat-invoke', dest='logcat_invoke', action='store',
                            default="adb logcat",
                            help="""Command to invoke to get logcat data from device. This command
                            is run in the background while all the tests are running then killed
                            with a SIGTERM.  By default it is 'adb logcat'. If one needs to perform
                            pre-processing (for example to clear the logcat) one should pass a shell
                            script that does the needed operations. This command will only be run if
                            a --logcat-file is given and at least one --target test is being
                            run.""")
  global_group.add_argument('--logcat-file', dest='logcat_file', action='store',
                            type=lambda f: open(f, 'w'),
                            help="""A file we will write the logcat output received during the test.
                            This is only valid during --target tests. The file will be created if it
  ...
  for variant_type, variant_set in VARIANT_TYPE_DICT.items():
    var_group = parser.add_argument_group(
        ...
        "Options that control the '{}' options".format(variant_type))
