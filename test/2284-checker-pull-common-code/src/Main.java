...
// TODO(solanes): Update the CHECKs
...
public class Main {
    public static void main(String[] args) {
        ...
        $noinline$assertIntEquals(11, $noinline$testNestedIfs(new TestClass(10), true, true));
        $noinline$assertIntEquals(12, $noinline$testNestedIfs(new TestClass(10), true, false));
        $noinline$assertIntEquals(13, $noinline$testNestedIfs(new TestClass(10), false, true));
        $noinline$assertIntEquals(14, $noinline$testNestedIfs(new TestClass(10), false, false));
    }
    ...
    // TODO(solanes): Do a case there the LoadClass cannot throw.
    ...
    // remaining NullCheck instructions. Similarly, LSE can remove the remaining InstanceFieldGet
    // instructions.
    ...
    /// CHECK-START: int Main.$noinline$testInstanceFieldGet(TestClass, boolean) GVN (before)
    /// CHECK:     NullCheck
    /// CHECK:     NullCheck
    /// CHECK:     NullCheck
    /// CHECK:     NullCheck
    ...
    /// CHECK-START: int Main.$noinline$testInstanceFieldGet(TestClass, boolean) load_store_elimination (before)
    /// CHECK:     InstanceFieldGet
    /// CHECK:     InstanceFieldGet
    /// CHECK-NOT: InstanceFieldGet

    /// CHECK-START: int Main.$noinline$testInstanceFieldGet(TestClass, boolean) load_store_elimination (after)
    /// CHECK:     InstanceFieldGet
    /// CHECK-NOT: InstanceFieldGet
    ...
    // Even when nested we can remove three NullCheck instructions, and GVN can remove the rest.
    // Similarly, LSE can remove the remaining InstanceFieldGet instructions.

    /// CHECK-START: int Main.$noinline$testNestedIfs(TestClass, boolean, boolean) code_pulling (before)
    /// CHECK: NullCheck
    /// CHECK: InstanceFieldGet
    /// CHECK: NullCheck
    /// CHECK: InstanceFieldSet
    ...
    /// CHECK-START: int Main.$noinline$testNestedIfs(TestClass, boolean, boolean) code_pulling (after)
    /// CHECK:     InstanceFieldGet
    /// CHECK:     InstanceFieldGet
    /// CHECK-NOT: InstanceFieldGet
    ...
    /// CHECK-START: int Main.$noinline$testNestedIfs(TestClass, boolean, boolean) GVN (before)
    /// CHECK:     NullCheck
    /// CHECK:     NullCheck
    /// CHECK:     NullCheck
    /// CHECK:     NullCheck
    ...
    /// CHECK-START: int Main.$noinline$testNestedIfs(TestClass, boolean, boolean) GVN (after)
    /// CHECK:     NullCheck
    /// CHECK-NOT: NullCheck

    /// CHECK-START: int Main.$noinline$testNestedIfs(TestClass, boolean, boolean) load_store_elimination (before)
    /// CHECK:     InstanceFieldGet
    /// CHECK:     InstanceFieldGet
    /// CHECK-NOT: InstanceFieldGet

    /// CHECK-START: int Main.$noinline$testNestedIfs(TestClass, boolean, boolean) load_store_elimination (after)
    /// CHECK:     InstanceFieldGet
    /// CHECK-NOT: InstanceFieldGet
    static int $noinline$testNestedIfs(TestClass obj, boolean b1, boolean b2) {
        if (b1) {
            if (b2) {
                obj.i += 1;
            } else {
