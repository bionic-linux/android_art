...
abstract class BaseTraceParser {
    ...
    public void InitializeParser(File file) throws IOException {
        ...
        nesting_level_map = new HashMap<Integer, Integer>();
    }
    ...
    public boolean ShouldIgnoreThread(int thread_id) throws Exception {
        if (thread_id_map.get(thread_id).contains("Daemon")) {
            return true;
        }
        return false;
    }

    public String eventTypeToString(int event_type, int thread_id) {
        if (!nesting_level_map.containsKey(thread_id)) {
            nesting_level_map.put(thread_id, 0);
        }

        int nesting_level = nesting_level_map.get(thread_id);
        String str = "";
        ...
        nesting_level_map.put(thread_id, nesting_level);
        return str;
    }
    ...
    public String ProcessEventEntry(int thread_id) throws IOException {
        ...
        String str = eventTypeToString(event_type, thread_id) + " " + thread_id_map.get(thread_id)
    ...
    public abstract void CheckTraceFileFormat(File trace_file, int expected_version)
            throws Exception;

    DataInputStream dataStream;
    HashMap<Integer, String> method_id_map;
    HashMap<Integer, String> thread_id_map;
    HashMap<Integer, Integer> nesting_level_map;
    int record_size = 0;
