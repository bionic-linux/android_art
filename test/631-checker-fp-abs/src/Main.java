...
public class Main {
  ...
  public static void main(String args[]) {
    ...
    final int SPQUIET = 1 << 22;
    for (int i = 0; i < spnans.length; i++) {
      float f = Float.intBitsToFloat(spnans[i]);
      expectEquals32(
          spnans[i] & Integer.MAX_VALUE | SPQUIET,
          Float.floatToRawIntBits($opt$noinline$absSP(f)) | SPQUIET);
    }
    ...
    final long DPQUIET = 1L << 51;
    for (int i = 0; i < dpnans.length; i++) {
      double d = Double.longBitsToDouble(dpnans[i]);
      expectEquals64(
          dpnans[i] & Long.MAX_VALUE | DPQUIET,
          Double.doubleToRawLongBits($opt$noinline$absDP(d)) | DPQUIET);
    }
