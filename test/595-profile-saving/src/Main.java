...
public class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (primaryTimestampAfter.compareTo(primaryTimestampBefore) <= 0) {
        throw new Error(String.format("Profile for primary APK not updated (before: %d, after: %d)",
                primaryTimestampBefore.toNanos(), primaryTimestampAfter.toNanos()));
    }
    if (splitTimestampAfter.compareTo(splitTimestampBefore) <= 0) {
        throw new Error(String.format("Profile for split APK not updated (before: %d, after: %d)",
                primaryTimestampBefore.toNanos(), primaryTimestampAfter.toNanos()));
    }
    if (primaryTimestampAfter.compareTo(splitTimestampAfter) < 0) {
        throw new Error(
                String.format("Profile for primary APK is unexpected updated before profile for "
