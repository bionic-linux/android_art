...
# Create bash script that compiles the boot image.
def create_setup_script(is64: bool):
  ...
  cmd = [
    ...
    "--runtime-arg", f"-Xbootclasspath:{':'.join(dex)}",
    "--runtime-arg", f"-Xbootclasspath-locations:{':'.join(dex)}",
  ] + [f"--dex-file={d}" for d in dex] + [f"--dex-location={d}" for d in dex] + [
