...
public class Main {
  ...
  private static final void unrollingInTheNest_TryCatchNotBlocking(int[] a, int[] b, int x) {
    for (int k = 0; k < 16; k++) {
      for (int j = 0; j < 16; j++) {
        ...
        // Try catch does not block optimization on the innermost loop.
        try {
          if (doThrow) {
            $noinline$unreachable();
          }
  ...
  public static void main(String[] args) {
    // Use existing tests to show that having a try catch in the graph (inside or outside the loop) is viable.
