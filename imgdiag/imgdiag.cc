...
class ImgDiagDumper {
 public:
  ...
  bool Init() {
    ...
    if (image_diff_pid_ < 0 || zygote_diff_pid_ < 0) {
      // TODO: ComputeDirtyBytes must be modified to support single app/zygote to bootimage comparison
 ...
 private:
  ...
  void PrintMappingData(const MappingData& mapping_data,
                        const ImageHeader& image_header) {
    std::ostream& os = *os_;
  ...
  bool DumpImageDiffMap(const ImageHeader& image_header, const std::string& image_location)
    ...
    if (!ComputeDirtyBytes(image_header, boot_map, remote_contents, zygote_contents, &mapping_data, &error_msg)) {
      os << error_msg;
      return false;
    }
