 ...
 public:
  bool Init(std::ostream& os,
    ...
    for (size_t image_index = 0; image_index != num_image_spaces; ++image_index) {
      ...
      CHECK_EQ(num_dex_files, static_cast<size_t>(dex_caches->GetLength()));
      ...
      for (size_t i = 0; i != num_dex_files; ++i) {
        ...
        CHECK(dex_cache->GetLocation()->Equals(oat_dex_files[i]->GetDexFileLocation().c_str()));
  ...
  const DexFile& GetDexFile(ObjPtr<mirror::DexCache> dex_cache) const
    ...
    for (int32_t i = 0, length = dex_caches->GetLength(); i != length; ++i) {
      if (dex_caches->GetWithoutChecks(i) == dex_cache) {
...
static std::string GetClassDescriptor(ObjPtr<mirror::Class> klass,
  ...
  if (klass->IsPrimitive<kVerifyNone>()) {
    ...
    uint16_t class_def_index = klass->GetDexClassDefIndex<kVerifyNone>();
    CHECK_NE(class_def_index, DexFile::kDexNoIndex16);
...
class RegionSpecializedBase<mirror::Object> : public RegionCommon<mirror::Object> {
 public:
  ...
  void DiffEntryContents(mirror::Object* entry,
    ...
    mirror::Class* klass = entry->GetClass();
    if (entry->IsClass()) {
      os_ << tabs << "Class " << PrettyClass(entry->AsClass(), this->boot_image_container_)
          << " " << entry << "\n";
    } else {
    ...
    for (size_t i = 0, count = entry->SizeOf(); i < count; ++i) {
      if (base_ptr[i] != remote_bytes[i]) {
        ...
        if (field != nullptr) {
          ...
          field = FindFieldContainingOffset(
              entry->AsClass()->GetSFields(), i, this->boot_image_container_);
          if (field != nullptr) {
