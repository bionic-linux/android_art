...
std::string GetInlineCacheLine(
    ...
    const SafeMap<TypeReference, FlattenProfileData::ItemMetadata::InlineCacheInfo>& inline_cache) {
  ...
  for (const auto& [target_ref, inline_cache_data] : inline_cache) {
    ...
    if (inline_cache_data.is_missing_types_) {
      ...
      bool first = true;
      for (const TypeReference& cls_ref : inline_cache_data.classes_) {
        if (!first) {
          dump_ic << kProfileParsingTypeSep;
        }
        first = false;
        dump_ic << profile_info.GetTypeDescriptor(cls_ref.dex_file, cls_ref.TypeIndex());
      }
    }
  }
  return dump_ic.str();
