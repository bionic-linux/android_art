...
bool GenerateBootImageProfile(
  ...
  ProfileCompilationInfo all_profiles(/*for_boot_image=*/ true);
  for (const std::string& profile_file : profile_files) {
    ...
    all_profiles.MergeWith(profile);
  ...
  for (const auto& it : profile_methods) {
    profile_content += MethodToProfileFormat(all_profiles, it.first, it.second, options.append_package_use_list)
        + "\n";
  }
