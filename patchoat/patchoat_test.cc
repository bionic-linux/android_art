...
class PatchoatTest : public DexoptTest {
 public:
  ...
  bool CompileBootImageToDir(
      const std::string& output_dir, uint32_t base_addr, std::string* error_msg) {
  ...
  bool CopyImageChecksumAndSetPatchDelta(
    ...
    if (dest_file.get() == nullptr) {
      *error_msg = StringPrintf("Failed to open destination image file %s", dest_image_filename.c_str());
      return false;
    }
    ImageHeader dest_header;
    if (!dest_file->ReadFully(&dest_header, sizeof(dest_header))) {
      *error_msg = StringPrintf("Failed to read destination image file %s", dest_image_filename.c_str());
      return false;
    }
    dest_header.SetOatChecksum(src_header.GetOatChecksum());
    dest_header.SetPatchDelta(dest_patch_delta);
    if (!dest_file->ResetOffset()) {
      *error_msg = StringPrintf("Failed to seek to start of destination image file %s", dest_image_filename.c_str());
      return false;
    }
    if (!dest_file->WriteFully(&dest_header, sizeof(dest_header))) {
      *error_msg = StringPrintf("Failed to write to destination image file %s", dest_image_filename.c_str());
      dest_file->Erase();
      return false;
    }
    if (dest_file->FlushCloseOrErase() != 0) {
      *error_msg = StringPrintf("Failed to flush/close destination image file %s", dest_image_filename.c_str());
      return false;
    }
  ...
  bool ReadFully(const std::string& filename, std::vector<uint8_t>* contents, std::string* error_msg) {
...
TEST_F(PatchoatTest, PatchoatRelocationSameAsDex2oatRelocation) {
  ...
  std::string error_msg;
  if (!CompileBootImageToDir(dex2oat_orig_dir, orig_base_addr, &error_msg)) {
  ...
  if (!CompileBootImageToDir(dex2oat_reloc_dir, reloc_base_addr, &error_msg)) {
  ...
  std::replace(patchoat_reloc_image_filename.begin() + 1, patchoat_reloc_image_filename.end(), '/', '@');
