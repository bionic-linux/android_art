...
static bool CheckImageIdenticalToOriginalExceptForRelocation(
  ...
  *error_msg = "";
  ...
  if (rel_size < 0) {
    *error_msg = StringPrintf("Error while getting size of image relocation file %s", rel_filename.c_str());
    return false;
  }
  ...
  if (image_file.get() == nullptr) {
    *error_msg = StringPrintf("Unable to open relocated image file  %s", relocated_filename.c_str());
    return false;
  }

  int64_t image_size = image_file->GetLength();
  if (image_size < 0) {
    *error_msg = StringPrintf("Error while getting size of relocated image file %s", relocated_filename.c_str());
    return false;
  }
  if ((image_size % 4) != 0) {
    *error_msg =
  ...
  if (rel_size < SHA256_DIGEST_LENGTH) {
    *error_msg = StringPrintf("Malformed image relocation file %s: too short", rel_filename.c_str());
    return false;
  }
  ...
  // Image in memory is now supposed to be identical to the original.
  // We confirm this by comparing the digest of the in-memory image to the expected digest from relocation file.
...
static std::string Dirname(const std::string& path) {
  // Need to use a copy because dirname may modify contents of its input
  std::string mutable_path(path);
  return std::string(dirname(const_cast<char*>(mutable_path.c_str())));
}

static std::string Basename(const std::string& path) {
  // Need to use a copy because basename may modify contents of its input
  std::string mutable_path(path);
  return std::string(basename(const_cast<char*>(mutable_path.c_str())));
}
...
bool PatchOat::Verify(const std::string& image_location,
  ...
  if (output_image_filename.empty()) {
    LOG(ERROR) << "Relocated image file not provided";
    return false;
  }
  ...
  // TODO: Ensure this is trusted input
  std::vector<gc::space::ImageSpace*> spaces = Runtime::Current()->GetHeap()->GetBootImageSpaces();

  // TODO: Check that no other .rel files exist in the original dir

  bool success = true;
  std::string image_location_dir = Dirname(image_location);
  for (size_t i = 0; i < spaces.size(); ++i) {
    gc::space::ImageSpace* space = spaces[i];
    std::string image_filename = space->GetImageFilename();
    if ((i == 0) && (image_filename != output_image_filename)) {
      LOG(ERROR) << "Unexpected image loaded by ART: " << image_filename << " instead of " << output_image_filename;
      success = false;
      break;
    }

    // /data/dalvik-cache/arm64/system@framework@boot.art -> boot.art
    // /system/framework/arm64/boot.art -> boot.art
    std::string image_basename = Basename(image_filename);
    if (image_basename.find("@") != std::string::npos) {
      image_basename.assign(image_basename.substr(image_basename.find_last_of("@") + 1));
    }
    ...
    std::string original_image_filename = image_location_dir + "/" + isa_name + "/" + image_basename;
    std::string image_relocation_filename = original_image_filename + ".rel";

    std::string error_msg;
    if (!CheckImageIdenticalToOriginalExceptForRelocation(
            image_filename, original_image_filename, &error_msg)) {
      LOG(ERROR) << error_msg;
      success = false;
      break;
    }
