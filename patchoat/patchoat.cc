...
bool PatchOat::Verify(const std::string& image_location,
  ...
  for (size_t i = 0; i < spaces.size(); ++i) {
    ...
    std::string relocated_image_filename = image_filename;
    std::replace(
        relocated_image_filename.begin() + 1, relocated_image_filename.end(), '/', '@');

    relocated_image_filename = output_image_directory + relocated_image_filename;
...
static int patchoat(int argc, char **argv) {
  ...
  std::string output_image_directory = Dirname(output_image_filename);
