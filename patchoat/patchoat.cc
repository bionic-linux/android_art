...
static bool WriteRelFile(const MemMap& original,
                         const MemMap& relocated,
                         const std::string& rel_filename,
                         std::string* error_msg) {
  // FORMAT of the image relocation file:
  ...
  //   is a delta from 0). Each delta is encoded using little-endian variable-length 7 bits per byte
  //   encoding, where all bytes have the highest bit (0x80) set except for the final byte which
  //   does not have that bit set. For example, 0x3f is offset 0x3f, whereas 0xbf 0x05 is offset
  //   (0x3f & 0x7f) | (0x5 << 7) which is 0x2bf. Most deltas end up being encoding using just one
  //   byte, achieving ~4x decrease in relocation file size compared to the encoding where offsets
  //   are stored verbatim, as uint32_t.
  ...
  std::vector<uint8_t> encoded_diff_offsets;
  ...
  for (size_t offset = 0; offset < original_size; offset += 4) {
    ...
    diff_offsets.push_back(offset);

    uint32_t offset_diff = offset - last_diff_offset;
    last_diff_offset = offset;

    // Encode the delta using little-endian 7 bits per byte encoding, with all bytes except the last
    // one having the highest bit set.
    while (offset_diff != 0) {
      uint8_t b = offset_diff & 0x7f;
      offset_diff >>= 7;
      if (offset_diff != 0) {
        b |= 0x80;
      }
      encoded_diff_offsets.push_back(b);
    }
  }

  if (diff_offsets.empty()) {
    *error_msg = "Original and patched images are identical";
    return false;
  }

  uint8_t original_image_digest[SHA256_DIGEST_LENGTH];
  SHA256(original_bytes, original_size, original_image_digest);
  ...
  if (!rel_file->WriteFully(original_image_digest, SHA256_DIGEST_LENGTH)) {
    *error_msg = StringPrintf("Failed to write to %s", rel_filename.c_str());
    return false;
  }
  if (!rel_file->WriteFully(encoded_diff_offsets.data(), encoded_diff_offsets.size())) {
    *error_msg = StringPrintf("Failed to write to %s", rel_filename.c_str());
    return false;
  }
