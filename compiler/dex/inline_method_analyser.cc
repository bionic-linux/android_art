...
class Matcher {
 public:
  ...
  // Similar to Required but we allow matches on either Fn1 or Fn2.
  template <bool (Matcher::*Fn1)(), bool (Matcher::*Fn2)()>
  static bool Or(Matcher* matcher);
...
template <bool (Matcher::*Fn1)(), bool (Matcher::*Fn2)()>
bool Matcher::Or(Matcher* matcher) {
  const bool result = (matcher->*Fn1)() || (matcher->*Fn2)();
  if (!result) {
    return false;
  }
  matcher->pos_ += 1u;
...
bool DoAnalyseConstructor(const CodeItemDataAccessor* code_item,
  ...
  static constexpr Matcher::MatchFn* const kConstructorPattern[] = {
      ...
      &Matcher::Or<&Matcher::Opcode<Instruction::INVOKE_DIRECT>,
