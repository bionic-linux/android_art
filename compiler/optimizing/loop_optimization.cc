...
bool HLoopOptimization::VectorizeIfCondition(LoopNode* node,
  ...
  // Only integral and floating point types are supported, i.e.: Reference types are not supported.
  if (!DataType::IsIntegralType(type) && !DataType::IsFloatingPointType(type)) {
    return false;
  }
