...
HLoopOptimization::HLoopOptimization(HGraph* graph,
    ...
    : HOptimization(graph, kLoopOptimizationPassName),
      ...
      linear_order_(loop_allocator_.Adapter()),  // uses phase-local allocator
      top_loop_(nullptr),
      last_loop_(nullptr) {
}
...
void HLoopOptimization::Run() {
  ...
  // Build the linear order on a phase-local allocated data structure. This step enables building
  // a loop hierarchy that properly reflects the outer-inner and previous-next relation, but avoids
  // keeping the storage of the linear order around as well as having an order that becomes stale
  // by subsequent graph mutations.
  graph_->Linearize(&linear_order_);
  // Build the loop hierarchy using the just computed linear order.
  for (HLinearOrderIterator it_graph(linear_order_); !it_graph.Done(); it_graph.Advance()) {
