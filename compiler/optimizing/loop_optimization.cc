...
HLoopOptimization::HLoopOptimization(HGraph* graph,
    ...
    : HOptimization(graph, name, stats),
      ...
      reductions_(nullptr),
      simplified_(false),
      predicated_vectorization_mode_(graph->HasPredicatedSIMDSupport()),
      vector_length_(0),
    ...
    for (HInstructionIterator it(block->GetInstructions()); !it.Done(); it.Advance()) {
      ...
      if (i != vector_map_->end() && !i->second->IsInBlock()) {
        ...
        if (IsInPredicatedVectorizationMode() && i->second->IsVecOperation()) {
          ...
          op->SetGoverningPredicate(set_pred);
        }
  ...
  if (vector_map_->find(org) == vector_map_->end()) {
    ...
    if (it != vector_permanent_map_->end()) {
      ...
      if (IsInPredicatedVectorizationMode()) {
        ...
        vector->AsVecOperation()->SetGoverningPredicate(set_pred);
      }
    }
...
void HLoopOptimization::GenerateVecReductionPhiInputs(HPhi* phi, HInstruction* reduction) {
  ...
  if (vector_mode_ == kVector) {
    ...
    if (IsInPredicatedVectorizationMode()) {
      ...
      new_init->AsVecOperation()->SetGoverningPredicate(set_pred);
    }
  } else {
  ...
  if (instruction->IsPhi()) {
    ...
    if (HVecOperation::ReturnsSIMDValue(input)) {
      ...
      if (IsInPredicatedVectorizationMode()) {
        ...
        reduce->AsVecOperation()->SetGoverningPredicate(set_pred);
        instruction->AsVecOperation()->SetGoverningPredicate(set_pred);
      }
    }
  }
  return instruction;
