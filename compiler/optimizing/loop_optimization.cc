...
static bool IsOnlyUsedAfterLoop(const HLoopInformation* loop_info, HPhi* phi, HInstruction* addsub) {
...
static HBasicBlock* MayBeRemovePreHeader(HBasicBlock* preheader, HBasicBlock* entry_block) {
...
HLoopOptimization::HLoopOptimization(HGraph* graph,
    ...
    : HOptimization(graph, kLoopOptimizationPassName),
      loop_allocator_(graph_->GetArena()->GetArenaPool()),  // local allocator on global pool
...
void HLoopOptimization::Run() {
  ...
  // properly reflects the outer-inner and prev-next relation.
  graph_->Linearize();
...
void HLoopOptimization::RemoveIfEmptyLoop(LoopNode* node) {
  ...
  if (IsEmptyHeader(header, &addsub) && IsEmptyBody(body, addsub)) {
    HBasicBlock* entry = MayBeRemovePreHeader(preheader, graph_->GetEntryBlock());
