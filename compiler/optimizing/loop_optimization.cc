...
void HLoopOptimization::Run() {
  ...
  SsaLivenessAnalysis liveness(graph_, nullptr);
  liveness.LinearizeGraph();
  ...
  for (HInstructionIterator it(header->GetPhis()); !it.Done(); it.Advance()) {
    ...
    if (IsPhiAddSub(phi, &addsub)) {
      ...
      if (IsOnlyUsedAfterLoop(loop, phi, addsub) && induction_range_.CanGenerateLastValue(phi)) {
        ...
        induction_range_.GenerateLastValue(phi, graph_, loop->GetPreHeader(), &last);
...
void HLoopOptimization::RemoveEmptyLoop(LoopNode* node) {
  ...
  if (exit->GetPredecessors().size() != 1) {
    std::cout << "BIK STRANGE: B" << exit->GetBlockId() << std::endl;  // TODO:
    return;
  }
