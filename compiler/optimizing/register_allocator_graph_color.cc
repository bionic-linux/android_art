...
void RegisterAllocatorGraphColor::AllocateRegisters() {
  ...
  for (bool processing_core_regs : {true, false}) {
    ...
    while (true) {
      ...
      for (InterferenceNode* node : physical_core_nodes_) { node->ClearCoalesceOpportunities(); }
      for (InterferenceNode* node : physical_fp_nodes_) { node->ClearCoalesceOpportunities(); }
...
bool ColoringIteration::ColorInterferenceGraph() {
  ...
  while (!pruned_nodes_.empty()) {
    ...
    if (alias != node) {
      ...
      if (interval->HasHighInterval()) {
        ...
        while (reg < num_regs_ - 1 && (conflict_mask[reg] || conflict_mask[reg + 1])) { reg += 2; }
  ...
  if (!successful) {
    for (LiveInterval* interval : colored_intervals) { interval->ClearRegister(); }
  }
