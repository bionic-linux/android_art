...
void LocationsBuilderARM::VisitInstanceOf(HInstanceOf* instruction) {
  ...
  if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
    codegen_->MaybeAddBakerCcEntrypointTemp(locations);
  }
}
  ...
  if (read_barrier_option == kWithReadBarrier) {
    ...
    if (kUseBakerReadBarrier) {
      ...
      if (kBakerReadBarrierLinkTimeThunksEnableForGcRoots &&
        ...
        constexpr size_t kOffsetFromAdrToReturnAddr = 14u;
        size_t code_size_before_adr = GetAssembler()->CodeSize();
        Label return_address;
        __ add(LR, PC, ShifterOperand(kOffsetFromAdrToReturnAddr));
        ...
        size_t code_size_at_return_address = GetAssembler()->CodeSize();
        CHECK_EQ(code_size_at_return_address - code_size_before_adr, kOffsetFromAdrToReturnAddr);
      } else {
...
void CodeGeneratorARM::MaybeAddBakerCcEntrypointTemp(LocationSummary* locations) {
  DCHECK(kEmitCompilerReadBarrier);
  DCHECK(kUseBakerReadBarrier);
  if (kBakerReadBarrierLinkTimeThunksEnableForFields) {
...
void CodeGeneratorARM::GenerateFieldLoadWithBakerReadBarrier(HInstruction* instruction,
  ...
  if (kBakerReadBarrierLinkTimeThunksEnableForFields &&
    ...
    constexpr size_t kOffsetFromAdrToReturnAddr = 14u + (kPoisonHeapReferences ? 4u : 0u);
    size_t code_size_before_adr = GetAssembler()->CodeSize();
    Label return_address;
    __ add(LR, PC, ShifterOperand(kOffsetFromAdrToReturnAddr));
    __ CmpConstant(kBakerCcEntrypointRegister, 0);
