...
static void GenerateEqualLong(HCondition* cond, CodeGeneratorARM* codegen) {
  ...
  if (right.IsConstant()) {
    ...
    const int64_t value = AdjustConstantForCondition(Int64FromConstant(right.GetConstant()),
                                                     &condition,
                                                     &opposite);
    ShifterOperand right_high = ShifterOperand(High32Bits(value));
    ShifterOperand right_low = ShifterOperand(Low32Bits(value));
    ...
    __ eor(IP, left_high, right_high);
  } else {
    DCHECK(right.IsRegisterPair());
    __ eor(IP, left_high, ShifterOperand(right.AsRegisterPairHigh<Register>()));
