...
static HInstruction* CreateUnsignedConditionReplacement(ArenaAllocator* allocator,
                                                        HCondition* cond,
                                                        HCompare* compare) {
  HInstruction* lhs_cond = cond->InputAt(0);
  HInstruction* rhs_cond = cond->InputAt(1);
  HBasicBlock* block = cond->GetBlock();

  DCHECK(rhs_cond->IsIntConstant());
  DCHECK_EQ(rhs_cond->AsIntConstant()->GetValue(), 0);
  DCHECK(lhs_cond->IsCompare());
...
void InstructionSimplifierVisitor::VisitCondition(HCondition* condition) {
  ...
  DataType::Type compare_type = left->AsCompare()->GetComparisonType();
  if (DataType::IsUnsignedType(compare_type)) {
...
static HInstruction* CheckSignedToUnsignedCompareConversion(HInstruction* operand,
  ...
  if (operand->IsConstant()) {
    ...
    if (constant->IsIntConstant()) {
      ...
      return operand->GetBlock()->GetGraph()->GetIntConstant(newValue, constant->GetDexPc());
      ...
      return operand->GetBlock()->GetGraph()->GetLongConstant(newValue, constant->GetDexPc());
    } else {
      return nullptr;
    }
  }
