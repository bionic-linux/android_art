...
class LoadClassSlowPathARM64 : public SlowPathCodeARM64 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    InvokeRuntimeCallingConvention calling_convention;
    __ Mov(calling_convention.GetRegisterAt(0).W(), cls_->GetTypeIndex().index);
...
vixl::aarch64::Label* CodeGeneratorARM64::NewPcRelativeTypePatch(
    ...
    vixl::aarch64::Label* adrp_label) {
  return NewPcRelativePatch(dex_file, type_index.index, adrp_label, &pc_relative_type_patches_);
}
...
void InstructionCodeGeneratorARM64::VisitLoadClass(HLoadClass* cls) {
  if (cls->NeedsAccessCheck()) {
    codegen_->MoveConstant(cls->GetLocations()->GetTemp(0), cls->GetTypeIndex().index);
  ...
  switch (cls->GetLoadKind()) {
    ...
    case HLoadClass::LoadKind::kDexCacheViaMethod: {
      ...
      GenerateGcRootFieldLoad(cls,
                              out_loc,
                              out.X(),
                              CodeGenerator::GetCacheOffset(cls->GetTypeIndex().index),
                              /* fixup_label */ nullptr,
                              read_barrier_option);
      generate_null_check = !cls->IsInDexCache();
      break;
...
void InstructionCodeGeneratorARM64::VisitNewArray(HNewArray* instruction) {
  ...
  __ Mov(type_index, instruction->GetTypeIndex().index);
