  ...
  if (!needs_write_barrier) {
    ...
    SlowPathCodeARM64* slow_path = nullptr;
    {
      ...
      if (may_need_runtime_call_for_type_check) {
        ...
        codegen_->AddSlowPath(slow_path);
        if (need_null_check) {
          __ Cbz(Register(value), &is_null);
        }

        // It is required to do the card mark this early since it requires 2 scratch registers and
        // we allocate the temp one shortly below.
        // The value is guaranteed not null at this point. It is safe to do the card mark up here
        // since there is no possible thread suspension before the store.
