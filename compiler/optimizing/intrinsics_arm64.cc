...
static void GenCas(CodeGeneratorARM64* codegen,
  ...
  bool relaxed_load = (order == std::memory_order_relaxed) || (order == std::memory_order_release);
  bool relaxed_store = (order == std::memory_order_relaxed) || (order == std::memory_order_acquire);
  DCHECK(relaxed_load || relaxed_store || order == std::memory_order_seq_cst);

  // do {
  //   old_value = [ptr];
  // } while ((old_value == expected || old_value == expected2) && failure([ptr] <- new_value));
  //
...
class ReadBarrierCasSlowPathARM64 : public SlowPathCodeARM64 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) override {
    ...
    if (update_old_value_) {
      // The comparison was successful. Update `old_value_` to the to-space reference `expected_`.
...
static void GenerateVarHandleCompareAndSetOrExchange(HInvoke* invoke,
  ...
  GenerateVarHandleFieldCheck(invoke, codegen, slow_path);
  {
    ...
    GenerateVarHandleAccessModeAndVarTypeChecks(
        invoke, codegen, slow_path, value_type, var_type_no_rb);
    // If the value type is a reference, check `new_value` and `expected` against the varType.
    // False negatives due to varType being an interface or array type
    // or due to the missing read barrier are handled by the slow path.
