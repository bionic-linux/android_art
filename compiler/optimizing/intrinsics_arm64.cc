...
static void GenCas(CodeGeneratorARM64* codegen,
  ...
  DCHECK(ptr.IsX());
  DCHECK_EQ(value.IsX(), type == DataType::Type::kInt64);
  ...
  // do {
  //   old_value = [tmp_ptr];
  // } while ((old_value == expected || old_value == expected2) &&
  //          failure([tmp_ptr] <- r_new_value));
  //
  ...
  // It the comparison succeeded, the X flag is set and remains set after the end of the
  ...
  switch (type) {
    case DataType::Type::kBool:
    case DataType::Type::kInt8:
      if (order == std::memory_order_relaxed || order == std::memory_order_acquire) {
        __ Stxrb(store_result, value, MemOperand(ptr));
      } else {
        __ Stlxrb(store_result, value, MemOperand(ptr));
      }
      break;
    case DataType::Type::kUint16:
    case DataType::Type::kInt16:
      if (order == std::memory_order_relaxed || order == std::memory_order_acquire) {
        __ Stxrh(store_result, value, MemOperand(ptr));
      } else {
        __ Stlxrh(store_result, value, MemOperand(ptr));
      }
      break;
    case DataType::Type::kInt32:
    case DataType::Type::kInt64:
    case DataType::Type::kReference:
      if (order == std::memory_order_relaxed || order == std::memory_order_acquire) {
        __ Stxr(store_result, value, MemOperand(ptr));
      } else {
        __ Stlxr(store_result, value, MemOperand(ptr));
      }
      break;
    default:
      LOG(FATAL) << "Unexpected type: " << type;
 ...
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) override {
    ...
    GenCas(arm64_codegen,
           ...
           order_,
           strong_,
           /*failure=*/ update_old_value_ ? &mark_old_value : GetExitLabel(),
           tmp_ptr,
           value_,
    ...
    // Z=true from the the CMP+CCMP in GenCas() above indicates comparison success.
...
static void GenerateVarHandleCompareAndSetOrExchange(HInvoke* invoke,
  ...
  GenerateVarHandleFieldCheck(invoke, codegen, slow_path);
  {
    ...
    GenerateVarHandleAccessModeAndVarTypeChecks(
        invoke, codegen, slow_path, value_type, var_type_no_rb);
    // If the value type is a reference, check `value` and `expected` against the varType.
    // False negatives due to varType being an interface or array type
    // or due to the missing read barrier are handled by the slow path.
  ...
  UseScratchRegisterScope temps(masm);
  Register ptr = offset.X();
  if (kEmitCompilerReadBarrier && value_type == DataType::Type::kReference) {
    ptr = temps.AcquireX();
  }
  __ Add(ptr, object.X(), offset.X());

  // Move floating point values to temporaries.
  Register expected_reg = MoveToTempIfFpRegister(expected, value_type, masm, &temps);
  Register value_reg = MoveToTempIfFpRegister(value, value_type, masm, &temps);
  ...
  if (return_success) {
    ...
    store_result = value.IsZero() ? temps.AcquireW() : WRegisterFrom(locations->GetTemp(next_temp));
  } else {
  ...
  if (kEmitCompilerReadBarrier && value_type == DataType::Type::kReference) {
    ...
    failure = rb_slow_path->GetEntryLabel();
  }
  ...
  if (return_success) {
    if (strong) {
      ...
      // Determine the final sucess value with a CSEL.
