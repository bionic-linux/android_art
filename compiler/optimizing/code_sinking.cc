  ...
  for (HBasicBlock* block : graph_->GetPostOrder()) {
    if (block == end_block) {
      ...
      if (block->GetSuccessors().empty()) {
        ...
        // We use `GetNormalSuccessors` to not check catch blocks post dominance for try entry blocks.
        // The reason for this is that the TryBoundary is the "first" instrucion of that try, and
        // the last for the block. Therefore those block's instructions are not going to be relevant
        // to the block's xhandler since they are before the try.
        const bool is_try_boundary_entry =
            block->EndsWithTryBoundary() && block->GetLastInstruction()->AsTryBoundary()->IsEntry();
        ArrayRef<HBasicBlock* const> successors =
            is_try_boundary_entry ? block->GetNormalSuccessors() :
