...
RegisterAllocatorLinearScan::~RegisterAllocatorLinearScan() {
  if (stats_ != nullptr) {
    ...
    for (HBasicBlock* block : codegen_->GetGraph()->GetReversePostOrderSkipEntryBlock()) {
    ...
    LOG(ERROR) << "\"" << name << "\"," << int_spill_slots_.size() << ","
               << long_spill_slots_.size() << "," << float_spill_slots_.size() << ","
               << double_spill_slots_.size() << "," << catch_phi_spill_slots_ << ","
               << GetNumberOfSpillSlots() << "," << number_of_instructions;
