...
static uint32_t GetExpectedVarHandleCoordinatesNum(HInvoke *invoke) {
...
static void GenerateSubTypeObjectCheck(Register object,
                                       // Temporary register necessary for loading the super class
                                       Register temp,
...
static void GenerateVarHandleCommonChecks(HInvoke *invoke,
                                          ...
                                          X86Assembler* assembler) {
  LocationSummary* locations = invoke->GetLocations();
  uint32_t expected_coordinates_num = GetExpectedVarHandleCoordinatesNum(invoke);
  ...
  GenerateVarHandleAccessModeCheck(vh_object,
                                   ...
                                   slow_path,
                                   assembler);
  switch (expected_coordinates_num) {
    case 0u:
...
static Register GenerateVarHandleFieldReference(HInvoke* invoke,
  ...
  DCHECK(expected_coordinates_num == 1u);
...
void IntrinsicLocationsBuilderX86::VisitVarHandleGet(HInvoke* invoke) {
  ...
  uint32_t expected_coordinates_num = GetExpectedVarHandleCoordinatesNum(invoke);
  if (expected_coordinates_num > 1u) {
...
void IntrinsicCodeGeneratorX86::VisitVarHandleGet(HInvoke* invoke) {
  ...
  Register ref = GenerateVarHandleFieldReference(invoke, temp, offset, codegen_);
...
void IntrinsicCodeGeneratorX86::VisitVarHandleSet(HInvoke* invoke) {
  ...
  // or the declaring class. The field offset will be placed in 'offset'. For static fields, the
  // declaring class will be placed in 'temp' register.
  Register reference = GenerateVarHandleFieldReference(invoke, temp, offset, codegen_);
  ...
  if (value_type == DataType::Type::kReference) {
    ...
    temp = expected_coordinates_num == 1u ? temp : locations->GetTemp(2).AsRegister<Register>();
    ...
    if (kPoisonHeapReferences && needs_write_barrier) {
      ...
      __ movl(Address(reference, offset, TIMES_1, 0), card);
