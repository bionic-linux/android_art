...
void IntrinsicCodeGeneratorX86::VisitVarHandleGet(HInvoke* invoke) {
  ...
  // Check the varType.primitiveType against the type we're trying to retrieve. For primitive
  // types, we do not need a read barrier when loading a reference only for loading constant
  // field through the reference. Reference types are checked later by an HCheckCast node.
