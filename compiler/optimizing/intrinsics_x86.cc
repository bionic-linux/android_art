...
static void GenerateMathFma(HInvoke* invoke, CodeGeneratorX86* codegen) {
  ...
  X86Assembler* assembler = codegen->GetAssembler();
  bool is_float = invoke->GetType() == DataType::Type::kFloat32;

  XmmRegister left = locations->InAt(1).AsFpuRegister<XmmRegister>();
  XmmRegister right = locations->InAt(2).AsFpuRegister<XmmRegister>();
  XmmRegister accumulator = locations->InAt(0).AsFpuRegister<XmmRegister>();

  is_float ? __ vfmadd213ss(accumulator, left, right) : __ vfmadd213sd(accumulator, left, right);
}
