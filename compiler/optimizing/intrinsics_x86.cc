...
static uint32_t GetExpectedVarHandleCoordinatesCount(HInvoke *invoke) {
  ...
  DCHECK_GE(accessor_argument_count, mirror::VarHandle::kMinCoordinateTypes + var_type_count);
  DCHECK_LE(accessor_argument_count, mirror::VarHandle::kMaxCoordinateTypes + var_type_count);
  return accessor_argument_count - var_type_count;
}

static DataType::Type MaybeGetDataTypeFromShorty(HInvoke* invoke, uint32_t index) {
  ...
  DCHECK_LT(index, invoke->GetNumberOfArguments());
  if (index == 0) {
    // shorty[0] is the return type
    return invoke->GetType();
  }
  return invoke->InputAt(index)->GetType();
}
...
// register. Field's offset is returned though 'offset' register.
static Register GenerateVarHandleFieldReference(HInvoke* invoke,
  ...
  DCHECK(expected_coordinates_count == 1u);
...
void IntrinsicLocationsBuilderX86::VisitVarHandleSet(HInvoke* invoke) {
  ...
  DataType::Type value_type = MaybeGetDataTypeFromShorty(invoke, value_index);
