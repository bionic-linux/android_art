...
void IntrinsicCodeGeneratorX86::VisitVarHandleGet(HInvoke* invoke) {
  ...
  // are also checked later by an HCheckCast node as an additional check.
  // We do not need a read barrier when loading a reference only for loading constant field through
  // the reference.
