...
void InstructionCodeGeneratorMIPS64::GenerateMinMax(LocationSummary* locations,
                                                    bool is_min,
                                                    // FIXME: the original code handled int/long
                                                    // similary, that cannot be right?
                                                    DataType::Type type ATTRIBUTE_UNUSED) {
  GpuRegister lhs = locations->InAt(0).AsRegister<GpuRegister>();
  GpuRegister rhs = locations->InAt(1).AsRegister<GpuRegister>();
  GpuRegister out = locations->Out().AsRegister<GpuRegister>();
