...
void LocationsBuilderARMVIXL::VisitInstanceFieldSet(HInstanceFieldSet* instruction) {
  HandleFieldSet(
      instruction, instruction->GetFieldInfo(), instruction->GetWriteBarrierKind());
}
  ...
  switch (value_type) {
    ...
    case DataType::Type::kReference: {
      ...
      if (instruction->GetWriteBarrierKind() != WriteBarrierKind::kDontEmit) {
        DCHECK(instruction->GetWriteBarrierKind() == WriteBarrierKind::kEmitNoNullCheck);
        codegen_->MarkGCCard(
            temp1, temp2, array, value, /* emit_null_check= */ false);
      }
