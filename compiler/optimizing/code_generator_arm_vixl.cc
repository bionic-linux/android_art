...
static void GenerateEqualLong(HCondition* cond, CodeGeneratorARMVIXL* codegen) {
  ...
  Operand right_high(0);
  Operand right_low(0);
  vixl32::Register temp;
  UseScratchRegisterScope temps(codegen->GetVIXLAssembler());

  if (right.IsConstant()) {
    const int64_t value = Int64ConstantFrom(right);

    right_high = High32Bits(value);
    right_low = Low32Bits(value);
  } else {
    DCHECK(right.IsRegisterPair());
    right_high = HighRegisterFrom(right);
    right_low = LowRegisterFrom(right);
  }

  // The output uses Location::kNoOutputOverlap.
  if (out.Is(left_high) || (!right.IsConstant() && out.Is(HighRegisterFrom(right)))) {
    std::swap(left_low, left_high);
    std::swap(right_low, right_high);
  }

  // Avoid 32-bit instructions if possible.
  if (!right.IsConstant() && out.Is(right_low.GetBaseRegister())) {
    temp = right_low.GetBaseRegister();
    right_low = left_low;
    left_low = temp;
  }
...
static void GenerateConditionLong(HCondition* cond, CodeGeneratorARMVIXL* codegen) {
  ...
  if (right.IsConstant()) {
    ...
    // Comparisons against 0 are common enough to deserve special attention.
    if (AdjustConstantForCondition(Int64ConstantFrom(right), &condition) == 0) {
      switch (condition) {
        case kCondNE:
        case kCondA:
          // This case uses an IT block, so we must set the flags.
...
static void GenerateConditionIntegralOrNonPrimitive(HCondition* cond, CodeGeneratorARMVIXL* codegen) {
  ...
  if (c == kCondEQ || c == kCondNE) {
    Operand operand = InputOperandAt(cond, 1);

    // Avoid 32-bit instructions if possible.
    if (!right.IsConstant() && out.Is(operand.GetBaseRegister())) {
      in = operand.GetBaseRegister();
