...
void InstructionWithAbsorbingInputSimplifier::VisitAbove(HAbove* instruction) {
  if (!DataType::IsFloatingPointType(instruction->GetLeft()->GetType()) &&
    ...
    //    ABOVE lhs, lhs
    //    CONSTANT false
    // FP special values work here because they return `false` to `Above` comparisons, but due
    // to other optimizations we might end up returning the wrong value.
    instruction->ReplaceWith(GetGraph()->GetConstant(DataType::Type::kBool, 0));
    instruction->GetBlock()->RemoveInstruction(instruction);
...
void InstructionWithAbsorbingInputSimplifier::VisitBelow(HBelow* instruction) {
  if (!DataType::IsFloatingPointType(instruction->GetLeft()->GetType()) &&
    ...
    //    BELOW lhs, lhs
    //    CONSTANT false
    // FP special values work here because they return `false` to `Below` comparisons, but due
    // to other optimizations we might end up returning the wrong value.
    instruction->ReplaceWith(GetGraph()->GetConstant(DataType::Type::kBool, 0));
    instruction->GetBlock()->RemoveInstruction(instruction);
  } else if (instruction->GetRight()->IsConstant() &&
...
void InstructionWithAbsorbingInputSimplifier::VisitGreaterThan(HGreaterThan* instruction) {
  if (!DataType::IsFloatingPointType(instruction->GetLeft()->GetType()) &&
    ...
    //    CONSTANT false
    // FP special values work here because they return `false` to `GreaterThan` comparisons, but due
    // to other optimizations we might end up returning the wrong value.
    instruction->ReplaceWith(GetGraph()->GetConstant(DataType::Type::kBool, 0));
    instruction->GetBlock()->RemoveInstruction(instruction);
  }
...
void InstructionWithAbsorbingInputSimplifier::VisitLessThan(HLessThan* instruction) {
  if (!DataType::IsFloatingPointType(instruction->GetLeft()->GetType()) &&
      instruction->GetLeft() == instruction->GetRight()) {  // Replace code looking like
    //    LESS_THAN lhs, lhs
    //    CONSTANT false
    // FP special values work here because they return `false` to `LessThan` comparisons, but due
    // to other optimizations we might end up returning the wrong value.
    instruction->ReplaceWith(GetGraph()->GetConstant(DataType::Type::kBool, 0));
    instruction->GetBlock()->RemoveInstruction(instruction);
  }
