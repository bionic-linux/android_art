...
void HConstantFoldingVisitor::VisitArrayLength(HArrayLength* inst) {
  ...
  if (input->IsNewArray()) {
    ...
    if (input->IsIntConstant()) {
      inst->ReplaceWith(input);
    }
  } else if (input->IsLoadString()) {
    HLoadString* load_string = input->AsLoadString();
    const DexFile& dex_file = load_string->GetDexFile();
    const dex::StringId& id = dex_file.GetStringId(load_string->GetStringIndex());
    inst->ReplaceWith(GetGraph()->GetIntConstant(dex_file.GetStringLength(id)));
  }
}
