...
static bool IsNarrowingIntegralConversion(DataType::Type from, DataType::Type to) {
  switch (from) {
    ...
    case DataType::Type::kInt16: return to == DataType::Type::kUint8 || to == DataType::Type::kInt8;
...
static DataType::Type ImplicitConversion(DataType::Type type) {
  switch (type) {
    ...
    case DataType::Type::kUint16:
    case DataType::Type::kInt16: return DataType::Type::kInt32;
    default: return type;
  }
}
      ...
      switch (op) {
        ...
        case kXor:
          ...
          return CreateInduction(kPeriodic,
                                 ...
                                 /*fetch*/ nullptr,
                                 type_);
        default: LOG(FATAL) << op; UNREACHABLE();
      }
    }
  }
  return nullptr;
...
bool HInductionVarAnalysis::IsTaken(InductionInfo* lower_expr,
  ...
  switch (cmp) {
    ...
    case kCondGE:
      return IsAtLeast(lower_expr, &lower_value) && IsAtMost(upper_expr, &upper_value) &&
             lower_value >= upper_value;
    default: LOG(FATAL) << "CONDITION UNREACHABLE"; UNREACHABLE();
  }
}
...
bool HInductionVarAnalysis::IsFinite(InductionInfo* upper_expr,
  ...
  switch (cmp) {
    ...
    case kCondLE: return (IsAtMost(upper_expr, &value) && value <= (max - stride_value));
    case kCondGT:
      return stride_value == -1 ||
             (IsAtLeast(upper_expr, &value) && value >= (min - stride_value - 1));
    case kCondGE: return (IsAtLeast(upper_expr, &value) && value >= (min - stride_value));
    default: LOG(FATAL) << "CONDITION UNREACHABLE"; UNREACHABLE();
  }
}
