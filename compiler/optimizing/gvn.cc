...
bool GlobalValueNumberer::TryReplaceWithOpposite(HCondition* condition, ValueSet* set) const {
  ...
  HInstruction* replacement = HCondition::Create(graph_,
                                                 condition->AsCondition()->GetOppositeCondition(),
                                                 condition->GetLeft(),
                                                 condition->GetRight());
  condition->GetBlock()->InsertInstructionBefore(replacement, condition);
  HInstruction* existing = set->Lookup(replacement);
  // Undo adding the fake replacement which was only used to check if the same instruction existed.
  replacement->GetBlock()->RemoveInstruction(replacement);
  if (existing == nullptr) {
    return false;
  }
  // We have to add an HNot since we searched for the opposite.
  HBooleanNot* boolean_not = new (graph_->GetAllocator()) HBooleanNot(existing);
  condition->GetBlock()->InsertInstructionBefore(boolean_not, condition);
  // We add the new instruction to the ValueSet to be able to be reused.
  set->Add(boolean_not);
  condition->ReplaceWith(boolean_not);
  condition->GetBlock()->RemoveInstruction(condition);
  return true;
}
