...
bool GlobalValueNumberer::TryReplaceWithOpposite(HCondition* condition, ValueSet* set) const {
  ...
  HInstruction* opposite = HCondition::Create(graph_,
                                              condition->AsCondition()->GetOppositeCondition(),
                                              condition->GetLeft(),
                                              condition->GetRight());
  condition->GetBlock()->InsertInstructionBefore(opposite, condition);
  ...
  // BooleanNot, so we try not to add it, if possible.
