...
bool HInliner::CanInlineBody(const HGraph* callee_graph,
  ...
  if (!has_one_return) {
    // If a method has a try catch, all throws are potentially be caught. We are conservative and
    ...
    // Some methods like infinite loops with try catches might fall into this line too. This will
    // mean that the stat `kNotInlinedAlwaysThrows` might not be 100% correct but:
