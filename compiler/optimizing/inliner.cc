...
void HInliner::UpdateInliningBudget() {
  if (total_number_of_instructions_ >= options_.InlinerMaximumNumberOfTotalInstructions()) {
    ...
    inlining_budget_ = std::max(options_.InlinerMaximumNumberOfInstructionsForSmallMethod(),
                                options_.InlinerMaximumNumberOfTotalInstructions() -
                                    (unsigned int)total_number_of_instructions_);
  }
}
