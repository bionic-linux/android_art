...
class HVecOperation : public HVariableInputSizeInstruction {
 public:
  ...
  // Returns whether the vector operation must be predicated. The method reflects semantics of
  ...
  // all instructions of the the same vector operation class must be either all predicated or all
  // not predicated (depending on the predicated SIMD support) in a correct graph.
  virtual bool MustBePredicated() {
    return GetBlock()->GetGraph()->HasPredicatedSIMDSupport();
  }
  ...
  // See HVecPredSetOperation.
  void SetGoverningPredicate(HInstruction* input,
                             PredicationKind pred_kind = PredicationKind::kMergingForm) {
    DCHECK(!IsPredicated());
...
// An abstract class for instructions of which the output value is a vector predicate -
// a special kind of of vector value:
//
...
// The value determines whether a vector element will be involved into OP calculations or not
...
class HVecPredSetAll final : public HVecPredSetOperation {
 public:
  ...
  bool MustBePredicated() override { return false; }
...
// Takes two scalar values X and Y, creates a series seq_n = X + n, compares (OP) each seq_n with
// y and set the corresponding element of the predicate register to the result of the comparison.
//
