...
static void CreateVarHandleGetAndUpdateLocations(HInvoke* invoke,
  ...
  LOG(ERROR) << "Before " << *invoke << " " << std::boolalpha << non_void << std::noboolalpha << " " << locations->GetTempCount() << " " << value_type << " " << return_type;
  if (!non_void) {
    if (DataType::IsFloatingPointType(value_type)) {
  ...
  LOG(ERROR) << "After " << *invoke << " " << std::boolalpha << non_void << std::noboolalpha << " " << locations->GetTempCount() << " " << value_type << " " << return_type;
}
...
static void GenerateVarHandleGetAndUpdate(HInvoke* invoke,
  ...
  LOG(ERROR) << "INFO: " << std::boolalpha << is_fp << " " << use_cas << " " << is_small << " "
             << is_small_and << " " << is_reference << " " << std::noboolalpha << op_type;
  ...
  auto get_temp = [&]() {
    if (available_scratch_registers != 0u) {
      LOG(ERROR) << "Using available_scratch_registers " << available_scratch_registers << " " << *invoke << " " << std::boolalpha << non_void << std::noboolalpha << " " << locations->GetTempCount() << " " << value_type << " " << return_type;
      available_scratch_registers -= 1u;
      return srs.AllocateXRegister();
    } else {
      LOG(ERROR) << "Inside get_temp " << *invoke << " " << std::boolalpha << non_void << std::noboolalpha << " " << locations->GetTempCount() << " " << value_type << " " << return_type << " next_temp: " << next_temp;
    ...
    if (byte_swap) {
      ...
      if (codegen->EmitBakerReadBarrier()) {
        ...
        SlowPathCodeRISCV64* rb_slow_path = codegen->AddReadBarrierSlowPath(
            invoke, result, result, base_loc, /*offset=*/ 0u, index);
