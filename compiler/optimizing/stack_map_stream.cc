...
size_t StackMapStream::PrepareForFillIn() {
  ...
  PrepareMethodIndices();  // Must be done before calling ComputeInlineInfoEncoding.
...
void StackMapStream::PrepareMethodIndices() {
  method_indices_.reserve(stack_maps_.size() + inline_infos_.size());
  ArenaUnorderedMap<uint32_t, size_t> dedupe(allocator_->Adapter(kArenaAllocStackMapStream));
  for (StackMapEntry& stack_map : stack_maps_) {
    const size_t index = method_indices_.size();
    ...
    if (method_index != DexFile::kDexNoIndex) {
      ...
      method_indices_.push_back(method_index);
    }
  }
    ...
    if (entry.inlining_depth != 0) {
      ...
      for (size_t d = 0; d < entry.inlining_depth; ++d) {
        ...
        if (inline_info.EncodesArtMethodAtDepth(encoding.inline_info.encoding, d)) {
          ...
          DCHECK_EQ(inline_info.GetMethodIndexIdxAtDepth(encoding.inline_info.encoding, d),
                    inline_entry.dex_method_index_idx);
        }
