...
void CodeGeneratorX86::MarkGCCard(Register temp,
                                  ...
                                  Register object,
                                  Register value,
                                  bool write_barrier_can_be_skipped_if_null) {
  NearLabel is_null;
  if (write_barrier_can_be_skipped_if_null) {
...
void LocationsBuilderX86::HandleFieldSet(HInstruction* instruction,
                                         const FieldInfo& field_info,
                                         bool maybe_generate_write_barrier) {
  ...
  if (is_byte_type) {
    ...
    if (CodeGenerator::StoreNeedsWriteBarrier(field_type, instruction->InputAt(1))) {
      if (maybe_generate_write_barrier) {
        locations->AddTemp(Location::RequiresRegister());
...
void InstructionCodeGeneratorX86::HandleFieldSet(HInstruction* instruction,
                                                 ...
                                                 bool maybe_generate_write_barrier,
                                                 bool always_generate_write_barrier) {
  LocationSummary* locations = instruction->GetLocations();
  ...
  if (needs_write_barrier && maybe_generate_write_barrier) {
    Register temp = locations->GetTemp(0).AsRegister<Register>();
    ...
    codegen_->MarkGCCard(temp,
                         ...
                         value_can_be_null && !always_generate_write_barrier);
  }
...
void InstructionCodeGeneratorX86::HandleFieldSet(HInstruction* instruction,
  ...
  HandleFieldSet(instruction,
                 ...
                 is_volatile,
                 value_can_be_null,
                 maybe_generate_write_barrier,
                 always_generate_write_barrier);
...
void LocationsBuilderX86::VisitStaticFieldSet(HStaticFieldSet* instruction) {
  HandleFieldSet(
      instruction, instruction->GetFieldInfo(), instruction->GetMaybeGenerateWriteBarrier());
}
...
void InstructionCodeGeneratorX86::VisitStaticFieldSet(HStaticFieldSet* instruction) {
  HandleFieldSet(instruction,
                 ...
                 instruction->GetMaybeGenerateWriteBarrier(),
                 instruction->GetAlwaysGenerateWriteBarrier());
}
...
void LocationsBuilderX86::VisitArraySet(HArraySet* instruction) {
  ...
  if (needs_write_barrier) {
    ...
    locations->AddTemp(Location::RequiresRegister());
    if (instruction->GetMaybeGenerateWriteBarrier()) {
    ...
    case DataType::Type::kReference: {
      ...
      if (instruction->GetMaybeGenerateWriteBarrier()) {
        ...
        codegen_->MarkGCCard(temp,
                             ...
                             array,
                             value.AsRegister<Register>(),
                             /* write_barrier_can_be_skipped_if_null= */ false);
      }
