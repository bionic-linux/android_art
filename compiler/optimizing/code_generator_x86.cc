...
void ParallelMoveResolverX86::MoveMemory32(int dst, int src, int number_of_words) {
  ScratchRegisterScope ensure_scratch(
      this, kNoRegister, EAX, codegen_->GetNumberOfCoreRegisters());
  Register temp_reg = static_cast<Register>(ensure_scratch.GetRegister());
  int stack_offset = ensure_scratch.IsSpilled() ? kX86WordSize : 0;

  // Now that temp register is avaible (possibly spilled), move blocks of memory.
...
void ParallelMoveResolverX86::EmitMove(size_t index) {
  ...
  if (source.IsRegister()) {
    ...
    if (destination.IsRegister()) {
      ...
      DCHECK(destination.IsStackSlot());
      MoveMemory32(destination.GetStackIndex(), source.GetStackIndex(), 1);
    }
  } else if (source.IsDoubleStackSlot()) {
    if (destination.IsRegisterPair()) {
      __ movl(destination.AsRegisterPairLow<Register>(), Address(ESP, source.GetStackIndex()));
      ...
      DCHECK(destination.IsDoubleStackSlot()) << destination;
      MoveMemory32(destination.GetStackIndex(), source.GetStackIndex(), 2);
    }
    ...
    if (destination.IsFpuRegister()) {
      ...
      DCHECK(destination.IsSIMDStackSlot());
      MoveMemory32(destination.GetStackIndex(), source.GetStackIndex(), 4);
    }
  } else if (source.IsConstant()) {
    HConstant* constant = source.GetConstant();
...
void ParallelMoveResolverX86::Exchange128(XmmRegister reg, int mem) {
  ...
  ExchangeMemory32(0, mem + extra_slot, 4);
  __ movups(XmmRegister(reg), Address(ESP, 0));
  __ addl(ESP, Immediate(extra_slot));
}

void ParallelMoveResolverX86::ExchangeMemory32(int mem1, int mem2, int number_of_words) {
...
void ParallelMoveResolverX86::EmitSwap(size_t index) {
  ...
  if (source.IsRegister() && destination.IsRegister()) {
    ...
    ExchangeMemory32(destination.GetStackIndex(), source.GetStackIndex(), 1);
    ...
    ExchangeMemory32(destination.GetStackIndex(), source.GetStackIndex(), 2);
  } else if (source.IsSIMDStackSlot() && destination.IsSIMDStackSlot()) {
    ExchangeMemory32(destination.GetStackIndex(), source.GetStackIndex(), 4);
