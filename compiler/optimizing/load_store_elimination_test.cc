 ...
 public:
  ...
  void CreateTestControlFlowGraph() {
    ...
    std::tie(pre_header_, loop_) =
        CreateDoWhileLoopWithInstructions(return_block_, {array_, i_, j_});
    phi_ = loop_->GetFirstPhi()->AsPhi();
    suspend_check_ = loop_->GetFirstInstruction();
    CHECK(suspend_check_->IsSuspendCheck());
  }
...
TEST_F(LoadStoreEliminationTest, ArrayNonLoopPhi) {
  ...
  // left
  HInvoke* left_value = MakeInvokeStatic(left, DataType::Type::kInt32, { zero_const }, { alloc_w });
  ...
  // right
  HInvoke* right_value =
      MakeInvokeStatic(right, DataType::Type::kInt32, { one_const }, { alloc_w });
...
TEST_P(TwoTypesConversionsTestGroup, StoreLoad) {
  ...
  // `Uint8` is not a valid field type but it's a valid load type we can set for `read` below.
  DataType::Type field_type =
      (load_type == DataType::Type::kUint8) ? DataType::Type::kInt8 : load_type;
  ...
  if (DataType::IsTypeConversionImplicit(param_type, load_type)) {
    ASSERT_TRUE(ret_input->IsParameterValue()) << ret_input->DebugName();
  } else {
    ASSERT_TRUE(ret_input->IsTypeConversion()) << ret_input->DebugName();
    ASSERT_EQ(load_type, ret_input->GetType());
    ASSERT_TRUE(ret_input->InputAt(0)->IsParameterValue()) << ret_input->InputAt(0)->DebugName();
  }
}
...
TEST_P(TwoTypesConversionsTestGroup, StoreLoadStoreLoad) {
  ...
  // `Uint8` is not a valid field type but it's a valid load type we can set for `read` below.
  DataType::Type field_type1 =
      (load_type1 == DataType::Type::kUint8) ? DataType::Type::kInt8 : load_type1;
  DataType::Type field_type2 =
      (load_type2 == DataType::Type::kUint8) ? DataType::Type::kInt8 : load_type2;
  ...
  // TODO: Sometimes we create two type conversions when one is enough (Int32->Int16->Int8).
  ...
  ASSERT_TRUE(current->IsParameterValue()) << current->DebugName();
}

TEST_P(TwoTypesConversionsTestGroup, DefaultValueStores_LoadAfterLoop) {
  auto [default_load_type, load_type] = GetParam();
  // `Uint8` is not a valid field type but it's a valid load type we can set for `read` below.
  DataType::Type default_field_type =
      (default_load_type == DataType::Type::kUint8) ? DataType::Type::kInt8 : default_load_type;
  DataType::Type field_type =
      (load_type == DataType::Type::kUint8) ? DataType::Type::kInt8 : load_type;
...
TEST_P(TwoTypesConversionsTestGroup, SingleValueStores_LoadAfterLoop) {
  ...
  // To avoid write elimination in the body, make a call that can clobber all fields.
  MakeInvokeStatic(loop_body, DataType::Type::kVoid, {object});

  // Write the same value in pre-header and body.
  HInstruction* write1 =
...
TEST_P(TwoTypesConversionsTestGroup, MergingConvertedValueStore) {
  ...
  if (DataType::IsTypeConversionImplicit(param_type, load_type)) {
    ...
    ASSERT_TRUE(pre_header_input->IsParameterValue());
