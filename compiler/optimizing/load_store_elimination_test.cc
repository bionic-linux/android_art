...
class LoadStoreEliminationTestBase : public SuperTest, public OptimizingUnitTestHelper {
 public:
  ...
  void CreateEntryBlockInstructions() {
    ...
    i_add1_ = MakeAdd(entry_block_, DataType::Type::kInt32, i_, c1);
    i_add4_ = MakeAdd(entry_block_, DataType::Type::kInt32, i_, c4);
  ...
  void CreateTestControlFlowGraph() {
    ...
    HInstruction* inc_phi = MakeAdd(loop_, DataType::Type::kInt32, phi_, c1);
...
#undef GET_BLOCK
  ...
  // BODY
  HInstruction* last_i = MakeSub(loop_body, DataType::Type::kInt32, i_phi, one_const);
  ...
  HInstruction* i_next = MakeAdd(loop_body, DataType::Type::kInt32, i_phi, one_const);
  MakeGoto(loop_body);
...
#undef GET_BLOCK
  ...
  HInstruction* add = MakeAdd(ret, DataType::Type::kInt32, read_1, read_2);
  MakeReturn(ret, add);
...
#undef GET_BLOCK
  ...
  HInstruction* body_add = MakeAdd(body, DataType::Type::kInt32, i_phi, c1);
  MakeGoto(body);
  ...
  // ret
  HInstruction* ret_sub = MakeSub(ret, DataType::Type::kInt32, i_phi, c1);
...
#undef GET_BLOCK
  ...
  HInstruction* ret_add = MakeAdd(ret, DataType::Type::kInt32, ret_get1, ret_get2);
