...
//
// In order to do this, we set `kFlagMaybeGenerateWriteBarrier` on the instructions that can ignore
// the write barrier. For InstanceFieldSet and StaticFieldSet, we also set
// `kFlagAlwaysGenerateWriteBarrier` in the instructions that we expect a write barrier. We do this
// because we have an optimization in codegen for these two HInstructions to avoid a write barrier
// at runtime in some cases. If `kFlagAlwaysGenerateWriteBarrier` is set, we will not perform that
