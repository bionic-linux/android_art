...
//
// In order to do this, we clear `kFlagMaybeGenerateWriteBarrier` on the instructions that can
// ignore the write barrier. For InstanceFieldSet and StaticFieldSet, we also set
// `kFlagAlwaysGenerateWriteBarrier` in the instructions that we must have a write barrier. We do
// this because we have an optimization in codegen for these two HInstructions to avoid a write
// barrier at runtime in some cases. If `kFlagAlwaysGenerateWriteBarrier` is set, we will not
// perform that runtime optimization.
