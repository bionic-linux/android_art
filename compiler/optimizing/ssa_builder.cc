    ...
    do {
      ...
      if (current->IsNewInstance()) {
        ...
        if (maybe_post_dominated.IsBitSet(current->GetBlock()->GetBlockId())) {
          ...
          // If the invoke does not post dominate the instruction, we know we are not going to replace
          // dominated uses of the instruction with the invoke. We should also not visit 
          if (kIsDebugBuild) {
