...
bool SsaBuilder::ReplaceUninitializedStringPhis() {
  for (HInvoke* invoke : uninitialized_string_phis_) {
    HInstruction* string = invoke->InputAt(invoke->InputCount() - 1);
    if (string->IsPhi()) {
      ...
      DCHECK(string->IsLoopHeaderPhi());
      return false;
    } else {
      DCHECK(string->IsNewInstance());
      AddUninitializedString(string->AsNewInstance());
    }
    string->ReplaceUsesDominatedBy(invoke, invoke);
    string->ReplaceEnvUsesDominatedBy(invoke, invoke);
...
GraphAnalysisResult SsaBuilder::BuildSsa() {
  ...
  // Replace Phis that feed in a String.<init> during instruction building.
