...
void CodegenTest::TestVectorComparison(IfCondition condition,
  ...
  // VecPredToBoolean is only used together with VecPredWhile in production so it is currently
  // limited to using PCondKind::kNFirst (see
  // InstructionCodeGeneratorARM64Sve::VisitVecPredToBoolean) while we want to test
  // PCondKind::kFirst. To workaround this, NOT the expected result as the comparison will set the
  // first condition flag if true.
