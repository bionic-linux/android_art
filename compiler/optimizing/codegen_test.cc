...
TEST_F(CodegenTest, ARMVIXLParallelMoveResolver) {
  ...
  HGraph* graph = CreateGraph();
  arm::CodeGeneratorARMVIXL codegen(graph, CompilerOptions());
...
TEST_F(CodegenTest, ARM64ParallelMoveResolverB34760542) {
  ...
  HGraph* graph = CreateGraph();
  arm64::CodeGeneratorARM64 codegen(graph, CompilerOptions());
...
TEST_F(CodegenTest, MipsClobberRA) {
  ...
  mips::CodeGeneratorMIPS codegenMIPS(graph, CompilerOptions());
