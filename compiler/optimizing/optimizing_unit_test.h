...
class OptimizingUnitTestHelper {
 public:
  ...
  // Insert "pre-header" and "loop" blocks before a given `loop_exit` block and connect them in
  // `do { ... } while (...);` loop pattern. Return the new blocks. Adds `HGoto` to the "pre-header"
  // block but leaves the "loop-header" block empty, leaving the construction of an appropriate
  // condition and `HIf` to the caller.
