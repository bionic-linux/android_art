 ...
 public:
  ...
  void VisitInstanceFieldSet(HInstanceFieldSet* instruction) override {
    ...
    if (it != current_write_barriers_.end()) {
      ...
      DCHECK(it->second->AsInstanceFieldSet()->GetMaybeGenerateWriteBarrier());
      DCHECK_EQ(it->second->GetBlock(), instruction->GetBlock());
      it->second->AsInstanceFieldSet()->SetAlwaysGenerateWriteBarrier();
      instruction->ClearMaybeGenerateWriteBarrier();
      ...
      DCHECK(inserted);
      DCHECK(instruction->GetMaybeGenerateWriteBarrier());
    }
  }
  ...
  void VisitStaticFieldSet(HStaticFieldSet* instruction) override {
    ...
    if (it != current_write_barriers_.end()) {
      ...
      DCHECK(it->second->AsStaticFieldSet()->GetMaybeGenerateWriteBarrier());
      DCHECK_EQ(it->second->GetBlock(), instruction->GetBlock());
      it->second->AsStaticFieldSet()->SetAlwaysGenerateWriteBarrier();
      instruction->ClearMaybeGenerateWriteBarrier();
      MaybeRecordStat(stats_, MethodCompilationStat::kRemovedWriteBarrier);
    } else {
  ...
  void VisitArraySet(HArraySet* instruction) override {
    ...
    if (it != current_write_barriers_.end()) {
      ...
      DCHECK(it->second->AsArraySet()->GetMaybeGenerateWriteBarrier());
      DCHECK_EQ(it->second->GetBlock(), instruction->GetBlock());
      // No need to SetAlwaysGenerateWriteBarrier, as we never skip a write barrier for array sets
      // in codegen.
      instruction->ClearMaybeGenerateWriteBarrier();
      MaybeRecordStat(stats_, MethodCompilationStat::kRemovedWriteBarrier);
    } else {
