 ...
 public:
  ...
  void VisitInstanceFieldSet(HInstanceFieldSet* instruction) override {
    ...
    if (it != current_write_barriers_.end()) {
      ...
      it->second->AsInstanceFieldSet()->SetWriteBarrierBeingReliedOn();
  ...
  void VisitStaticFieldSet(HStaticFieldSet* instruction) override {
    ...
    if (it != current_write_barriers_.end()) {
      ...
      DCHECK_EQ(it->second->GetBlock(), instruction->GetBlock());
      it->second->AsStaticFieldSet()->SetWriteBarrierBeingReliedOn();
      instruction->SetIgnoreWriteBarrier();
      MaybeRecordStat(stats_, MethodCompilationStat::kRemovedWriteBarrier);
    } else {
  ...
  void VisitArraySet(HArraySet* instruction) override {
    ...
    if (it != current_write_barriers_.end()) {
      ...
      DCHECK_EQ(it->second->GetBlock(), instruction->GetBlock());
      // No need to SetWriteBarrierBeingReliedOn, as we never skip a write barrier for array sets in
      // codegen.
      instruction->SetIgnoreWriteBarrier();
      MaybeRecordStat(stats_, MethodCompilationStat::kRemovedWriteBarrier);
    } else {
