  ...
  switch (instruction->GetPackedType()) {
    case DataType::Type::kInt32:
      ...
      switch (instruction->GetReductionKind()) {
        case HVecReduce::kSum: __ Addv(dst.S(), src.V4S()); break;
        case HVecReduce::kMin: __ Sminv(dst.S(), src.V4S()); break;
        case HVecReduce::kMax: __ Smaxv(dst.S(), src.V4S()); break;
      }
      break;
    case DataType::Type::kInt64:
      DCHECK_EQ(2u, instruction->GetVectorLength());
      switch (instruction->GetReductionKind()) {
        case HVecReduce::kSum: __ Addp(dst.D(), src.V2D()); break;
        default: LOG(FATAL) << "Unsupported SIMD min/max"; UNREACHABLE();
      }
      break;
    default: LOG(FATAL) << "Unsupported SIMD type: " << instruction->GetPackedType(); UNREACHABLE();
  }
...
void InstructionCodeGeneratorARM64Neon::VisitVecSADAccumulate(HVecSADAccumulate* instruction) {
  ...
  switch (a->GetPackedType()) {
    ...
    case DataType::Type::kInt64:
      ...
      break;
    default: LOG(FATAL) << "Unsupported SIMD type: " << instruction->GetPackedType();
  }
}
