...
class HLoopOptimization : public HOptimization {
 ...
 private:
  ...
  // Phase-local heap memory allocator for the loop optimizer (we can release all
  // storage required for e.g. the linear order and loop hierarchy when done).
  ArenaAllocator loop_allocator_;

  // Linear order of the graph, required to compute loop hierarchy.
