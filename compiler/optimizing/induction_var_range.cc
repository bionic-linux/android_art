...
bool InductionVarRange::GetInductionRange(HInstruction* context,
  ...
  *needs_finite_test = NeedsTripCount(info, &s) && IsUnsafeTripCount(trip);
  return true;
}
...
bool InductionVarRange::CanGenerateRange(HInstruction* context,
  ...
  return GenerateCode(context,
                      ...
                      &s, needs_finite_test, needs_taken_test)
      && (s == -1 || s == 0 || s == 1);  // avoid wrap-around anomalies.
}
...
void InductionVarRange::GenerateRange(HInstruction* context,
  ...
  if (!GenerateCode(context,
                    instruction,
                    is_last_value,
                    graph, block, lower, upper, nullptr,
                    &s, &b1, &b2)) {
...
void InductionVarRange::GenerateTakenTest(HInstruction* context,
  ...
  if (!GenerateCode(context,
                    context,
                    is_last_value,
                    graph, block, nullptr, nullptr, taken_test,
                    &s, &b1, &b2)) {
    LOG(FATAL) << "Failed precondition: CanGenerateRange()";
  }
}
...
void InductionVarRange::GenerateLastValue(HInstruction* instruction, HGraph* graph, HBasicBlock* block, HInstruction** last) {
  bool is_last_value = true;
  int64_t s = 0;
  bool b1, b2;  // unused
...
bool InductionVarRange::GenerateCode(HInductionVarAnalysis::InductionInfo* info,
                                     ...
                                     bool is_min) const {
  if (info != nullptr) {
    // Success during codegen when this result is not needed.
