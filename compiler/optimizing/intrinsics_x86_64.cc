...
static void GenUnsafePutAbsolute(LocationSummary* locations,
                                 ...
                                 CodeGeneratorX86_64* codegen) {
  X86_64Assembler* assembler = down_cast<X86_64Assembler*>(codegen->GetAssembler());
  CpuRegister address = locations->InAt(1).AsRegister<CpuRegister>();
  CpuRegister value = locations->InAt(2).AsRegister<CpuRegister>();
  Address address_to_store = Address(address, 0);

  if (type == DataType::Type::kInt64) {
    __ movq(address_to_store, value);
  } else if (kPoisonHeapReferences && type == DataType::Type::kReference) {
    CpuRegister temp = locations->GetTemp(0).AsRegister<CpuRegister>();
    __ movl(temp, value);
    __ PoisonHeapReference(temp);
    __ movl(address_to_store, temp);
  } else if (type == DataType::Type::kInt32 || type == DataType::Type::kReference) {
    __ movl(address_to_store, value);
  } else {
    CHECK_EQ(type, DataType::Type::kInt8) << "Unimplemented GenUnsafePut data type";
    __ movb(address_to_store, value);
  }
