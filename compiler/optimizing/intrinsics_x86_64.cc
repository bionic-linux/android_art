...
class InvokePolymorphicSlowPathX86_64 : public SlowPathCode {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) override {
    ...
    __ movq(CpuRegister(RAX), method_handle_);
...
void IntrinsicCodeGeneratorX86_64::VisitMethodHandleInvokeExact(HInvoke* invoke) {
  ...
  // direct method (that what current `MethodHandles$Lookup.findVirtual` is doing). We don't check
  // whether `method` is an interface method explicitly: in that case subtype check will fail.
