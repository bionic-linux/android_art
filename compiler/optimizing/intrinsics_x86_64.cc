...
static void GenerateVarHandleGetAndAdd(HInvoke* invoke,
  ...
  if (DataType::IsFloatingPointType(type)) {
    ...
    if (is64bit) {
      ...
      __ movd(CpuRegister(RAX), fptemp, /*is64bit=*/ true);
      __ addsd(fptemp, value.AsFpuRegister<XmmRegister>());
      __ movd(temp, fptemp, /*is64bit=*/ true);
      __ LockCmpxchgq(field_addr, temp);
    } else {
      __ movss(fptemp, field_addr);
      __ movd(CpuRegister(RAX), fptemp, /*is64bit=*/ false);
      __ addss(fptemp, value.AsFpuRegister<XmmRegister>());
      __ movd(temp, fptemp, /*is64bit=*/ false);
      __ LockCmpxchgl(field_addr, temp);
    }
    __ j(kNotZero, &retry);

    // The old value is in RAX.
    codegen->Move(locations->Out(), Location::RegisterLocation(RAX));
  } else {
    ...
    switch (type) {
      case DataType::Type::kBool:
      case DataType::Type::kUint8:
        __ LockXaddb(valreg, field_addr);
        __ movzxb(valreg, valreg);
        break;
      case DataType::Type::kInt8:
        __ LockXaddb(valreg, field_addr);
        ...
        break;
      case DataType::Type::kUint16:
        __ LockXaddw(valreg, field_addr);
        __ movzxw(valreg, valreg);
        break;
      case DataType::Type::kInt16:
        __ LockXaddw(valreg, field_addr);
      ...
      case DataType::Type::kInt32:
      case DataType::Type::kUint32:
        __ LockXaddl(valreg, field_addr);
        DCHECK_EQ(valreg, valreg);
        break;
      case DataType::Type::kInt64:
      case DataType::Type::kUint64:
        __ LockXaddq(valreg, field_addr);
        DCHECK_EQ(valreg, valreg);
        break;
      default:
