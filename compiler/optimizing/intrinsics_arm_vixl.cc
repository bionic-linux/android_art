  ...
  switch (type) {
    ...
    case DataType::Type::kInt64:
      if (Use64BitExclusiveLoadStore(atomic, codegen)) {
        UseScratchRegisterScope temps(assembler->GetVIXLAssembler());
        vixl32::Register address_reg;
        if (offset.IsValid()) {
          // If offset is valid then this is a get from a relative address.
          // LDREX/STREX don't support reg+offset addressing.
          // We need to store the base+offset address in a register.
          address_reg = temps.Acquire();
          __ Add(address_reg, base, offset);
        } else {
          // We can reuse the base register.
          address_reg = base;
        }
        ...
        vixl32::Label loop;
        __ Bind(&loop);
        __ Ldrexd(LowRegisterFrom(out), HighRegisterFrom(out), MemOperand(address_reg));
        __ Strexd(strexd_tmp, LowRegisterFrom(out), HighRegisterFrom(out), MemOperand(address_reg));
        __ Cmp(strexd_tmp, 0);
        __ B(ne, &loop);
      } else {
    ...
    case DataType::Type::kReference:
      if (codegen->EmitBakerReadBarrier()) {
        ...
        codegen->GenerateFieldLoadWithBakerReadBarrier(
            invoke, out, base, address, /* needs_null_check= */ false);
      } else {
        __ Ldr(RegisterFrom(out), address);
      }
      break;
    ...
    case DataType::Type::kFloat64: {
      UseScratchRegisterScope temps(assembler->GetVIXLAssembler());
      vixl32::Register address_reg;
      bool use_64bit_ldrd = Use64BitExclusiveLoadStore(atomic, codegen);
      if (use_64bit_ldrd && offset.IsValid()) {
        // If offset is valid then this is a get from a relative address.
        // LDREX/STREX don't support reg+offset addressing.
        // We need to store the base+offset address in a register.
        address_reg = temps.Acquire();
      ...
      if (use_64bit_ldrd) {
        ...
        vixl32::Label loop;
        __ Bind(&loop);
        __ Ldrexd(lo, hi, MemOperand(address_reg));
        __ Strexd(strexd_tmp, lo, hi, MemOperand(address_reg));
        __ Cmp(strexd_tmp, 0);
        __ B(ne, &loop);
