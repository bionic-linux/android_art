...
static void GenerateIntrinsicGet(HInvoke* invoke,
  ...
  ArmVIXLAssembler* assembler = codegen->GetAssembler();
  MemOperand address(NoReg);
  MemOperand absolute_address(NoReg);

  // If offset is valid then this is a get from a relative address, otherwise absolute.
  if (offset.IsValid()) {
