...
static void GenerateCompareAndSet(CodeGeneratorARMVIXL* codegen,
  ...
  //
  // FIXME: How to indicate success/failure.
  // Flag `store_result` indicates whether `old_value == expected || old_value == expected2`.
  ...
  if (success.IsValid() && !success.Is(old_value)) {
  ...
  __ B(ne, cmp_failure, /*is_far_label=*/ cmp_failure_is_far_label);
  ...
  if (strong) {
    ...
    // is essentially free, so prepare the success value here if we can.
    ...
    if (success.IsValid() && success.Is(store_result)) {
      __ Mov(LeaveFlags, success, 1);  // Store succeeded, indicate success.
    }
  } else {
 ...
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) override {
    ...
    GenerateCompareAndSet(arm_codegen,
                          ...
                          /*cmp_failure_is_far_label=*/ !success_.IsValid(),
                          tmp_ptr,
                          new_value_,
...
static void GenUnsafeCas(HInvoke* invoke, DataType::Type type, CodeGeneratorARMVIXL* codegen) {
  ...
  bool cmp_failure_is_far_label = false;
  ...
  // FIXME: Unsafe benchmarks from
  //    https://android-review.googlesource.com/1420959
  // regress if these useless instructions are removed.
  // (blueline little cores with fixed frequency 1420800.)
  if ((true)) {
    ExactAssemblyScope guard(assembler->GetVIXLAssembler(),
                             2 * vixl32::k32BitT32InstructionSizeInBytes);
    __ clz(tmp, tmp);
    __ lsr(EncodingSize(Wide), tmp, tmp, WhichPowerOf2(out.GetSizeInBits()));
  }
...
static void GenerateVarHandleCompareAndSetOrExchange(HInvoke* invoke,
  ...
  // the address shall be constructed in the scratch register before these temporaries are
  // clobbered. We are more careful for references due to read and write barrier, see below. 
  ...
  if (value_type == DataType::Type::kFloat64) {
    ...
    // Use the last temp. For read barriers, this is an extra temporary allocated to avoid
    // overwriting the temporaries for declaring class (if present) and offset. Otherwise,
    // this is the offset temporary which also works for references without read barriers
    // where we still need to preserve the object register for write barrier.
  ...
  DCHECK(acquire_barrier || release_barrier || order == std::memory_order_relaxed);
  ...
  GenerateCompareAndSet(codegen,
                        ...
                        strong,
                        cmp_failure,
                        /*cmp_failure_is_far_label=*/ cmp_failure != &exit_loop_label,
                        tmp_ptr,
                        new_value_reg,
                        new_value_high,
                        old_value,
