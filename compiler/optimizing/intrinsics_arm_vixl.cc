...
static void GenerateIntrinsicGet(HInvoke* invoke,
  ...
  switch (type) {
    ...
    case DataType::Type::kInt64:
      if (atomic && !codegen->GetInstructionSetFeatures().HasAtomicLdrdAndStrd()) {
        UseScratchRegisterScope temps(assembler->GetVIXLAssembler());
        const vixl32::Register temp_reg = temps.Acquire();
...
static void CreateUnsafeGetLocations(HInvoke* invoke, DataType::Type type) {
  bool can_call = kEmitCompilerReadBarrier &&
...
void IntrinsicLocationsBuilderARMVIXL::VisitUnsafeGet(HInvoke* invoke) {
  CreateUnsafeGetLocations(invoke, DataType::Type::kInt32);
}
...
void IntrinsicLocationsBuilderARMVIXL::VisitUnsafeGetLong(HInvoke* invoke) {
  CreateUnsafeGetLocations(invoke, DataType::Type::kInt64);
}
...
void IntrinsicLocationsBuilderARMVIXL::VisitUnsafeGetObject(HInvoke* invoke) {
  CreateUnsafeGetLocations(invoke, DataType::Type::kReference);
}
...
static void GenUnsafePut(HInvoke* invoke,
  ...
  if (type == DataType::Type::kInt64 &&
      atomic &&
      !codegen->GetInstructionSetFeatures().HasAtomicLdrdAndStrd()) {
    maybe_temp = locations->GetTemp(0);
...
static void CreateVarHandleGetLocations(HInvoke* invoke,
  ...
  if (type == DataType::Type::kFloat64 &&
      atomic &&
      !codegen->GetInstructionSetFeatures().HasAtomicLdrdAndStrd()) {
    // We need 2 temporaries for GenerateIntrinsicGet() but we can reuse the
    ...
    locations->AddRegisterTemps(2u - locations->GetTempCount());
  }
}
