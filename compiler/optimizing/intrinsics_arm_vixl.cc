...
static void GenUnsafeGetAndUpdate(HInvoke* invoke,
  ...
  if (non_void && type == DataType::Type::kReference && codegen->EmitReadBarrier()) {
    DCHECK(get_and_update_op == GetAndUpdateOp::kSet);
    if (kUseBakerReadBarrier) {
...
static void GenerateVarHandleGetAndUpdate(HInvoke* invoke,
  ...
  size_t temp_registers_that_mimic_out;
  Location result;
  if (non_void) {
    temp_registers_that_mimic_out = 0u;
    result = locations->Out();
  } else if (value_type == DataType::Type::kFloat32) {
    // Note: Since we allocated a D register, use the low part.
    DCHECK(locations->GetTemp(locations->GetTempCount() - 1u).IsFpuRegisterPair());
    temp_registers_that_mimic_out = 1u;
    result = locations->GetTemp(locations->GetTempCount() - 1u).ToLow();
  } else if (!DataType::IsFloatingPointType(value_type) && DataType::Is64BitType(value_type)) {
    temp_registers_that_mimic_out = 2u;
    result = LocationFrom(RegisterFrom(locations->GetTemp(locations->GetTempCount() - 2u)),
                          RegisterFrom(locations->GetTemp(locations->GetTempCount() - 1u)));
  } else {
    temp_registers_that_mimic_out = 1u;
    result = locations->GetTemp(locations->GetTempCount() - 1u);
  }
  ...
  if (get_and_update_op == GetAndUpdateOp::kSet) {
    ...
    if (value_type == DataType::Type::kFloat64) {
      ...
      // "out" register.
      DCHECK_EQ(locations->GetTempCount(), 5u + temp_registers_that_mimic_out);
      old_value =
          LocationFrom(RegisterFrom(locations->GetTemp(1)), RegisterFrom(locations->GetTemp(2)));
      ...
      // "out" register.
      DCHECK_EQ(locations->GetTempCount(), 3u + temp_registers_that_mimic_out);
      ...
      if (kUseBakerReadBarrier) {
        ...
        store_result = RegisterFrom(result);  // Use the `out` for the exclusive store result.
      } else {
    ...
    if (DataType::IsFloatingPointType(value_type)) {
      maybe_vreg_temp =
          locations->GetTemp(locations->GetTempCount() - 1u - temp_registers_that_mimic_out);
