...
void OptimizingCompiler::GenerateJitDebugInfo(debug::MethodDebugInfo info,
  ...
  if (mini_debug_info && kMaxMethodsPerMiniDebugInfoEntry > 1) {
    ...
    info.custom_name = info.dex_file->PrettyMethod(info.dex_method_index, false);
    ...
    if (recent_method_debug_infos_.size() == kMaxMethodsPerMiniDebugInfoEntry) {
      ...
      for (const debug::MethodDebugInfo& mi : recent_method_debug_infos_) {
        // Transfer the ref-count to the new entry if it is still alive
        // (the code might be have GCed just before we took the mutex).
        if (DecrementJITCodeEntryRefcountFor(mi.code_address)) {
          IncrementJITCodeEntryRefcount(entry, mi.code_address);
          num_live++;
        }
      }
      if (num_live == 0) {
        // Highly unlikely scenario where all recent methods were promptly GCed.
