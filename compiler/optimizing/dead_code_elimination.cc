...
//           B1
//          /  \
//          |  foo()  // always throws and is control flow
//          |   |
//         B2  Exit
bool HDeadCodeElimination::SimplifyAlwaysThrows() {
  ...
  for (HBasicBlock* block : graph_->GetReversePostOrder()) {
    ...
    if (first->AlwaysThrows() &&
      ...
      if (!block->Dominates(pred) && pred->Dominates(succ) && succ->GetPhis().IsEmpty()) {
        block->ReplaceSuccessor(succ, graph_->GetExitBlock());
        block->RemoveInstruction(last);
        DCHECK(first->IsInvoke());
        first->AsInvoke()->SetIsControlFlow(true);   // is now control flow!
