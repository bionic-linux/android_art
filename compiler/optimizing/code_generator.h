...
class CodeGenerator : public DeletableArenaObject<kArenaAllocCodeGenerator> {
 public:
  ...
  bool IsImplementedIntrinsic(HInvoke* invoke) const {
    return invoke->IsIntrinsic() && unimplemented_intrinsics_.find(invoke->GetIntrinsic()) ==
                                        unimplemented_intrinsics_.end();
  }
 ...
 private:
  ...
  // Intrinsics for which we haven't handcraft code for.
  const std::unordered_set<Intrinsics> unimplemented_intrinsics_;
