...
class CompilerOptions final {
 public:
  ...
  static constexpr size_t kDefaultMaximumNumberOfTotalInstructions = 1024;
  ...
  // are not reached.
  static constexpr size_t kDefaultMaximumNumberOfInstructionsForSmallMethod = 3;

  // Limit the number of dex registers that we accumulate while inlining
  // to avoid creating large amount of nested environments.
  static constexpr size_t kDefaultMaximumNumberOfCumulatedDexRegisters = 32;

  // Limit recursive call inlining, which do not benefit from too
  // much inlining compared to code locality.
  static constexpr size_t kDefaultMaximumNumberOfRecursiveCalls = 4;

  // Limit recursive polymorphic call inlining to prevent code bloat, since it can quickly get out
  // of hand in the presence of multiple Wrapper classes. We set this to 0 to disallow polymorphic
  // recursive calls at all.
  static constexpr size_t kDefaultMaximumNumberOfPolymorphicRecursiveCalls = 0;
  ...
  uint32_t InlinerMaximumNumberOfTotalInstructions() const {
    return inliner_maximum_number_of_total_instructions_;
  }

  uint32_t InlinerMaximumNumberOfInstructionsForSmallMethod() const {
    return inliner_maximum_number_of_instructions_for_small_method_;
  }

  uint32_t InlinerMaximumNumberOfCumulatedDexRegisters() const {
    return inliner_maximum_number_of_cumulated_dex_registers_;
  }

  uint32_t InlinerMaximumNumberOfRecursiveCalls() const {
    return inliner_maximum_number_of_recursive_calls_;
  }

  uint32_t InlinerMaximumNumberOfPolymorphicRecursiveCalls() const {
 ...
 private:
  ...
  uint32_t inliner_maximum_number_of_total_instructions_;
  uint32_t inliner_maximum_number_of_instructions_for_small_method_;
  uint32_t inliner_maximum_number_of_cumulated_dex_registers_;
  uint32_t inliner_maximum_number_of_recursive_calls_;
  uint32_t inliner_maximum_number_of_polymorphic_recursive_calls_;
