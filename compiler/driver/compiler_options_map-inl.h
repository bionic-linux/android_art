...
inline bool ReadCompilerOptions(Base& map, CompilerOptions* options, std::string* error_msg) {
  ...
  options->inliner_maximum_number_of_total_instructions_ =
      map.GetOrDefault(Base::InlineMaximumNumberOfTotalInstructions);
  options->inliner_maximum_number_of_instructions_for_small_method_ =
      map.GetOrDefault(Base::InlineMaximumNumberOfInstructionsForSmallMethod);
  options->inliner_maximum_number_of_cumulated_dex_registers_ =
      map.GetOrDefault(Base::InlineMaximumNumberOfCumulatedDexRegisters);
  options->inliner_maximum_number_of_recursive_calls_ =
      map.GetOrDefault(Base::InlineMaximumNumberOfRecursiveCalls);
  options->inliner_maximum_number_of_polymorphic_recursive_calls_ =
      map.GetOrDefault(Base::InlineMaximumNumberOfPolymorphicRecursiveCalls);
...
NO_INLINE void AddCompilerOptionsArgumentParserOptions(Builder& b) {
  ...
  b.
      ...
      .Define("--inline-max-num-cumulated-dex-registers=_")
          .template WithType<size_t>()
      ...
      .Define("--inline-max-num-recursive-calls=_")
          .template WithType<size_t>()
      ...
      .Define("--inline-max-num-polymorphic-recursive-calls=_")
          .template WithType<size_t>()
