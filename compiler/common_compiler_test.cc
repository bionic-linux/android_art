...
void CommonCompilerTest::MakeExecutable(const void* code_start, size_t code_length) {
  ...
  // Remove hwasan tag. This is done in kernel in newer versions. Do it here as well to support older kernels.
  int result = mprotect(HWASanUntag(reinterpret_cast<void*>(base)), len, PROT_READ | PROT_WRITE | PROT_EXEC);
  CHECK_EQ(result, 0);
