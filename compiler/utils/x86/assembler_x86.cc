...
void X86Assembler::SetX86InstructionSetFeatureFlags(bool has_AVX_flag, bool has_AVX2_flag) {
...
void X86Assembler::vmovaps(XmmRegister dst, XmmRegister src) {
  ...
  uint8_t byte_zero = EmitVexPrefixByteZero(true /**is_twobyte_form*/);
  ...
  uint8_t byte_one = EmitVexPrefixByteOne(false, vvvv_reg /**VEX.X*/,
                                          SET_VEX_L_128, SET_VEX_PP_NONE);
  EmitUint8(byte_zero);
  EmitUint8(byte_one);
  /**Instruction Opcode*/
...
void X86Assembler::vmovaps(XmmRegister dst, const Address& src) {
  AssemblerBuffer::EnsureCapacity ensured(&buffer_);
  /**Instruction VEX Prefix*/
  uint8_t ByteZero = EmitVexPrefixByteZero(true /**is_twobyte_form*/);
  X86ManagedRegister vvvv_reg = ManagedRegister::NoRegister().AsX86();
  /**a REX prefix is necessary only if an instruction references one of the
  extended registers or uses a 64-bit operand.*/
  uint8_t ByteOne = EmitVexPrefixByteOne(false, vvvv_reg, SET_VEX_L_128,
                                         SET_VEX_PP_NONE);
  EmitUint8(ByteZero);
  EmitUint8(ByteOne);
  /**Instruction Opcode*/
...
void X86Assembler::andn(Register dst, Register src1, Register src2) {
  ...
  uint8_t byte_zero = EmitVexPrefixByteZero(false /**is_two_byte*/);
  uint8_t byte_one = EmitVexPrefixByteOne(false, false, false, SET_VEX_M_0F_38);
  uint8_t byte_two = EmitVexPrefixByteTwo(false, X86ManagedRegister::FromCpuRegister(src1),
                                          SET_VEX_L_128, SET_VEX_PP_NONE);
  EmitUint8(byte_zero);
  EmitUint8(byte_one);
  EmitUint8(byte_two);
...
uint8_t X86Assembler::EmitVexPrefixByteOne(bool R, bool X, bool B, int SET_VEX_M) {
  /**Vex Byte 1, */
...
uint8_t X86Assembler::EmitVexPrefixByteOne(bool R, X86ManagedRegister operand,
                                           int SET_VEX_L, int SET_VEX_PP) {
  /**Vex Byte 1, */
  uint8_t vex_prefix = VEX_INIT;
  /** Bit[7] This bit needs to be set to '1'
  otherwise the instruction is LES or LDS */
...
uint8_t X86Assembler::EmitVexPrefixByteTwo(bool W, X86ManagedRegister operand,
                                           int SET_VEX_L, int SET_VEX_PP) {
  /** Vex Byte 2, */
  uint8_t vex_prefix = VEX_INIT;
...
uint8_t X86Assembler::EmitVexPrefixByteTwo(bool W, int SET_VEX_L, int SET_VEX_PP) {
  /**Vex Byte 2, */
  uint8_t vex_prefix = VEX_INIT;
