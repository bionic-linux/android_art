...
class AssemblerRISCV64Test : public AssemblerTest<riscv64::Riscv64Assembler,
 ...
 protected:
  ...
  void TestJalRd(const std::string& test_name,
                 size_t gap_size,
...
TEST_F(AssemblerRISCV64Test, JalRd3KiB) {
  TestJalRd("JalRd3KiB", 3 * KB, "1", GetPrintJalRd());
}

TEST_F(AssemblerRISCV64Test, JalRd2MiB) {
  TestJalRd("JalRd2MiB", 2 * MB, "1", GetPrintCallRd("2"));
}
...
TEST_F(AssemblerRISCV64Test, LoadLabelAddress) {
  ...
  Riscv64Label label;
  for (size_t i = 0; i != kNumLoadsForward; ++i) {
    ...
    expected += "1:\n"
                "auipc " + rd_name + ", %pcrel_hi(2f)\n"
                "addi " + rd_name + ", " + rd_name + ", %pcrel_lo(1b)\n";
  }
  __ Bind(&label);
  expected += "2:\n";
  for (size_t i = 0; i != kNumLoadsBackward; ++i) {
    // Cycle through non-Zero registers.
    XRegister rd = enum_cast<XRegister>((i % (kNumberOfXRegisters - 1)) + 1);
    DCHECK_NE(rd, Zero);
    std::string rd_name = GetRegisterName(rd);
