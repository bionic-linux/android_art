...
uint32_t Riscv64Assembler::Branch::PromoteIfNeeded(bool compression_allowed) {
  // All targets should be resolved before we start promoting branches.
  DCHECK(IsResolved());
  Type old_type = type_;
  ...
  switch (type_) {
    ...
    case kCondBranch: {
      ...
      if (compression_allowed && cond21Type == kCondBranch21 && IsCompressableCondition()) {
...
void Riscv64Assembler::PromoteBranches() {
  ...
  bool compression_allowed = IsExtensionEnabled(Riscv64Extension::kZca);
  do {
    changed = false;
    for (auto& branch : branches_) {
      CHECK(branch.IsResolved());
      uint32_t delta = branch.PromoteIfNeeded(compression_allowed);
