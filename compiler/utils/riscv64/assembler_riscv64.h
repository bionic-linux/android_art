...
enum class FPRoundingMode : uint32_t {
  ...
  kDefault = kDYN,
  // Some instructions never round even though the spec includes the RM field.
  // To simplify testing, emit the RM as 0 by default for these instrutions because that's what
...
class Riscv64Assembler final : public Assembler {
 ...
 private:
  ...
  void LoadLiteral(XRegister rd, Literal* literal, Branch::Type literal_type);
