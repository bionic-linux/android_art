...
uint8_t* JitMemoryRegion::AllocateCode(const uint8_t* code,
                                       size_t code_size,
  ...
  // Write the header.
  OatQuickMethodHeader* method_header = new (w_memory) OatQuickMethodHeader(
  ...
  uint8_t* x_memory = region->GetExecutableAddress(w_memory);
