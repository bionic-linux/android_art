...
bool MayHaveBadFlushFaultBehaviour() {
  ...
  //
  // This behaviour means we should avoid the dual view JIT on the device.
...
bool JitMemoryRegion::Initialize(size_t initial_capacity,
  ...
  // If we are the zygote, use all the capacity as we're not going to GC code generated.
  current_capacity_ = is_zygote ? max_capacity : initial_capacity;
  ...
  bool try_dual_view = !MayHaveBadFlushFaultBehaviour();
  if (try_dual_view) {
    if (is_zygote) {
