...
bool JitMemoryRegion::Initialize(size_t initial_capacity,
  ...
  initial_capacity_ = RoundDown(initial_capacity, 2 * page_size_);
  max_capacity_ = RoundDown(max_capacity, 2 * page_size_);
  ...
  CHECK(IsAlignedParam(data_capacity, page_size_));
...
void JitMemoryRegion::SetFootprintLimit(size_t new_footprint) {
  ...
  DCHECK(IsAlignedParam(data_space_footprint, page_size_));
