...
bool ProfileCompilationInfo::AddClassTypeIndex(const std::string& dex_location,
                                               uint32_t checksum,
                                               uint16_t type_idx) {
  DexFileData* const data = GetOrAddDexFileData(dex_location, checksum);
  if (data == nullptr) {
...
bool ProfileCompilationInfo::ProcessLine(SafeBuffer& line_buffer,
  ...
  for (uint16_t i = 0; i < class_set_size; i++) {
    uint16_t class_type_idx = line_buffer.ReadUintAndAdvance<uint16_t>();
    if (!AddClassTypeIndex(dex_location, checksum, class_type_idx)) {
      return false;
    }
  }
  return true;
...
bool ProfileCompilationInfo::ContainsClassType(const DexFile& dex_file,
                                               uint16_t class_type_idx) const {
  ...
  if (info_it != info_.end()) {
    ...
    return classes.find(class_type_idx) != classes.end();
  }
  return false;
}
...
bool ProfileCompilationInfo::GenerateTestProfile(int fd,
  ...
  for (uint16_t i = 0; i < number_of_dex_files; i++) {
    ...
    for (uint16_t c = 0; c < number_of_classes; c++) {
      ...
      info.AddClassTypeIndex(profile_key, 0, class_idx);
    }
  }
  return info.Save(fd);
}
