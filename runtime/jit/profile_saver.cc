...
bool ProfileSaver::ProcessProfilingInfo(bool force_save, /*out*/uint16_t* number_of_new_methods) {
  ...
  for (const auto& it : tracked_locations) {
    ...
    }
    {
      ...
      // obsolete data (e.g. updated dex checksums). If this happens we clear the profile data and
      // for the save to ensure the file is cleared.
      if (!info.AddMethods(profile_methods,
              ProfileCompilationInfo::MethodHotness::kFlagPostStartup)) {
        LOG(WARNING) << "Could not add methods to the existing profiler. Clearing the profile data.";
        info.ClearData();
        force_save = true;
      }
      ...
      if (profile_cache_it != profile_cache_.end()) {
        ProfileCompilationInfo* cached_profile = profile_cache_it->second;
        if (!info.MergeWith(*cached_profile)) {
