...
void ProfileSaver::Run() {
  ...
  // classes save (unless they started before the initial saving was done).
  {
    ...
    const uint64_t end_time = NanoTime() + MsToNs(
      force_early_first_save ? options_.GetMinFirstSaveMs() : options_.GetSaveResolvedClassesDelayMs());
  ...
  while (!ShuttingDown(self)) {
    // Sleep, only if don't have to force an early first save configured
