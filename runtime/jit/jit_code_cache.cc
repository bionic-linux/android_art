...
JitCodeCache* JitCodeCache::Create(size_t initial_capacity,
  ...
  // The map is data, pre-code-spacing, code, post-code-spacing.
  size_t total_spacing_size = kMaxMapSpacingPages * kPageSize;
  size_t data_size = RoundUp((max_capacity - total_spacing_size) / 2, kPageSize);
  size_t pre_code_size =
      GetRandomNumber(kMinMapSpacingPages, kMaxMapSpacingPages) * kPageSize;
  size_t code_size = max_capacity - total_spacing_size - data_size;
  size_t post_code_size = total_spacing_size - pre_code_size;
  DCHECK_EQ(code_size + data_size + total_spacing_size, max_capacity);

  // Create pre-code padding region after data region.
...
// Convert one address withing the source map to the same offset within thedestination map.
