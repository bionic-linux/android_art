...
static void FlushJitCodeCacheRange(uint8_t* code_ptr,
  ...
  // faults should be handled by the kernel. However, Linux kernels
  // before Android O, may surface these permission faults to
  // user-land which cannot deal with them. To work around this, we
  // read a value from each page to fault it in before attempting to
  // perform cache maintenance operations.  lines. This should be
  // removed once this code no longer needs to run on pre-N kernels.
  //
