...
JitCodeCache* JitCodeCache::Create(size_t initial_capacity,
  ...
  if (use_two_mappings) {
    // When two mappings are employed a region of spacing pages to
    // kMaxPmapSpacingPages is placed after the executable code region.
  ...
  // The map is data, code, spacing.
  size_t spacing_size = use_two_mappings
      ? GetRandomNumber(kMinMapSpacingPages, kMaxMapSpacingPages) * kPageSize
      : 0;
  size_t data_size = RoundUp((max_capacity - spacing_size) / 2, kPageSize);
  size_t code_size = max_capacity - spacing_size - data_size;
  DCHECK_EQ(code_size + data_size + spacing_size, max_capacity);

  // Create code region after data region.
  uint8_t* data_code_divider = data_map->Begin() + data_size;
  unique_fd writable_code_fd;
  std::unique_ptr<MemMap>
      code_map(data_map->RemapAtEnd(data_code_divider,
                                    "jit-code-cache",
                                    kProtAll,
                                    &error_str,
                                    use_ashmem,
  ...
  // Sanity check that the beginning of the code_map is right after data_map.
  DCHECK_EQ(code_map->Begin(), data_code_divider);
  std::unique_ptr<MemMap> writable_code_map;
  if (use_two_mappings) {
    // Allocate padding at end of code that we'll clean-up after the
    // writable code is allocated. This is to produce a randomised
    // offset between the executable region and the writable region.
    uint8_t* code_spacing_divider = code_map->Begin() + code_size;
    std::unique_ptr<MemMap> space_map(
        code_map->RemapAtEnd(code_spacing_divider,
                             "jit-code-cache-space",
                             kProtNone,
                             &error_str,
                             use_ashmem,
                             nullptr));
    if (space_map == nullptr) {
      std::ostringstream oss;
...
static bool IsAddressInMap(const void* addr,
  ...
  if (mem_map->Begin() <= addr &&
      addr < mem_map->Begin() + mem_map->Size()) {
    return true;
  }
