...
bool JitCodeCache::Commit(Thread* self,
  ...
  const uint8_t* code_ptr = region->CommitCode(
      reserved_code, header_size, code, stack_map_data, has_should_deoptimize_flag);
  if (code_ptr == nullptr) {
    return false;
  }
  OatQuickMethodHeader* method_header = OatQuickMethodHeader::FromCodePointer(code_ptr);
  ...
  if (!region->CommitData(roots_data, roots, stack_map.data(), stack_map.size())) {
    return false;
  }
...
bool JitCodeCache::Reserve(Thread* self,
  ...
  code_size = header_size + code_size;
