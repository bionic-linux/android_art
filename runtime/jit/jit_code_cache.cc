...
static void FlushJitCodeCacheRange(uint8_t* code_ptr,
                                   ...
                                   size_t code_size) {
  // Cache-maintenance instructions can cause permission faults when
  // the page is not present (e.g. swapped out or not backed). Faults
  // in this case should be handled by the kernel. However, kernels
  // before O, may surface these permission faults to user-land which
  // can not deal with them. To work around this, we read a value from
  // each page to page it-in before attempting to invalidate cache
  // lines. This should be removed once this code no longer needs to run
  // on pre-N kernels.
  //
