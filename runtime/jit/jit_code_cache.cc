...
static void FlushJitCodeCacheRange(uint8_t* code_ptr,
  ...
  // cannot deal with them. To work around this, we read a value from
  // each page to fault it in before attempting to perform cache
  // maintenance operations.  lines. This should be removed once this
  // code no longer needs to run on pre-N kernels.
  //
