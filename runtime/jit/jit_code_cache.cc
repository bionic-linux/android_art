...
JitCodeCache* JitCodeCache::Create(size_t initial_capacity,
  ...
  // Allocate the R/W view.  Mark it as read-only until we need to write to the cache.
...
void JitCodeCache::FreeCode(const void* code_ptr) {
  // TODO(sehr): translate.
  if (code_ptr != nullptr) {
    CHECK_LE(executable_code_map_->Begin(), code_ptr);
    CHECK_LT(code_ptr, executable_code_map_->Begin() + executable_code_map_->Size());
  }
...
uint8_t* JitCodeCache::CommitCodeInternal(Thread* self,
  ...
  LOG(ERROR) << "------------ STARTING\n";
  {
    ...
    LOG(ERROR) << "------------ WAITED FOR GC\n";
    {
      ...
      LOG(ERROR) << "------------ QMH" << reinterpret_cast<void*>(writable_method_header) << "\n";
      ...
      LOG(ERROR) << "------------ FLUSHED IC\n";
      DCHECK(!Runtime::Current()->IsAotCompiler());
      if (has_should_deoptimize_flag) {
        writable_method_header->SetHasShouldDeoptimizeFlag();
      }
      method_header =
          reinterpret_cast<OatQuickMethodHeader*>(ToExecutableAddress(
              reinterpret_cast<uint8_t*>(writable_method_header)));
      LOG(ERROR) << "------------ MH=" << reinterpret_cast<void*>(method_header) << "\n";
    }

    number_of_compilations_++;
    LOG(ERROR) << "------------ FINISHED COMMIT\n";
  }
  // We need to update the entry point in the runnable state for the instrumentation.
  {
    ...
    LOG(ERROR) << "------------ FINISHED UPDATING ENTRYPOINT\n";
  }
...
uint8_t* JitCodeCache::AllocateCode(size_t code_size) {
  ...
  if (result != nullptr) {
    CHECK_LE(writable_code_map_->Begin(), result);
    CHECK_LT(result, writable_code_map_->Begin() + writable_code_map_->Size());
  }
...
void JitCodeCache::FreeCode(uint8_t* code) {
  ...
  if (code != nullptr) {
    CHECK_LE(executable_code_map_->Begin(), code);
    CHECK_LT(code, executable_code_map_->Begin() + executable_code_map_->Size());
  }
  uint8_t* writable_code = ToWritableAddress(code);
  if (writable_code != nullptr) {
    CHECK_LE(writable_code_map_->Begin(), writable_code);
    CHECK_LT(writable_code, writable_code_map_->Begin() + writable_code_map_->Size());
  }
...
uint8_t* JitCodeCache::AllocateData(size_t data_size) {
  ...
  if (result != nullptr) {
    CHECK_LE(data_map_->Begin(), result);
    CHECK_LT(result, data_map_->Begin() + data_map_->Size());
  }
...
void JitCodeCache::FreeData(uint8_t* data) {
  if (data != nullptr) {
    CHECK_LE(data_map_->Begin(), data);
    CHECK_LT(data, data_map_->Begin() + data_map_->Size());
  }
