...
uint8_t* JitCodeCache::CommitCodeInternal(Thread* self,
  ...
  // LOG(ERROR) << "------------ STARTING\n";
  {
    ...
    // LOG(ERROR) << "------------ WAITED FOR GC\n";
    {
      ...
      // code_ptr is an EXECUTABLE address
      // stack_map is a DATA address
      // method_info is a DATA address
      new (writable_method_header) OatQuickMethodHeader(
          ...
          code_size);
      // LOG(ERROR) << "WMH="
      //            << reinterpret_cast<const void*>(writable_method_header)
      //            << " size = "
      //            << writable_method_header->GetCodeSize();
      ...
      FlushInstructionCache(code_ptr, code_size);
      ...
      // LOG(ERROR) << "MH="
      //            << reinterpret_cast<const void*>(method_header)
      //            << " size = "
  ...
  // LOG(ERROR) << "------------ COMMITTED METHOD @"
  //            << reinterpret_cast<const void*>(method_header)
  //            << " "
  //            << reinterpret_cast<const void*>(method_header->GetCode())
  //            << " size = "
  //            << method_header->GetCodeSize();
