...
void ZygoteMap::Initialize(uint32_t number_of_methods) {
  MutexLock mu(Thread::Current(), *Locks::jit_lock_);
  // Allocate for 70% capacity. This will offer OK lookup times, and termination
  // cases.
  size_t capacity = RoundUpToPowerOfTwo(number_of_methods * 100 / 70);
  map_ = ArrayRef(
      reinterpret_cast<Entry*>(
          region_->AllocateData(capacity * sizeof(Entry), /* initialize= */ true)),
      capacity);
}
...
const void* ZygoteMap::GetCodeFor(ArtMethod* method, uintptr_t pc) const {
  ...
  // Note that the zygote may concurrently write to the map. That's OK as the
