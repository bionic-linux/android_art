      ...
      if (object == nullptr || object == Runtime::GetWeakClassSentinel()) {
        ...
        if (new_method_type != nullptr) {
          ...
          if (kIsDebugBuild) {
            ...
            mirror::Class* object_class = object->GetClass<kVerifyNone, kWithoutReadBarrier>();
            mirror::Object* new_object_class = visitor->IsMarked(object_class);

            CHECK_EQ(new_object_class, method_type_class.Ptr());
    ...
    if (GetLiveBitmap()->Test(allocation)) {
      ...
      if (method_it != method_code_map_.end()) {
        ...
        if (code_ptrs_it != method_code_map_reversed_.end()) {
          ...
          code_ptrs.erase(std::find(code_ptrs.begin(), code_ptrs.end(), code_ptr));
