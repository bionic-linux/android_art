...
static uint8_t* GetRootTable(const void* code_ptr, uint32_t* number_of_roots = nullptr) {
  //  DCHECK(IsExecutableAddress(code_ptr));
...
void JitCodeCache::SweepRootTables(IsMarkedVisitor* visitor) {
  ...
  for (const auto& entry : method_code_map_) {
    ...
    DCHECK(IsWritableAddress(entry.first));
...
void JitCodeCache::FreeCode(const void* code_ptr) {
  ...
  LOG(ERROR) << "Deleting code " << code_ptr;
  //  const void* old_code_ptr = code_ptr;
  code_ptr = ToWritableAddress(static_cast<const uint8_t*>(code_ptr));
  uintptr_t allocation = FromCodeToAllocation(code_ptr);
  // Notify native debugger that we are about to remove the code.
  // It does nothing if we are not using native debugger.
  DeleteJITCodeEntryForAddress(reinterpret_cast<uintptr_t>(code_ptr));
  FreeData(GetRootTable(code_ptr));
  FreeCode(ToExecutableAddress(reinterpret_cast<uint8_t*>(allocation)));
}
...
void JitCodeCache::FreeAllMethodHeaders(
  ...
  for (const OatQuickMethodHeader* method_header : method_headers) {
    ...
    // TODO(OTH): issue GetCode() is in the writable region, FreeCode(const void* ) is expecting executable.
    // Calls FreeCode(uint8_t*);
...
uint8_t* JitCodeCache::CommitCodeInternal(Thread* self,
  ...
  // LOG(ERROR) << "------------ STARTING\n";
  {
    ...
    // LOG(ERROR) << "------------ WAITED FOR GC\n";
    {
      ...
      // LOG(ERROR) << "------------ QMH" << reinterpret_cast<void*>(code_ptr - stack_map) << "\n";
      // LOG(ERROR) << "------------ QMH" << reinterpret_cast<void*>(code_ptr - method_info) << "\n";
...
void JitCodeCache::FreeData(uint8_t* data) {
  LOG(ERROR) << "FreeData " << static_cast<void*>(data);
  data +=
      executable_code_map_->Begin() -
      writable_code_map_->Begin();
