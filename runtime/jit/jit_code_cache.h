...
class JitCodeCache {
 ...
 private:
  ...
  // Free in the mspace allocations for `writable_code_ptr`.
  void FreeCodeAndData(void* writable_code_ptr) REQUIRES(lock_);
  ...
  void FreeRawCode(void* writable_code) REQUIRES(lock_);
  ...
  static bool IsAddressType(const void* addr, const std::unique_ptr<MemMap>& mem_map, const char* check_name) {
    if (addr == nullptr) {
      return true;
    }
  ...
  template <typename T>
  bool IsDataAddress(const T* raw_addr) const {
    return IsAddressType(raw_addr, data_map_, "Data");
  }

  template <typename T>
  bool IsExecutableAddress(const T* raw_addr) const {
    return IsAddressType(raw_addr, executable_code_map_, "Executable");
  }

  template <typename T>
  bool IsWritableAddress(const T* raw_addr) const {
    return IsAddressType(raw_addr, writable_code_map_, "Writable");
  }

  template <typename T>
  const T* ToExecutableAddress(T* writable_address) const {
    CHECK(IsWritableAddress(writable_address));
    if (writable_address == nullptr) {
      return nullptr;
    }
    ptrdiff_t offset = reinterpret_cast<const uint8_t*>(writable_address) - writable_code_map_->Begin();
  ...
  template <typename T>
  T* ToWritableAddress(const T* executable_address) const {
    CHECK(IsExecutableAddress(executable_address));
    if (executable_address == nullptr) {
      return nullptr;
    }
    ptrdiff_t offset = reinterpret_cast<const uint8_t*>(executable_address) -
        reinterpret_cast<const uint8_t*>(executable_code_map_->Begin());
    T* writable_address = reinterpret_cast<T*>(writable_code_map_->Begin() + offset);
    CHECK(IsWritableAddress(writable_address));
