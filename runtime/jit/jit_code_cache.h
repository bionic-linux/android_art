...
class JitCodeCache {
 ...
 private:
  ...
  static bool IsAddressType(const void* addr,
  ...
  bool IsDataAddress(const T* raw_addr) const {
    return IsAddressType(raw_addr, data_map_.get(), "Data");
  }

  template <typename T>
  bool IsExecutableAddress(const T* raw_addr) const {
    return IsAddressType(raw_addr, executable_code_map_.get(), "Executable");
  }

  template <typename T>
  bool IsWritableAddress(const T* raw_addr) const {
    return IsAddressType(raw_addr, GetWritableMemMap(), "Writable");
  }
  ...
  T* ToExecutableAddress(T* writable_address) const {
    ...
    ptrdiff_t offset =
        reinterpret_cast<const uint8_t*>(writable_address) - writable_map->Begin();
  ...
  T* ToWritableAddress(const T* executable_address) const {
    ...
    MemMap* writable_map = GetWritableMemMap();
    T* writable_address = reinterpret_cast<T*>(writable_map->Begin() + offset);
    CHECK(IsWritableAddress(writable_address));
