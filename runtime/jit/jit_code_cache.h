...
class JitCodeCache {
 ...
 private:
  ...
  bool IsExecutableAddress(const void* addr) const {
    ...
    LOG(ERROR) << "IsExecutable = " << addr
               << " not in [" << reinterpret_cast<void*>(executable_code_map_->Begin())
               << ", " << reinterpret_cast<void*>(executable_code_map_->Begin() + executable_code_map_->Size()) << ")";
    return false;
  }
  bool IsWritableAddress(const void* addr) const {
    ...
    LOG(ERROR) << "IsWritable = " << addr
               << " not in [" << reinterpret_cast<void*>(writable_code_map_->Begin())
               << ", " << reinterpret_cast<void*>(writable_code_map_->Begin() + writable_code_map_->Size()) << ")";
    return false;
  }
  bool IsDataAddress(const void* addr) const {
    ...
    LOG(ERROR) << "IsData = " << addr
               << " not in [" << reinterpret_cast<void*>(data_map_->Begin())
               << ", " << reinterpret_cast<void*>(data_map_->Begin() + data_map_->Size()) << ")\n";
    return false;
  }
