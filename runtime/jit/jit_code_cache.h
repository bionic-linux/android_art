...
class JitCodeCache {
 public:
  ...
  LIBART_PROTECTED void NotifyMethodRedefined(ArtMethod* method)
  ...
  LIBART_PROTECTED bool ContainsPc(const void* pc) const;
  ...
  LIBART_PROTECTED bool ContainsMethod(ArtMethod* method)
  ...
  LIBART_PROTECTED void GarbageCollectCache(Thread* self)
      REQUIRES(!Locks::jit_lock_)
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  LIBART_PROTECTED OatQuickMethodHeader* LookupOsrMethodHeader(ArtMethod* method)
      REQUIRES(!Locks::jit_lock_)
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  LIBART_PROTECTED bool RemoveMethod(ArtMethod* method, bool release_memory)
      REQUIRES(!Locks::jit_lock_)
      REQUIRES(Locks::mutator_lock_);
  ...
  // baseline compiled.
  LIBART_PROTECTED void GetProfiledMethods(const std::set<std::string>& dex_base_locations,
                                 std::vector<ProfileMethodInfo>& methods,
                                 uint16_t inline_cache_threshold) REQUIRES(!Locks::jit_lock_)
      REQUIRES_SHARED(Locks::mutator_lock_);

  LIBART_PROTECTED void InvalidateAllCompiledCode()
      REQUIRES(!Locks::jit_lock_)
      REQUIRES_SHARED(Locks::mutator_lock_);
  ...
  LIBART_PROTECTED void MoveObsoleteMethod(ArtMethod* old_method, ArtMethod* new_method)
      REQUIRES(!Locks::jit_lock_) REQUIRES(Locks::mutator_lock_);

  // Dynamically change whether we want to garbage collect code.
  LIBART_PROTECTED void SetGarbageCollectCode(bool value) REQUIRES(!Locks::jit_lock_);
  ...
  // doesn't come back.
  LIBART_PROTECTED void TransitionToDebuggable() REQUIRES(!Locks::jit_lock_) REQUIRES(Locks::mutator_lock_);
