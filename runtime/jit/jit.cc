...
void Jit::NotifyZygoteCompilationDone() {
  ...
  // In order to have the zygote and children share the memory, we also repmap
...
void Jit::CreateThreadPool() {
  ...
  if (runtime->IsZygote() && runtime->IsUsingApexBootImageLocation() && UseJitCompilation()) {
    ...
    if (total_capacity > 0) {
      ...
      if (IsSealFutureWriteSupported()) {
        // Seal now.
        if (fcntl(mem_fd, F_ADD_SEALS, F_SEAL_SHRINK | F_SEAL_GROW | F_SEAL_SEAL | F_SEAL_FUTURE_WRITE)
                == -1) {
          PLOG(WARNING) << "Failed to seal boot image methods file descriptor";
          zygote_mapping_methods_ = MemMap();
          return;
