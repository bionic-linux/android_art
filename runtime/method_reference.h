...
// A method is uniquely located by its IDexFile and the method_ids_ table index into that IDexFile
class MethodReference : public DexFileReference {
 public:
  MethodReference(const IDexFile* file, uint32_t index) : DexFileReference(file, index) {}
  std::string PrettyMethod(bool with_signature = true) const {
    return dex_file->PrettyMethod(index, with_signature);
  }
  const IDexFile::MethodId& GetMethodId() const {
...
struct MethodReferenceValueComparator {
  ...
  bool SlowCompare(MethodReference mr1, MethodReference mr2) const {
    ...
    const IDexFile::MethodId& mid1 = mr1.GetMethodId();
    const IDexFile::MethodId& mid2 = mr2.GetMethodId();
    ...
    const IDexFile::ProtoId& prid1 = mr1.dex_file->GetProtoId(mid1.proto_idx_);
    const IDexFile::ProtoId& prid2 = mr2.dex_file->GetProtoId(mid2.proto_idx_);
    ...
    const IDexFile::TypeList* params1 = mr1.dex_file->GetProtoParameters(prid1);
    size_t param1_size = (params1 != nullptr) ? params1->Size() : 0u;
    const IDexFile::TypeList* params2 = mr2.dex_file->GetProtoParameters(prid2);
