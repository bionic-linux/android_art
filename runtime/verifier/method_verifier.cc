...
class MethodVerifier final : public ::art::verifier::MethodVerifier {
  ...
  */
  ...
  NO_INLINE void FailForRegisterType(uint32_t vsrc,
                                     ...
                                     const RegType& src_type)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    FailForRegisterType(vsrc, reg_types_.GetFromRegKind(check_kind), src_type);
  }
  ...
  ALWAYS_INLINE inline bool VerifyRegisterType(uint32_t vsrc, RegType::Kind check_kind)
    ...
    // Verify the src register type against the check type refining the type of the register
    const RegType& src_type = work_line_->GetRegisterType(this, vsrc);
    RegType::Assignability assignability =
        RegType::AssignabilityFrom(check_kind, src_type.GetKind());
    if (assignability != RegType::Assignability::kAssignable) {
      // Integer or float assignability is never a `kNarrowingConversion` or `kReference`.
      DCHECK_EQ(assignability, RegType::Assignability::kNotAssignable);
      FailForRegisterType(vsrc, check_kind, src_type);
      return false;
    }
    DCHECK(!src_type.IsLowHalf());
    return true;
  }
    ...
    if (VerifyRegisterType(vregB, src_kind1) &&
      ...
      if (check_boolean_op) {
        ...
        if (work_line_->GetRegisterType(this, vregB).IsBooleanTypes() &&
          ...
          work_line_->SetRegisterType(inst->VRegA_23x(), RegType::Kind::kBoolean);
          return;
        }
      }
...
bool MethodVerifier<kVerifierDebug>::CodeFlowVerifyInstruction(uint32_t* start_guess) {
  ...
  switch (inst->Opcode()) {
    ...
    case Instruction::CMPG_FLOAT:
      if (!VerifyRegisterType(inst->VRegB_23x(), kFloat)) {
        break;
      }
      if (!VerifyRegisterType(inst->VRegC_23x(), kFloat)) {
        break;
      }
    ...
    case Instruction::CMPG_DOUBLE:
      if (!VerifyRegisterTypeWide(inst->VRegB_23x(), kDoubleLo)) {
        break;
      }
      if (!VerifyRegisterTypeWide(inst->VRegC_23x(), kDoubleLo)) {
        break;
      }
      work_line_->SetRegisterType<LockOp::kClear>(inst->VRegA_23x(), reg_types_.Integer());
      break;
    case Instruction::CMP_LONG:
      if (!VerifyRegisterTypeWide(inst->VRegB_23x(), kLongLo)) {
        break;
      }
      if (!VerifyRegisterTypeWide(inst->VRegC_23x(), kLongLo)) {
        break;
      }
      work_line_->SetRegisterType<LockOp::kClear>(inst->VRegA_23x(), reg_types_.Integer());
      break;
    ...
    case Instruction::USHR_INT:
      CheckBinaryOp(inst, kInteger, kInteger, kInteger, false);
      break;
    case Instruction::AND_INT:
    case Instruction::OR_INT:
    case Instruction::XOR_INT:
      CheckBinaryOp(inst, kInteger, kInteger, kInteger, true);
      break;
    ...
    case Instruction::REM_FLOAT:
      CheckBinaryOp(inst, kFloat, kFloat, kFloat, false);
      break;
