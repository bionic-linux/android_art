...
class MethodVerifier final : public ::art::verifier::MethodVerifier {
  ...
  */
  ...
  bool CanAccess(const RegType& other) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    const RegType& declaring_class_type = GetDeclaringClass();
    if (declaring_class_type.Equals(other)) {
    ...
    } else if (declaring_class_type.IsUnresolvedReference()) {
      // Be conservative, only allow if `other` is public.
      return other.IsJavaLangObject() || (other.IsReference() && other.GetClass()->IsPublic());
    } else {
      return GetRegTypeClass(declaring_class_type)->CanAccess(GetRegTypeClass(other));
    }
  }
  ...
  bool CanAccessMember(ObjPtr<mirror::Class> klass, uint32_t access_flags)
      REQUIRES_SHARED(Locks::mutator_lock_) {
    const RegType& declaring_class_type = GetDeclaringClass();
    if (declaring_class_type.IsUnresolvedReference()) {
      return false;  // More complicated test not possible on unresolved types, be conservative.
    }
    DCHECK(declaring_class_type.IsJavaLangObject() || declaring_class_type.IsReference());
    return GetRegTypeClass(declaring_class_type)->CanAccessMember(klass, access_flags);
  }
...
ArtMethod* MethodVerifier<kVerifierDebug>::ResolveMethodAndCheckAccess(
  ...
  ObjPtr<mirror::Class> klass = klass_type.IsJavaLangObject()
      ? GetClassRoot<mirror::Object>(class_linker)
      : klass_type.GetClass();
