...
class MethodVerifier final : public ::art::verifier::MethodVerifier {
 ...
 private:
  ...
  // Helper function for `VisitInstructions()`.
  template <uint32_t opcode>
  static constexpr bool IsOpcodeOrNextOpcodeNopReturnOrCheckCast() {
    auto is_nop_return_or_check_cast = [](Instruction::Code kOpcode) constexpr {
      return kOpcode == Instruction::NOP ||
             Instruction::IsReturn(kOpcode) ||
             kOpcode == Instruction::CHECK_CAST;
    };
    if (is_nop_return_or_check_cast(enum_cast<Instruction::Code>(opcode))) {
      return true;
    }
    if constexpr (opcode == std::numeric_limits<uint8_t>::max()) {
      return false;
    } else {
      return is_nop_return_or_check_cast(enum_cast<Instruction::Code>(opcode + 1));
    }
  }
    ...
    auto find_dispatch_opcode = [](Instruction::Code opcode) constexpr {
      ...
      for (uint32_t raw_other = 0; raw_other != opcode; ++raw_other) {
        ...
        if (Instruction::FormatOf(opcode) == Instruction::FormatOf(other) &&
          ...
          DCHECK_EQ(Instruction::SizeInCodeUnits(Instruction::FormatOf(opcode)),
                    Instruction::SizeInCodeUnits(Instruction::FormatOf(other)));
          return other;
        }
      }
      return opcode;
