...
class RegTypeWrapper {
 public:
  RegTypeWrapper(const RegType& reg_type) : reg_type_(reg_type) {}
 private:
...
TEST_F(RegisterLineTest, NewInstanceDexPcsMerging) {
  ...
  StackHandleScope<3u> hs(soa.Self());
  ...
  ArtMethod* method = object_class->FindClassMethod("notify", "()V", kRuntimePointerSize);
