...
class MergeImpl : public RegType {
 public:
  constexpr MergeImpl(RegType::Kind kind)
...
constexpr RegType::Kind MergeImpl::MergeKind(RegType::Kind incoming_kind) const {
  const MergeImpl incoming_type(incoming_kind);
  ...
  static constexpr std::array<std::array<RegType::Kind, kNumKinds>, kNumKinds> kMergeTable =
    ...
    for (size_t lhs = 0u; lhs != kNumKinds; ++lhs) {
      for (size_t rhs = 0u; rhs != kNumKinds; ++rhs) {
        result[lhs][rhs] =
            MergeImpl(enum_cast<RegType::Kind>(lhs)).MergeKind(enum_cast<RegType::Kind>(rhs));
      }
    }
    return result;
  }();
