...
const RegType& RegType::Merge(const RegType& incoming_type,
  ...
  if (this == &undefined || &incoming_type == &undefined) {
    ...
    // Enumerator values are ordered by increasing range within the non-negative and
    // can-be-negative groups, so the type with the higher kind can be used within
    // these groups and merging across the groups is also quite simple.
