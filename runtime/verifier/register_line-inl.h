...
inline void RegisterLine::DCheckUniqueNewInstanceDexPc(MethodVerifier* verifier, uint32_t dex_pc) {
  if (kIsDebugBuild && allocation_dex_pcs_ != nullptr) {
    ...
    RegTypeCache* cache = verifier->GetRegTypeCache();
    for (uint32_t i = 0; i != num_regs_; ++i) {
      if (NeedsAllocationDexPc(cache->GetFromId(line_[i]))) {
        CHECK_NE(allocation_dex_pcs_[i], dex_pc) << i << " " << GetRegisterType(verifier, i);
      }
    }
  }
}
