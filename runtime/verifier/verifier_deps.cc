...
uint32_t VerifierDeps::GetFieldDeclaringClassStringId(const DexFile& dex_file, ArtField* field) {
  static_assert(kAccJavaFlagsMask == 0xFFFF, "Unexpected value of a constant");
  // We could add the same optimization as GetMethodDeclaringClassStringId but it seems like it
  // would not be necessary since the field case rarely reaches GetStringId.
  if (field == nullptr) {
    return VerifierDeps::kUnresolvedMarker;
  }
