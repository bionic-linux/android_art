...
std::vector<std::unique_ptr<const DexFile>> OatFileManager::OpenDexFilesFromOat(
  ...
  // We perform the check only if the class loaded context match failed or did
  ...
  if (context != nullptr
    ...
    if (!already_exists_in_classpath.empty()) {
      ...
      // Here we are trying to give a heads up on why the app might be have performance issues.
      LOG(ERROR) << out.str();
    }
  }
  return dex_files;
