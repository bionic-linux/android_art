...
inline mirror::Class* ArtMethod::GetDexCacheResolvedType(dex::TypeIndex type_index,
  ...
  if (kWithCheck) {
    ...
    if (UNLIKELY(type_index.index >= dex_cache->NumResolvedTypes())) {
      ThrowArrayIndexOutOfBoundsException(type_index.index, dex_cache->NumResolvedTypes());
      return nullptr;
    }
  }
  mirror::Class* klass = GetDexCacheResolvedTypes(pointer_size)[type_index.index].Read();
