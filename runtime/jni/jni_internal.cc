...
ALWAYS_INLINE static bool ShouldDenyAccessToMember(T* member, Thread* self)
  ...
  if (Runtime::Current()->GetCorePlatformApiEnforcementPolicy() != hiddenapi::EnforcementPolicy::kDisabled) {
    // The IsCalledWhitelisted() check is only intended for Core Platform API and is expensive due
    // to stack walking.
    if (hiddenapi::ScopedJniStackMarker::IsCallerWhitelisted()) {
      return false;
    }
  }
...
class JNI {
 public:
  ...
  static jmethodID GetMethodID(JNIEnv* env, jclass java_class, const char* name, const char* sig) {
    ...
    ScopedObjectAccess soa(env);
    hiddenapi::ScopedJniStackMarker sjsm;
