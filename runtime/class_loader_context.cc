...
static bool CollectDexFilesFromJavaDexFile(ObjPtr<mirror::Object> java_dex_file,
  ...
  for (int32_t j = 1; j < long_array_size; ++j) {
    ...
    LOG(ERROR) << "DEX FILE IS " << cp_dex_file;
    LOG(ERROR) << "DEX FILE IS " << (void*)&cp_dex_file->GetHeader();
    LOG(ERROR) << "DEX FILE IS " << cp_dex_file->GetLocation();
...
bool ClassLoaderContext::CreateInfoFromClassLoader(
  ...
  if (!CreateInfoFromClassLoader(
          soa, parent, null_dex_elements, child_info, /*is_shared_library=*/ false)) {
    return false;
  }
  return true;
}
