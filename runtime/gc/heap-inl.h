...
inline mirror::Object* Heap::AllocObjectWithAllocator(Thread* self,
  ...
  uint32_t starting_gc_num;  // o.w. GC number at which we observed need for GC.
  {
    ...
    if (kCheckLargeObject && UNLIKELY(byte_count >= large_object_threshold_)) {
      ...
      // Only primitive objects can go into the LOS, since the LOS is not within the card
      // table range. This also means that we rely on SetClass not dirtying the object's card.
      if (klass->IsPrimitiveArray() || klass->IsStringClass()) {
