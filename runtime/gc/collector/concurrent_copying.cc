  ...
  auto visitor = [&](mirror::Object* obj)
    ...
    if ((!IsFullGC() && *card == gc::accounting::CardTable::kCardClean) ||
      ...
      if (IsFullGC()) {
        ...
        // Don't expect such cards to be remembered, so fake it here
 ...
 private:
  void CheckReference(mirror::Object* ref) const
    ...
    if (!collector_->TestAndSetMarkBitForRef(ref)) {
      // Don't expect to see any tenured objects here for mid_term GC.
...
void ConcurrentCopying::AddLiveBytesAndScanRef(mirror::Object* ref) {
  ...
  // we don't consider any references from them into TenuredGen of region space
  RegionGen obj_gen = RegionGen::kRegionGenTenured;
 ...
 public:
  ...
  void operator()(ObjPtr<mirror::Object> obj) const REQUIRES(Locks::heap_bitmap_lock_)
    ...
    if (visitor.ContainsInterGenRefs()) {
      ...
      // We don't care about inter-region refs here, only inter-gen refs are interesting to us
      ...
      card_table->Scan<false>(
          ...
          [this, space](mirror::Object* obj)
            ...
            if (IsYoungGC()) {
              ...
              // Don't bother checking intergen refs as the dirty cards are anyway remembered
              if (IsMidTermGC()) {
  ...
  switch (rtype) {
    ...
    case space::RegionSpace::RegionType::kRegionTypeUnevacFromSpace:
      ...
      if (!kUseBakerReadBarrier || !region_space_bitmap_->Set(to_ref)) {
        // For young gen or mid term we don't expect un-marked UnevacFromSpaceTenured object.
