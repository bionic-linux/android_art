...
class MarkCompact : public GarbageCollector {
 ...
 private:
  ...
  // spaces.
  void ReclaimPhase() REQUIRES_SHARED(Locks::mutator_lock_) REQURES(!Locks::heap_bitmap_lock_);

  // Mark GC-roots during a STW pause.
  void ReMarkRoots(Runtime* runtime) REQUIRES(Locks::mutator_lock_, Locks::heap_bitmap_lock_);
  // Concurrently mark GC-roots the first time.
  void MarkRoots() REQUIRES_SHARED(Locks::mutator_lock_) REQUIRES(Locks::heap_bitmap_lock_);
  ...
  void MarkReachableObjects() REQUIRES_SHARED(Locks::mutator_lock_)
      REQUIRES(Lock::heap_bitmap_lock_);
  // Scan mod-union and card tables to identify objects with 'minimum age', which
  // is kCardAged in case of concurrent marking, and kCardDirty during the STW pause.
