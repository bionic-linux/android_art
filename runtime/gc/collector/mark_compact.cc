...
static constexpr size_t kConcurrentThreadCount = 2;
...
MarkCompact::MarkCompact(Heap* heap)
  ...
  // stack can't be used. We also use the first page-sized buffer for the
  // purpose of terminating concurrent compaction.
  const size_t num_pages = 1 + std::max(kConcurrentThreadCount,
                                        std::max(heap_->GetParallelGCThreadCount(),
                                                 heap_->GetConcGCThreadCount()));
  compaction_buffers_map_.reset(
          new MemMap(MemMap::MapAnonymous("Concurrent mark-compact compaction buffers",
...
class MarkCompact::ConcurrentCompactionGcTask : public SelfDeletingTask {
 public:
  explicit ConcurrentCompactionGcTask(MarkCompact* collector, size_t idx) : collector_(collector) {
    if (kObjPtrPoisoning) {
      page_ = collector->compaction_buffers_map_->Begin() + idx * kPageSize;
    }
  }
...
#  pragma clang diagnostic ignored "-Wframe-larger-than="
  void Run(Thread* self ATTRIBUTE_UNUSED) override REQUIRES_SHARED(Locks::mutator_lock_) {
    if (kObjPtrPoisoning) {
      collector_->ConcurrentCompaction(page_);
    } else {
...
void MarkCompact::PrepareForCompaction() {
  ...
  // volatile load is required to ensure that the compiler doesn't elide the
  // subsequent loads to uffd_ below.
  if (UNLIKELY(*static_cast<volatile int*>(&uffd_) == -1)) {
    Runtime* runtime = Runtime::Current();
    // Zygote does this in every GC cycle, and also doesn't require SIGSYS
    // signal handler for fallback. So avoid that.
    const bool check_fallback = !runtime->GetNoSigChain();
    auto sigsys_handler = [](int, siginfo_t* info, void*) -> bool {
                            if (info->si_syscall == __NR_userfaultfd) {
                              Runtime::Current()->GetHeap()->MarkCompactCollector()->SetFallback();
                              // Return true so that the ligsigchain skips the
                              // other handlers, which could be added to SIGSYS
                              return true;
                            }
                            return false;
                          };
    ...
    if (check_fallback) {
      art::SigchainAction action = {
          .sc_sigaction = sigsys_handler,
          .sc_mask = {},
          .sc_flags = 0,
      };
      art::AddSpecialSignalHandlerFn(SIGSYS, &action);
      art::EnsureFrontOfChain(SIGSYS);
    }
    int ret = syscall(__NR_userfaultfd, O_CLOEXEC | UFFD_USER_MODE_ONLY);
    // SetFallback() sets uffd_ to kFallbackMode if SIGSYS gets triggered
    // due to userfaultfd syscall.
    if (check_fallback) {
      art::RemoveSpecialSignalHandlerFn(SIGSYS, sigsys_handler);
      // Signal fence required to ensure that uffd_ load below doesn't happen
      // before the signal handler, which could store kFallbackMode to it.
      std::atomic_signal_fence(std::memory_order_acquire);
      if (UNLIKELY(*static_cast<volatile int*>(&uffd_) == kFallbackMode)) {
        LOG(WARNING) << "Userfaultfd is blocked by seccomp and therefore "
                        "falling back to stop-the-world compaction";
        return;
      }
    }
    ...
    if (UNLIKELY(ret == -1 && errno == EINVAL)) {
      ret = syscall(__NR_userfaultfd, O_CLOEXEC);
    }
    if (UNLIKELY(ret == -1)) {
      uffd_ = kFallbackMode;
  ...
  if (UNLIKELY(pool == nullptr)) {
    heap_->CreateThreadPool(kConcurrentThreadCount);
  ...
  while (true) {
    ...
    if (fault_page < post_compact_end_) {
      ...
      if (first_obj != nullptr) {
        ...
        SlideBlackPage(first_obj,
                       pre_compact_offset_moving_space_[page_idx],
                       first_objs_moving_space_[page_idx + 1].AsMirrorPtr(),
                       pre_compact_page,
                       page);
        copy_ioctl(fault_page, page);
      } else {
...
void MarkCompact::CompactionPhase() {
  ...
  // The following triggers 'special' userfaults, which, when received by the
