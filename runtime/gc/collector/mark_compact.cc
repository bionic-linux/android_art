...
size_t MarkCompact::InitializeInfoMap(uint8_t* p) {
  size_t moving_space_sz = bump_pointer_space_->Capacity();
...
MarkCompact::MarkCompact(Heap* heap)
  ...
  if (UNLIKELY(!info_map_.IsValid())) {
    ...
    size_t total = InitializeInfoMap(info_map_.Begin());
...
void MarkCompact::ClampGrowthLimit(size_t new_capacity) {
  ...
  if (new_capacity < old_capacity) {
    ...
    if (thread_running_gc_ == nullptr) {
      ...
      // away. This is the most likely case. 'thread_running_gc_' may become
      // non-null under us if a new GC starts. However, it is safe to clamp
      // 'info_map_' as all the changes will get committed to memory before
      // GC-thread starts using them in MarkingPhase().
      MaybeClampInfoMap();
    }
  }
}

void MarkCompact::MaybeClampInfoMap() {
  if (UNLIKELY(clamp_info_map_status_ == ClampInfoStatus::kClampInfoPending)) {
    size_t set_size = InitializeInfoMap(info_map_.Begin());
