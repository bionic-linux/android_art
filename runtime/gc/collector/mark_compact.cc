...
// A region which is more than kBlackDenseRegionThreshold precent live doesn't
...
bool MarkCompact::PrepareForCompaction() {
  ...
  if (gc_cause != kGcCauseExplicit && gc_cause != kGcCauseCollectorTransition &&
    ...
    if (threshold_passing_marker > 0) {
      live_bytes = 0;
      total_bytes = 0;
      for (auto iter = pages_live_bytes.rbegin() + (num_pages - threshold_passing_marker);
           iter != pages_live_bytes.rend();
           iter++) {
        live_bytes += *iter;
        total_bytes += gPageSize;
        if (live_bytes * 100U >= total_bytes * kBlackDenseRegionThreshold) {
          threshold_passing_marker = pages_live_bytes.rend() - iter;
          break;
        }
      }
    }
    black_dense_idx = threshold_passing_marker * chunk_info_per_page;
    ...
    DCHECK_ALIGNED_PARAM(black_dense_end_, gPageSize);
    {
      ...
      uint8_t* check_range_end = black_allocations_begin_;
      while (black_dense_end_ > moving_space_begin_) {
        uint8_t* max_black_dense_addr = black_dense_end_;
        bool ran_loop = false;
        // Check for all classes (whose objects are preceding them on moving
        // space) in [black_dense_end_, check_range_end).
        for (auto iter = class_after_obj_map_.lower_bound(
                 ObjReference::FromMirrorPtr(reinterpret_cast<mirror::Object*>(black_dense_end_)));
             iter != class_after_obj_map_.end() &&
             reinterpret_cast<uint8_t*>(iter->first.AsMirrorPtr()) < check_range_end;
             iter++, ran_loop = true) {
          max_black_dense_addr = std::min(max_black_dense_addr,
                                          reinterpret_cast<uint8_t*>(iter->second.AsMirrorPtr()));
        }
        if (!ran_loop) {
          // Didn't find any class in [black_dense_end_, check_range_end), so our search is done.
          break;
        }
        // Ensure that next iteration only checks the classes which are in
        // unchecked range starting from updated black_dense_end_ in
        // class_after_obj_map_.
        check_range_end = black_dense_end_;
        black_dense_end_ = AlignDown(max_black_dense_addr, gPageSize);
      }
    ...
    // TODO: Do we need to set chunk_info_vec for black dense entries with
    // kOffsetChunkSize and possibly also set all bits in live_words_bitmap_?
