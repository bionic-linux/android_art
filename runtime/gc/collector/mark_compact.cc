...
MarkCompact::MarkCompact(Heap* heap)
  ...
  const size_t num_pages = 1 + std::max(kConcurrentThreadCount, std::max(heap_->GetParallelGCThreadCount(),
                                                                         heap_->GetConcGCThreadCount()));
  compaction_buffers_map_.reset(new MemMap(MemMap::MapAnonymous("Concurrent mark-compact compaction buffers",
...
void MarkCompact::PrepareForCompaction() {
  ...
  if (uffd_ == -1) {
    ...
    if (uffd_ == -1 && errno == EINVAL) {
      uffd_ = syscall(__NR_userfaultfd, O_CLOEXEC);
    }
    CHECK(uffd_ >= 0) << "Userfaultfd: " << strerror(errno);
...
void MarkCompact::CompactMovingSpace() {
  ...
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
  uint8_t buf[kPageSize];
  uint8_t* page = kObjPtrPoisoning ? compaction_buffers_map_->Begin() : buf;
  ...
  while (idx < moving_first_objs_count_) {
    if (moving_pages_status_[idx].exchange(PageState::kCompacting, std::memory_order_relaxed)
...
std::atomic<size_t> conc_threads_started;
