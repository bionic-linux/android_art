...
void MarkCompact::PrepareForCompaction() {
  ...
  if (uffd_ == -1) {
    ...
    const bool check_fallback = !runtime->IsZygote() && !runtime->GetNoSigChain();
    ...
    // We don't need to cover the following syscall, which is just to cover the
    // case where the host kernel may not have security patches.
    if (check_fallback) {
      art::RemoveSpecialSignalHandlerFn(SIGSYS, sigsys_handler);
    }
    if (uffd_ == kFallbackMode) {
      LOG(WARNING) << "Userfaultfd isn't supported and therefore "
    ...
    CHECK(ret >= 0) << "Userfaultfd: " << strerror(errno);
    uffd_ = ret;
...
void MarkCompact::CompactionPhase() {
  ...
  ThreadPool* pool = heap_->GetThreadPool();
  pool->StartWorkers(thread_running_gc_);
  if (kObjPtrPoisoning) {
  ...
  pool->StopWorkers(thread_running_gc_);
}
#pragma clang diagnostic pop
...
void MarkCompact::FinishPhase() {
  ...
  if (UNLIKELY(Runtime::Current()->IsZygote())) {
