...
void MarkCompact::RunPhases() {
  ...
  if (perform_compaction) {
    ...
    post_compact_end_ = mid_gen_end_ = black_allocations_begin_;
  }
  FinishPhase();
...
class MarkCompact::RefsUpdateVisitor {
 public:
  ...
  bool ShouldDirtyCard() { return dirty_card_; }
...
class MarkCompact::RefFieldsVisitor {
 public:
  ALWAYS_INLINE explicit RefFieldsVisitor(MarkCompact* const mark_compact, mirror::Object* obj)
      : mark_compact_(mark_compact),
        young_gen_begin_(mark_compact->mid_gen_end_),
        young_gen_end_(mark_compact->moving_space_end_),
        dirty_card_(false) {
    if (mark_compact->use_generational_) {
      check_refs_to_young_gen_ = reinterpret_cast<uint8_t*>(obj) < young_gen_begin_ ||
                                 reinterpret_cast<uint8_t*>(obj) >= young_gen_end_;
    } else {
      check_refs_to_young_gen_ = false;
    }
  }
  ...
  if (use_generational_) {
    {
      ...
      while (iter != class_after_obj_map_.end()) {
        ...
        if (reinterpret_cast<uint8_t*>(compacted_obj) >= mid_gen_end_) {
          CHECK_GE(reinterpret_cast<uint8_t*>(compacted_klass), mid_gen_end_)
              << " old_gen_end:" << reinterpret_cast<void*>(old_gen_end_)
              << " post_compact_end:" << reinterpret_cast<void*>(post_compact_end_)
              << " black_allocations_begin:" << reinterpret_cast<void*>(black_allocations_begin_)
              << " compacted_obj:" << compacted_obj;
