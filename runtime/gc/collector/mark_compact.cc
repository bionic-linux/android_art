...
bool MarkCompact::MapUpdatedLinearAllocPages(uint8_t* start_page,
  ...
  while (start_page < end_page) {
    ...
    if (map_len == 0) {
      ...
      for (PageState s = state->load(std::memory_order_relaxed);
           length > 0 && s != PageState::kProcessed;
           s = state->load(std::memory_order_relaxed)) {
        // If any page is being processed (only possible for a non-gc thread)
        // then it may require accessing some preceeding page containing the
        // beginning of first-object of the page. Therefore, we cannot madvise
        // the shadow pages.
        // NOTE: We rely on the fact that range passed to this function contain
        // entire arenas. And when mutator calls this function, it doesn't ask
        // to madvise.
        if (s > PageState::kUnprocessed && s < PageState::kProcessedAndMapped) {
          free_pages = false;
        }
        state++;
        length -= gPageSize;
