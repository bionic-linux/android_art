...
void Heap::SetIdealFootprint(size_t target_footprint) {
  ...
  ATraceIntegerValue(TRACE_TARGET_PEAK_HEAP_SIZE, target_footprint / KB);
}
...
bool TryIncreaseTargetFootprint(size_t target) {
  while (true) {
    ...
    if (target_footprint_.compare_exchange_weak(orig_target_footprint,
                                              target,
                                              std::memory_order_relaxed))
      break;
    }
  ...
  ATraceIntegerValue(TRACE_TARGET_PEAK_HEAP_SIZE, target / KB);
  return true;
}
...
void Heap::ClearGrowthLimit() {
  if (target_footprint_.load(std::memory_order_relaxed) == growth_limit_
    ...
    ATraceIntegerValue(TRACE_TARGET_PEAK_HEAP_SIZE, capacity_ / KB);
    ...
    if (heap->GetCurrentGcNum() == initial_gc_num_
      ...
      if (target_footprint > new_target_sz_) {
        if (heap->target_footprint_.CompareAndSetStrongRelaxed(target_footprint, new_target_sz_)) {
          ...
          ATraceIntegerValue(TRACE_TARGET_PEAK_HEAP_SIZE, new_target_sz_ / KB);
        }
      }
    }
  }
