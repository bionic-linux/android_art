...
void Heap::PreZygoteFork() {
  ...
  if (kCompactZygote) {
    ...
    // For an AOSP boot, we saw num_nonmovable == 25.
...
bool Heap::IsNonMovable(ObjPtr<mirror::Object> obj) const {
  ...
  if (space != nullptr) {
    if (space == GetNonMovingSpace()) {
      return true;
    }
    if (space == zygote_space_) {
      return non_movable_zygote_objects_.contains(
          mirror::CompressedReference<mirror::Object>::FromMirrorPtr(obj.Ptr()).AsVRegValue());
    }
  }
...
bool Heap::PossiblyAllocatedMovable(ObjPtr<mirror::Object> obj) const {
  ...
  if (space != nullptr) {
    ...
    if (!gUseReadBarrier && space == GetNonMovingSpace()) {
      return false;
    }
  }
  return true;
