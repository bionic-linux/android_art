...
inline void RegionSpace::ZeroAllocRange(uint8_t* start, size_t length) {
  ...
  DCHECK_LE(start, end);
  uint8_t* page = AlignUp(start, kPageSize);
  uint8_t* const end_page = AlignDown(end, kPageSize);
  DCHECK_LE(end_page, end);
  DCHECK_GE(page, start);
  if (page >= end_page) {
    ...
    do {
      if (*page != 0) {
        DCHECK(*page == kMadvFreeMagic);
        if (zero_range_begin == nullptr) {
          zero_range_begin = page;
        }
      } else if (zero_range_begin != nullptr) {
        DCHECK_GE(page, zero_range_begin);
        std::fill(zero_range_begin, page, 0);
        zero_range_begin = nullptr;
      }
      page += kPageSize;
    } while (page < end_page);
    if (zero_range_begin != nullptr) {
      DCHECK_GT(page, zero_range_begin);
      DCHECK_EQ(page, end_page);
      std::fill(zero_range_begin, end, 0);
    } else {
      std::fill(end_page, end, 0);
    }
  }
}
