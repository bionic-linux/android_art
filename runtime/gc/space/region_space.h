...
class RegionSpace final : public ContinuousMemMapAllocSpace {
 public:
  ...
  LIBART_PROTECTED size_t AllocationSizeNonvirtual(mirror::Object* obj, size_t* usable_size)
      REQUIRES_SHARED(Locks::mutator_lock_) REQUIRES(!region_lock_);
  ...
  LIBART_PROTECTED void Clear() override REQUIRES(!region_lock_);
  ...
  // reference.
  LIBART_PROTECTED void Unprotect();
  ...
  LIBART_PROTECTED void Dump(std::ostream& os) const override;
  ...
  LIBART_PROTECTED void DumpNonFreeRegions(std::ostream& os) REQUIRES(!region_lock_);

  LIBART_PROTECTED size_t RevokeThreadLocalBuffers(Thread* thread) override REQUIRES(!region_lock_);
  size_t RevokeThreadLocalBuffers(Thread* thread, const bool reuse) REQUIRES(!region_lock_);
  LIBART_PROTECTED size_t RevokeAllThreadLocalBuffers() override
  ...
  LIBART_PROTECTED bool LogFragmentationAllocFailure(std::ostream& os, size_t failed_alloc_bytes) override
      REQUIRES_SHARED(Locks::mutator_lock_) REQUIRES(!region_lock_);
 ...
 private:
  ...
  class Region {
   public:
    ...
    LIBART_PROTECTED void UnfreeLarge(RegionSpace* region_space, uint32_t alloc_time)
        REQUIRES(region_space->region_lock_);

    // Given a free region, declare it non-free (allocated) and large tail.
    LIBART_PROTECTED void UnfreeLargeTail(RegionSpace* region_space, uint32_t alloc_time)
        REQUIRES(region_space->region_lock_);
  ...
  LIBART_PROTECTED Region* AllocateRegion(bool for_evac) REQUIRES(region_lock_);
