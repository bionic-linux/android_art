 ...
 private:
  ...
  bool LoadComponents(const BootImageLayout::ImageChunk& chunk,
    ...
    if (chunk.art_fd.get() >= 0) {
      ...
      image_fd.reset(std::move(chunk.art_fd));
    } else {
      size_t pos = chunk.start_index;
      int arg_image_fd = pos < boot_class_path_image_fds_.size() ? boot_class_path_image_fds_[pos] : -1;
      if (arg_image_fd >= 0) {
        image_fd.reset(arg_image_fd);
      }
    }

    for (size_t i = 0u, size = locations.size(); i != size; ++i) {
      android::base::unique_fd dupped_image_fd;
      if (image_fd.get() >= 0) {
        dupped_image_fd.reset(DupCloexec(image_fd));
      }
      spaces->push_back(Load(locations[i],
                             filenames[i],
                             chunk.profile_files,
                             std::move(dupped_image_fd),
                             logger,
                             image_reservation,
                             error_msg));
