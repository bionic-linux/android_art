 ...
 private:
  ...
  bool LoadComponents(const BootImageLayout::ImageChunk& chunk,
    ...
    for (size_t i = 0u, size = locations.size(); i != size; ++i) {
      ...
      int vdex_fd = pos < boot_class_path_vdex_fds_.size() ? boot_class_path_vdex_fds_[pos] : -1;
      int oat_fd = pos < boot_class_path_oat_fds_.size() ? boot_class_path_oat_fds_[pos] : -1;
