    ...
    if (fixup_image) {
      ...
      FixupObjectVisitor fixup_object_visitor(visited_bitmap.get(), boot_image, app_image, app_oat);
      {
        ...
        if (class_table_section.Size() > 0u) {
          // Note that we require that ReadFromMemory does not make an internal copy of the
          // elements. This also relies on visit roots not doing any verification which could fail
          // after we update the roots to be the image addresses.
          ...
          for (ClassTable::TableSlot& slot : temp_set) {
            ...
            if (iftable != nullptr && fixup_object_visitor.IsInAppImage(vtable)) {
              // Avoid processing the fields of iftable since we will rpocess them later anyways
              // below.
