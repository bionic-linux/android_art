 ...
 private:
  ...
  static bool RelocateInPlace(ImageHeader& image_header,
    ...
    if (fixup_image) {
                                              ...
                                              app_oat);
      {
        ...
        if (class_table_section.Size() > 0u) {
          ...
          for (ClassTable::TableSlot& slot : temp_set) {
            ...
            CHECK(!visited_bitmap->Set(klass)) << "App image class already visited";
            ...
            if (iftable != nullptr && fixup_object_visitor.IsInAppImage(vtable)) {
              ...
              for (int32_t i = 0; i != ifcount; ++i) {
                ...
                if (unpatched_ifarray != nullptr &&
                    fixup_object_visitor.IsInAppImage(unpatched_ifarray)) {
      ...
      ScopedObjectAccess soa(Thread::Current());
      logger.NewTiming("Fixup objects");
      ...
      for (int32_t i = 0, count = dex_caches->GetLength(); i < count; ++i) {
        ...
        if (methods != nullptr) {
          ...
          for (size_t j = 0, num = dex_cache->NumResolvedMethods(); j != num; ++j) {
            auto pair = mirror::DexCache::GetNativePairPtrSize(new_methods, j, kPointerSize);
            ArtMethod* orig = pair.object;
            ArtMethod* copy = fixup_adapter.ForwardObject(orig);
            if (orig != copy) {
              pair.object = copy;
              mirror::DexCache::SetNativePairPtrSize(new_methods, j, pair, kPointerSize);
            }
          }
        }
        ...
        if (fields != nullptr) {
          ...
          for (size_t j = 0, num = dex_cache->NumResolvedFields(); j != num; ++j) {
            mirror::FieldDexCachePair orig =
                mirror::DexCache::GetNativePairPtrSize(new_fields, j, kPointerSize);
            mirror::FieldDexCachePair copy(fixup_adapter.ForwardObject(orig.object), orig.index);
            if (orig.object != copy.object) {
              mirror::DexCache::SetNativePairPtrSize(new_fields, j, copy, kPointerSize);
            }
          }
        }
