  ...
  bool LoadComponents(const BootImageLayout::ImageChunk& chunk,
    ...
    for (size_t i = 0u, size = locations.size(); i != size; ++i) {
      ...
      if (chunk.vdex_fd.get() >= 0) {
        if (arg_vdex_fd >= 0) {
          LOG(ERROR) << "Unexpected to receive vdex FDs from multiple sources";
          return false;
        }
        vdex_fd.reset(dup(chunk.vdex_fd.get()));
      } else if (arg_vdex_fd >= 0) {
        vdex_fd.reset(dup(arg_vdex_fd));
      }
      if (chunk.oat_fd.get() >= 0) {
        if (arg_oat_fd >= 0) {
          LOG(ERROR) << "Unexpected to receive oat FDs from multiple sources";
          return false;
        }
        oat_fd.reset(dup(chunk.oat_fd.get()));
      } else if (arg_oat_fd >= 0) {
        oat_fd.reset(dup(arg_oat_fd));
      }
