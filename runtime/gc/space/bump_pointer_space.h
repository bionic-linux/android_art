...
class BumpPointerSpace final : public ContinuousMemMapAllocSpace {
 public:
  ...
  // clamps to whatever possible. Returns the new capacity.
  // 'lock_' is used to ensure that TLAB allocations, which are the only ones
  // which may be the only happening concurrently with this function are
  // synchronized. The other Alloc* functions are used in single-threaded mode
  // or doesn't have clamping implemented for them.
