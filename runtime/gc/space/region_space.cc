...
inline bool RegionSpace::Region::ShouldBeEvacuated(EvacMode evac_mode) {
  ...
  if (is_newly_allocated_) {
    ...
    if (IsAllocated()) {
      // We always evacute newly-allocated non-large regions as we
      // believe they contain many dead object (a very simple form of
      // the generational hypothesis).
      //
      // TODO: Verify that assertion by outputting statistics on the
      ...
      // later be removed, as its live object will be copied to an
      // evacuation region, which won't be marked as "newly
      // allocated" (see RegionSpace::AllocateRegion).
      result = true;
    } else {
      DCHECK(IsLarge());
      // We never want to evacuate a large region (and the
      // associated tail regions), except if:
      // - we are forced to (see the `kEvacModeForceAll` case above); or
      // - we are sure the (sole) object contained in this region is
      //   dead (see the logic below in the
      ...
      // in a to-space one between its allocation and now), so we
      // prefer not to evacuate it.
      result = false;
    }
