...
RegionSpace::RegionSpace(const std::string& name, MemMap* mem_map)
    : ContinuousMemMapAllocSpace(name, mem_map, mem_map->Begin(), mem_map->End(), mem_map->End(),
      ...
      num_regions_(mem_map->Size() / kRegionSize), num_non_free_regions_(0),
      num_evac_regions_(0), max_peak_num_non_free_regions_(0),
      non_free_region_index_limit_(0), current_region_(&full_region_),
