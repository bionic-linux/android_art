...
RosAlloc::RosAlloc(void* base, size_t capacity, size_t max_capacity,
  ...
  DCHECK_ALIGNED_PARAM(base, page_size);
  DCHECK_EQ(RoundUp(capacity, page_size), capacity);
  DCHECK_EQ(RoundUp(max_capacity, page_size), max_capacity);
  CHECK_LE(capacity, max_capacity);
  CHECK_ALIGNED_PARAM(page_release_size_threshold_, page_size);
  ...
  size_t num_of_pages = footprint_ / page_size;
  size_t max_num_of_pages = max_capacity_ / page_size;
  std::string error_msg;
  page_map_mem_map_ = MemMap::MapAnonymous("rosalloc page map",
                                           RoundUp(max_num_of_pages, page_size),
  ...
  DCHECK_EQ(capacity_ % page_size, static_cast<size_t>(0));
...
void* RosAlloc::AllocPages(Thread* self, size_t num_pages, uint8_t page_map_type) {
  ...
  const size_t page_size = GetPageSize();
  const size_t req_byte_size = num_pages * page_size;
  ...
  for (auto it = free_page_runs_.begin(); it != free_page_runs_.end(); ) {
    ...
    DCHECK_EQ(fpr_byte_size % page_size, static_cast<size_t>(0));
    if (req_byte_size <= fpr_byte_size) {
      ...
      if (req_byte_size < fpr_byte_size) {
        ...
        DCHECK_EQ(remainder->ByteSize(this) % page_size, static_cast<size_t>(0));
        ...
        DCHECK_EQ(fpr->ByteSize(this) % page_size, static_cast<size_t>(0));
      }
      res = fpr;
      break;
    } else {
  ...
  if (UNLIKELY(res == nullptr && capacity_ > footprint_)) {
    ...
    if (capacity_ - footprint_ + last_free_page_run_size >= req_byte_size) {
      ...
      DCHECK_EQ(increment % page_size, static_cast<size_t>(0));
      size_t new_footprint = footprint_ + increment;
      size_t new_num_of_pages = new_footprint / page_size;
      ...
      if (last_free_page_run_size > 0) {
        ...
        DCHECK_EQ(last_free_page_run->ByteSize(this) % page_size, static_cast<size_t>(0));
        ...
        DCHECK_EQ(new_free_page_run->ByteSize(this) % page_size, static_cast<size_t>(0));
  ...
  if (LIKELY(res != nullptr)) {
    ...
    if (kTraceRosAlloc) {
      LOG(INFO) << "RosAlloc::AllocPages() : 0x" << std::hex << reinterpret_cast<intptr_t>(res)
                << "-0x" << (reinterpret_cast<intptr_t>(res) + num_pages * page_size)
                << "(" << std::dec << (num_pages * page_size) << ")";
    }
    return res;
  }
...
size_t RosAlloc::FreePages(Thread* self, void* ptr, bool already_zero) {
  ...
  const size_t page_size = GetPageSize();
  const size_t byte_size = num_pages * page_size;
  if (already_zero) {
  ...
  DCHECK_ALIGNED_PARAM(fpr->ByteSize(this), page_size);
  ...
  if (!free_page_runs_.empty()) {
    ...
    for (auto it = free_page_runs_.upper_bound(fpr); it != free_page_runs_.end(); ) {
      FreePageRun* h = *it;
      DCHECK_EQ(h->ByteSize(this) % page_size, static_cast<size_t>(0));
      if (kTraceRosAlloc) {
    ...
    for (auto it = free_page_runs_.upper_bound(fpr); it != free_page_runs_.begin(); ) {
      ...
      FreePageRun* l = *it;
      DCHECK_EQ(l->ByteSize(this) % page_size, static_cast<size_t>(0));
      if (kTraceRosAlloc) {
...
void* RosAlloc::AllocLargeObject(Thread* self, size_t size, size_t* bytes_allocated,
  ...
  const size_t page_size = GetPageSize();
  size_t num_pages = RoundUp(size, page_size) / page_size;
  void* r;
  {
  ...
  *bytes_tl_bulk_allocated = total_bytes;
  if (kTraceRosAlloc) {
    LOG(INFO) << "RosAlloc::AllocLargeObject() : 0x" << std::hex << reinterpret_cast<intptr_t>(r)
              << "-0x" << (reinterpret_cast<intptr_t>(r) + num_pages * page_size)
              << "(" << std::dec << (num_pages * page_size) << ")";
  }
  ...
  Run* run = nullptr;
  {
    ...
    switch (page_map_[pm_idx]) {
      ...
      case kPageMapRunPart: {
        ...
        do {
          --pm_idx;
          DCHECK_LT(pm_idx, capacity_ / GetPageSize());
        } while (page_map_[pm_idx] != kPageMapRun);
        FALLTHROUGH_INTENDED;
      case kPageMapRun:
        run = reinterpret_cast<Run*>(base_ + pm_idx * GetPageSize());
  ...
  for (size_t i = 0; i < num_ptrs; i++) {
    ...
    if (kReadPageMapEntryWithoutLockInBulkFree) {
      ...
      if (LIKELY(page_map_entry == kPageMapRun)) {
        run = reinterpret_cast<Run*>(base_ + pm_idx * page_size);
      } else if (LIKELY(page_map_entry == kPageMapRunPart)) {
        size_t pi = pm_idx;
        // Find the beginning of the run.
        do {
          --pi;
          DCHECK_LT(pi, capacity_ / page_size);
        } while (page_map_[pi] != kPageMapRun);
        run = reinterpret_cast<Run*>(base_ + pi * page_size);
  ...
  for (size_t i = 0; i < end; ++i) {
    ...
    switch (pm) {
      case kPageMapReleased:
        // Fall-through.
      case kPageMapEmpty: {
        FreePageRun* fpr = reinterpret_cast<FreePageRun*>(base_ + i * page_size);
        if (free_page_runs_.find(fpr) != free_page_runs_.end()) {
          ...
          DCHECK_EQ(curr_fpr_size % page_size, static_cast<size_t>(0));
          remaining_curr_fpr_size = curr_fpr_size - page_size;
          ...
          DCHECK_EQ(remaining_curr_fpr_size % page_size, static_cast<size_t>(0));
          DCHECK_GE(remaining_curr_fpr_size, static_cast<size_t>(page_size));
          remaining_curr_fpr_size -= page_size;
      ...
      case kPageMapRun: {
        DCHECK_EQ(remaining_curr_fpr_size, static_cast<size_t>(0));
        num_running_empty_pages = 0;
        Run* run = reinterpret_cast<Run*>(base_ + i * page_size);
        size_t idx = run->size_bracket_idx_;
        stream << "[" << i << "]=Run (start)"
               << " idx=" << idx
...
size_t RosAlloc::UsableSize(const void* ptr) {
  ...
  switch (page_map_[pm_idx]) {
    ...
    case kPageMapRunPart: {
      ...
      while (page_map_[pm_idx] != kPageMapRun) {
        pm_idx--;
        DCHECK_LT(pm_idx, capacity_ / page_size);
      }
      DCHECK_EQ(page_map_[pm_idx], kPageMapRun);
      Run* run = reinterpret_cast<Run*>(base_ + pm_idx * page_size);
      DCHECK_EQ(run->magic_num_, kMagicNum);
      size_t idx = run->size_bracket_idx_;
...
bool RosAlloc::Trim() {
  ...
  DCHECK_EQ(footprint_ % page_size, static_cast<size_t>(0));
  auto it = free_page_runs_.rbegin();
  if (it != free_page_runs_.rend() && (last_free_page_run = *it)->End(this) == base_ + footprint_) {
    ...
    DCHECK_EQ(new_footprint % page_size, static_cast<size_t>(0));
    size_t new_num_of_pages = new_footprint / page_size;
    ...
    uint8_t* madvise_begin = AlignUp(zero_begin, page_size);
    DCHECK_LE(madvise_begin, page_map_mem_map_.End());
    size_t madvise_size = page_map_mem_map_.End() - madvise_begin;
    if (madvise_size > 0) {
      DCHECK_ALIGNED_PARAM(madvise_begin, page_size);
      DCHECK_EQ(RoundUp(madvise_size, page_size), madvise_size);
      if (!kMadviseZeroes) {
  ...
  while (i < pm_end) {
    ...
    switch (pm) {
      ...
      case kPageMapLargeObject: {
        ...
        void* start = base_ + i * page_size;
        void* end = base_ + (i + num_pages) * page_size;
        size_t used_bytes = num_pages * page_size;
...
void RosAlloc::SetFootprintLimit(size_t new_capacity) {
  MutexLock mu(Thread::Current(), lock_);
  DCHECK_EQ(RoundUp(new_capacity, GetPageSize()), new_capacity);
...
void RosAlloc::Initialize() {
  ...
  for (size_t i = 0; i < kNumOfSizeBrackets; i++) {
    ...
    size_t run_size = page_size * numOfPages[i];
    ...
    while (i < pm_end) {
      ...
      switch (pm) {
        ...
        case kPageMapEmpty: {
          ...
          size_t fpr_size = fpr->ByteSize(this);
          CHECK_ALIGNED_PARAM(fpr_size, page_size)
        ...
        case kPageMapLargeObject: {
          ...
          uint8_t* start = base_ + i * page_size;
          if (is_running_on_memory_tool_) {
          ...
          CHECK_EQ(num_pages, RoundUp(obj_size + memory_tool_modifier, page_size) / page_size)
              << "A rosalloc large object size " << obj_size + memory_tool_modifier
              << " does not match the page map table " << (num_pages * page_size)
              << std::endl << DumpPageMap();
          i += num_pages;
          CHECK_LE(i, pm_end) << "Page map index " << i << " out of range < " << pm_end
                              << std::endl << DumpPageMap();
      ...
      case kPageMapEmpty: {
        ...
        if (IsFreePage(i)) {
          ...
          if (free_page_runs_.find(fpr) != free_page_runs_.end()) {
            ...
            size_t pages = fpr_size / page_size;
...
size_t RosAlloc::ReleasePageRange(uint8_t* start, uint8_t* end) {
  ...
  const size_t max_idx = pm_idx + (end - start) / page_size;
...
bool RosAlloc::LogFragmentationAllocFailure(std::ostream& os, size_t failed_alloc_bytes) {
  ...
  if (failed_alloc_bytes > kLargeSizeThreshold) {
    // Large allocation.
    required_bytes = RoundUp(failed_alloc_bytes, page_size);
  } else {
    // Non-large allocation.
    required_bytes = numOfPages[SizeToIndex(failed_alloc_bytes)] * page_size;
...
void RosAlloc::DumpStats(std::ostream& os) {
  ...
  for (size_t i = 0; i < kNumOfSizeBrackets; ++i) {
    os << "Bracket " << i << " (" << bracketSizes[i] << "):"
       ...
       << " (" << PrettySize(num_pages_runs[i] * page_size) << ")"
  ...
  os << "Large #allocations=" << num_large_objects
     ...
     << " (" << PrettySize(num_pages_large_objects * page_size) << ")\n";
  ...
  total_allocated_bytes += num_pages_large_objects * page_size;
  os << "Total #total_bytes=" << PrettySize(total_num_pages * page_size)
