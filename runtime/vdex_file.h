...
class VdexFile {
 public:
  ...
  struct VdexFileHeader {
   public:
    VdexFileHeader(bool has_dex_section);
  ...
  size_t GetComputedFileSize() const {
    size_t size = sizeof(VdexFileHeader);
    const VdexFileHeader& header = GetVdexFileHeader();
    size += header.GetNumberOfSections() * sizeof(VdexSectionHeader);
    for (uint32_t i = 0; i < header.GetNumberOfSections(); ++i) {
      size += GetSectionHeaderAt(i).section_size;
    }
    return size;
  }
  ...
  const uint8_t* GetVerifierDepsStart() const {
    return Begin() + GetSectionHeader(VdexSection::kVerifierDepsSection).section_offset;
  }
  ...
  uint32_t GetLocationChecksum(uint32_t dex_file_index) const {
    ...
    return GetDexChecksumAtOffset(dex_file_index);
  }
