...
static inline bool DoCallCommon(ArtMethod* called_method,
  ...
  // Number of registers for the callee's call frame. For the interpreter, we reserve enough space
  // for all the registers in the callee. If the method has a code item, we use its registers size.
  // Otherwise, the callee has no extra registers and we need only reserve space for the inputs.
  // For the compiler, we also reserve only enough space for the inputs, since the compiled code
  // does not use the extra space. This also avoids touching the code item which causes extra reads
  // from the dex file.
  uint16_t num_regs;
