...
class ShadowFrame {
 private:
  ...
  enum class FrameFlags : uint32_t {
    ...
    // Used to specify if ExceptionHandledEvent has to be reported. When enabled
    // these events are reported when we reach the catch block after an exceptio
    // was thrown. These events have to be reported after the DexPCMoveEvent if
    // enabled.
