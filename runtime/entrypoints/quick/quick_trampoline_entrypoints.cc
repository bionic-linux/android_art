...
constexpr size_t kCookieSize = sizeof(uint32_t);

static inline HandleScope* GetGenericJniHandleScope(ArtMethod** managed_sp,
                                                    size_t num_handle_scope_references) {
  // The HandleScope is just below the cookie and padding to align as uintptr_t.
  const size_t offset =
      RoundUp(HandleScope::SizeOf(num_handle_scope_references) + kCookieSize, sizeof(uintptr_t));
  ...
  uintptr_t* ComputeLayout(Thread* self,
    ...
    *handle_scope =
    ...
    DCHECK_GE(static_cast<size_t>(reinterpret_cast<uint8_t*>(managed_sp) - sp8),
              HandleScope::SizeOf(num_handle_scope_references_) + kCookieSize);
