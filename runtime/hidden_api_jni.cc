...
struct CorePlatformApiCookie {
  explicit CorePlatformApiCookie(uint32_t value) {
    u32 = value;
  }
...
ScopedCorePlatformApiCheck::ScopedCorePlatformApiCheck() {
  ...
  // Assume Core Platform API usage is approved if the build is not for a target device.
  bool is_core_platform_api_approved = !kIsTargetBuild;
  ...
  if (cookie.depth == 0u && kIsTargetBuild) {
    ...
    if (policy != hiddenapi::EnforcementPolicy::kDisabled) {
      ...
      if (caller_pc != nullptr) {
        ...
        is_core_platform_api_approved = (kind == SharedObjectKind::kRuntime) || (kind == SharedObjectKind::kApexModule);
      }
    }
  }
...
bool ScopedCorePlatformApiCheck::IsCurrentCallerApproved() {
  return true;
}
