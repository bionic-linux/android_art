...
std::ostream& operator<<(std::ostream& os, SharedObjectKind kind) {
  switch (kind) {
    case SharedObjectKind::kNativeCorePlatform:
      os << "Native Core Platform";
      break;
...
class CodeRangeCache final {
 ...
 private:
  ...
  static SharedObjectKind GetKind(const char* so_name) {
    if (art::LocationIsOnArtModule(so_name) && art::EndsWith(so_name, kLibnativehelper)) {
      return SharedObjectKind::kNativeCorePlatform;
    } else {
      return SharedObjectKind::kOther;
    }
  }
  ...
  if (kIsTargetBuild || is_under_test) {
    ...
    if (policy == hiddenapi::EnforcementPolicy::kDisabled) {
      ...
      if (caller_pc != nullptr) {
        ...
        is_core_platform_api_approved = (kind == SharedObjectKind::kNativeCorePlatform);
      }
    }
  }
