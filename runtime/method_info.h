...
// Method info is for non necessarily dedupe friendly data for a method. Currently it only holds
// methods indices. Putting this data in MethodInfo instead of code infos saves ~5% oat size.
class MethodInfo {
 ...
 public:
  ...
  // Writing mode
  explicit MethodInfo(uint8_t* ptr, size_t num_method_indices)
      : num_method_indices_(num_method_indices) {
    DCHECK(ptr != nullptr);
  ...
  MethodIndexType GetMethodIndex(size_t index) const {
