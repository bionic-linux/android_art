...
void Transaction::VisitObjectLogs(RootVisitor* visitor, ArenaStack* arena_stack) {
  ...
  for (const ObjectPair& pair : moving_roots) {
    ...
    auto node = object_logs_.extract(old_root);
    CHECK(!node.empty());
    node.key() = new_root;
    bool inserted = object_logs_.insert(std::move(node)).inserted;
    CHECK(inserted);
  }
}
...
void Transaction::VisitArrayLogs(RootVisitor* visitor, ArenaStack* arena_stack) {
  ...
  for (const ArrayPair& pair : moving_roots) {
    ...
    auto node = array_logs_.extract(old_root);
    CHECK(!node.empty());
    node.key() = new_root;
    bool inserted = array_logs_.insert(std::move(node)).inserted;
    CHECK(inserted);
  }
}
