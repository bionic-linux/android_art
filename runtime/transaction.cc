...
Transaction::Transaction()
    : log_lock_("transaction log lock", kTransactionLogLock), aborted_(false), app_image_(false) {
...
void Transaction::AbortNoLock(const std::string &abort_message) {
  // We may abort more than once if the exception thrown at the time of the
  // previous abort has been caught during execution of a class initializer.
  // We just keep the message of the first abort because it will cause the
  // transaction to be rolled back anyway.
...
void
Transaction::ThrowAbortError(Thread *self, const std::string *abort_message) {
...
// This function should be called after logging, otherwise the transaction can't
void Transaction::WriteConstrainCheck(mirror::Object *obj,
                                      const MemberOffset& field_offset ATTRIBUTE_UNUSED) {
    ScopedObjectAccess soa(Thread::Current());
    if (app_image_ && object_created_.find(obj) == object_created_.end()) {
      AbortNoLock("Can't modify " + obj->PrettyTypeOf() + " since this "
          "object is not created within this transaction");
      // The actual constrain is: cannot modify object that doesn't belong to
      // the object owning this transaction.
      return;
    }
  }
...
void Transaction::RecordWriteFieldBoolean(mirror::Object *obj,
  ...
  WriteConstrainCheck(obj, field_offset);
}
...
void Transaction::RecordWriteArray(mirror::Array *array,
                                   size_t index,
                                   uint64_t value) {
  ...
  WriteConstrainCheck(array, MemberOffset(index));
}
...
void Transaction::RecordStrongStringInsertion(ObjPtr<mirror::String> s) {
  InternStringLog
      log(s, InternStringLog::kStrongString, InternStringLog::kInsert);
  LogInternedString(std::move(log));
}

void Transaction::RecordWeakStringInsertion(ObjPtr<mirror::String> s) {
  InternStringLog
      log(s, InternStringLog::kWeakString, InternStringLog::kInsert);
  LogInternedString(std::move(log));
}

void Transaction::RecordStrongStringRemoval(ObjPtr<mirror::String> s) {
  InternStringLog
      log(s, InternStringLog::kStrongString, InternStringLog::kRemove);
  LogInternedString(std::move(log));
}

void Transaction::RecordWeakStringRemoval(ObjPtr<mirror::String> s) {
  InternStringLog
      log(s, InternStringLog::kWeakString, InternStringLog::kRemove);
  LogInternedString(std::move(log));
}
...
void Transaction::VisitObjectLogs(RootVisitor *visitor) {
  ...
  for (const ObjectPair &pair : moving_roots) {
    ...
    // Keep the track of objects created after transaction updated.
    auto it = object_created_.find(old_root);
    if (it != object_created_.end()) {
      object_created_.erase(it);
      object_created_.insert(new_root);
    }
...
void Transaction::VisitArrayLogs(RootVisitor *visitor) {
  ...
  for (auto &it : array_logs_) {
    ...
    visitor->VisitRoot(reinterpret_cast<mirror::Object **>(&new_root),
...
void Transaction::ObjectLog::LogBooleanValue(MemberOffset offset,
                                             uint8_t value,
                                             bool is_volatile) {
  LogValue(ObjectLog::kBoolean, offset, value, is_volatile);
}

void Transaction::ObjectLog::LogByteValue(MemberOffset offset,
                                          int8_t value,
                                          bool is_volatile) {
  LogValue(ObjectLog::kByte, offset, value, is_volatile);
}

void Transaction::ObjectLog::LogCharValue(MemberOffset offset,
                                          uint16_t value,
                                          bool is_volatile) {
  LogValue(ObjectLog::kChar, offset, value, is_volatile);
}

void Transaction::ObjectLog::LogShortValue(MemberOffset offset,
                                           int16_t value,
                                           bool is_volatile) {
  LogValue(ObjectLog::kShort, offset, value, is_volatile);
}

void Transaction::ObjectLog::Log32BitsValue(MemberOffset offset,
                                            uint32_t value,
                                            bool is_volatile) {
  LogValue(ObjectLog::k32Bits, offset, value, is_volatile);
}

void Transaction::ObjectLog::Log64BitsValue(MemberOffset offset,
                                            uint64_t value,
                                            bool is_volatile) {
...
void Transaction::ObjectLog::Undo(mirror::Object *obj) const {
  for (auto &it : field_values_) {
    ...
    if (field_offset.Uint32Value()
        == mirror::Class::ClassOffset().Uint32Value()) {
...
void Transaction::ObjectLog::UndoFieldWrite(mirror::Object *obj,
  ...
  switch (field_value.kind) {
    ...
    case k64Bits:
      if (UNLIKELY(field_value.is_volatile)) {
        obj->SetField64Volatile<false, kCheckTransaction>(field_offset,
                                                          field_value.value);
      } else {
        obj->SetField64<false, kCheckTransaction>(field_offset,
                                                  field_value.value);
      }
      break;
    case kReference:
...
void Transaction::ArrayLog::Undo(mirror::Array *array) const {
  ...
  DCHECK(array->IsArrayInstance());
  Primitive::Type
      type = array->GetClass()->GetComponentType()->GetPrimitiveType();
...
void Transaction::ArrayLog::UndoArrayWrite(mirror::Array *array,
  ...
  switch (array_type) {
    case Primitive::kPrimBoolean:
      array->AsBooleanArray()->SetWithoutChecks<false>(index,
                                                       static_cast<uint8_t>(value));
      break;
    case Primitive::kPrimByte:
      array->AsByteArray()->SetWithoutChecks<false>(index,
                                                    static_cast<int8_t>(value));
      break;
    case Primitive::kPrimChar:
      array->AsCharArray()->SetWithoutChecks<false>(index,
                                                    static_cast<uint16_t>(value));
      break;
    case Primitive::kPrimShort:
      array->AsShortArray()->SetWithoutChecks<false>(index,
                                                     static_cast<int16_t>(value));
      break;
    case Primitive::kPrimInt:
      array->AsIntArray()->SetWithoutChecks<false>(index,
                                                   static_cast<int32_t>(value));
      break;
    case Primitive::kPrimFloat:
      array->AsFloatArray()->SetWithoutChecks<false>(index,
                                                     static_cast<float>(value));
      break;
    case Primitive::kPrimLong:
      array->AsLongArray()->SetWithoutChecks<false>(index,
                                                    static_cast<int64_t>(value));
      break;
    case Primitive::kPrimDouble:
      array->AsDoubleArray()->SetWithoutChecks<false>(index,
...
void Transaction::AddNewObject(mirror::Object *obj) {
  MutexLock mu(Thread::Current(), log_lock_);
  object_created_.insert(obj);
  object_logs_[obj];
}
