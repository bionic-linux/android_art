...
ObjPtr<mirror::ObjectArray<mirror::Object>> GetAnnotationsForField(ArtField* field)
    REQUIRES_SHARED(Locks::mutator_lock_);
LIBART_PROTECTED ObjPtr<mirror::ObjectArray<mirror::String>> GetSignatureAnnotationForField(ArtField* field)
    REQUIRES_SHARED(Locks::mutator_lock_);
...
LIBART_PROTECTED ObjPtr<mirror::ObjectArray<mirror::String>> GetSignatureAnnotationForMethod(
...
LIBART_PROTECTED uint32_t GetNativeMethodAnnotationAccessFlags(const DexFile& dex_file,
...
LIBART_PROTECTED bool MethodIsNeverCompile(const DexFile& dex_file,
                                 const dex::ClassDef& class_def,
                                 uint32_t method_index);
// Is the method from the `dex_file` with the given `field_index`
...
bool GetInnerClassFlags(Handle<mirror::Class> klass, uint32_t* flags)
    REQUIRES_SHARED(Locks::mutator_lock_);
LIBART_PROTECTED ObjPtr<mirror::ObjectArray<mirror::String>> GetSignatureAnnotationForClass(
    Handle<mirror::Class> klass) REQUIRES_SHARED(Locks::mutator_lock_);
LIBART_PROTECTED const char* GetSourceDebugExtension(Handle<mirror::Class> klass)
    REQUIRES_SHARED(Locks::mutator_lock_);
