...
static VisitorStatus VisitEncodedValue(const ClassData& klass,
  ...
  switch (annotation_value.type_) {
    ...
    default: {
      // * kDexAnnotationArray and kDexAnnotationAnnotation are the only 2 valid value_types causing
      // ProcessAnnotationValue return false.
      ...
      // * For other value_types, we shouldn't need to iterate over annotation_ptr and skip the
      // value here.
      const bool unexpected_annotation =
          annotation_value.type_ == DexFile::kDexAnnotationMethodType ||
          annotation_value.type_ == DexFile::kDexAnnotationMethodHandle;
      if (UNLIKELY(unexpected_annotation)) {
