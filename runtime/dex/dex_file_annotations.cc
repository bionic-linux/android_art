...
bool MethodContainsRSensitiveAccess(const DexFile& dex_file,
  ...
  // presence of @DeadReferenceSafe, which will be rarely. In the long run, we need to quicky
  // check once whether a class has any @ReachabilitySensitive annotations. If not, we can
  // immediately rteturn false here for any method in that class.
  ...
  for (DexInstructionIterator iter = accessor.begin(); iter != accessor.end(); ++iter) {
    switch (iter->Opcode()) {
      ...
      case Instruction::IPUT_SHORT:
        {
          uint32_t field_index = iter->VRegB_22c();
      ...
      case Instruction::INVOKE_CUSTOM:
        {
          ...
          DCHECK(called_method_index < dex_file.NumMethodIds());  // ??? fails
          const dex::MethodId& method_id = dex_file.GetMethodId(called_method_index);
          dex::TypeIndex class_index = method_id.class_idx_;
          const dex::ClassDef * method_class_def = dex_file.FindClassDef(class_index);
