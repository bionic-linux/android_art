...
std::optional<std::string> ThreadList::WaitForSuspendBarrier(AtomicInteger* barrier,
  ...
  if (attempt_of_4 != 1) {
    ...
    static std::atomic<bool> starting_up(true);  // Avoids redundant NanoTime calls.
    if (starting_up.load(std::memory_order_relaxed)) {
      uint64_t nanos_at_start = Runtime::Current()->NanosAtStart();
      if (NanoTime() - nanos_at_start > kStartupNanos) {
        starting_up.store(false, std::memory_order_relaxed);
      }
    }
