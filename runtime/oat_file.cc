  ...
  if (DataBimgRelRoBegin() != nullptr) {
    ...
    if (UNLIKELY(runtime == nullptr)) {
      ...
      *error_msg = StringPrintf("Cannot load oat file '%s' with .data.bimg.rel.ro as executable "
      ...
      // ClassLinker shall perform the relocation when we register a dex file from this oat file.
      // This is to avoid dirtying the pages if the code is never actually ready to be executed.
    }
  }
