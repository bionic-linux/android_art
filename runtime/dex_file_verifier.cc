...
const char* DexFileVerifier::CheckLoadStringByTypeIdx(dex::TypeIndex type_idx,
  ...
  if (UNLIKELY(!CheckIndex(type_idx.index, dex_file_->NumTypeIds(), error_string))) {
    return nullptr;
  }
...
bool DexFileVerifier::FindClassFlags(uint32_t index,
  ...
  if (class_type_index->index >= header_->type_ids_size_) {
    return false;
  }
...
bool DexFileVerifier::CheckInterProtoIdItem() {
  ...
  while (it.HasNext() && *shorty != '\0') {
    if (!CheckIndex(it.GetTypeIdx().index,
...
bool DexFileVerifier::CheckInterClassDefItem() {
  ...
  if (defined_classes_.find(item->class_idx_) != defined_classes_.end()) {
    ErrorStringPrintf("Redefinition of class with type idx: '%d'", item->class_idx_.index);
    return false;
  }
        ...
        if (interface_def != nullptr) {
          ...
          if (interface_def > item) {
            ...
            ErrorStringPrintf("Invalid class definition ordering:"
                              ...
                              item->class_idx_.index,
                              interfaces->GetTypeItem(i).type_idx_.index);
            return false;
          }
        }
      }
...
static std::string GetClassOrError(const uint8_t* const begin,
  ...
  CHECK_LT(class_idx.index, header->type_ids_size_);

  const DexFile::TypeId* type_id =
      reinterpret_cast<const DexFile::TypeId*>(begin + header->type_ids_off_) + class_idx.index;
