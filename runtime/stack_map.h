...
class MethodHeader : public BitTableAccessor<4> {
 public:
  BIT_TABLE_HEADER()
...
class CodeInfo {
 public:
  ...
  ALWAYS_INLINE MethodHeader GetMethodHeader() const {
    DCHECK_EQ(method_headers_.NumRows(), 1u);
    return method_headers_.GetRow(0);
  }
  ...
  ALWAYS_INLINE DexRegisterMap GetDexRegisterMapOf(StackMap stack_map) const {
    if (stack_map.HasDexRegisterMap()) {
      uint32_t number_of_dex_registers = GetMethodHeader().GetNumberOfDexRegisters();
  ...
  ALWAYS_INLINE DexRegisterMap GetDexRegisterMapAtDepth(uint8_t depth, StackMap stack_map) const {
    if (stack_map.HasDexRegisterMap()) {
      ...
      uint32_t first = (depth == 0) ? number_of_dex_registers
