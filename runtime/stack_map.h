...
class InlineInfo : public BitTable<6>::Accessor {
 public:
  ...
  BIT_TABLE_COLUMN(5, NumDexRegisters)  // Includes counts from higher inline levels and from stackmap.
  BIT_TABLE_COLUMN(6, DexRegisterMapIndex)
  ...
  ALWAYS_INLINE DexRegisterMap GetDexRegisterMapAtDepth(uint8_t depth,
    ...
    if (stack_map.HasDexRegisterMap()) {
      uint32_t first = (depth != 0)
          ? GetInlineInfoAtDepth(stack_map, depth - 1).GetNumDexRegisters()
          : num_dex_registers_;
      uint32_t last = GetInlineInfoAtDepth(stack_map, depth).GetNumDexRegisters();
