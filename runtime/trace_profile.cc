...
// 8 is the maximum number of bits we need for encoding one entry. We don't handle buffer overflows
// when processing the buffer. We use this value to ensure we have sufficient in the buffer that
// stores the encoded data.
static constexpr size_t kMinBufSizeForEncodedData = kAlwaysOnTraceBufSize * 8;
...
uint8_t* TraceProfiler::DumpBuffer(uint32_t thread_id,
  ...
  uint8_t* curr_buffer_ptr = buffer + header_size;
...
void TraceProfiler::Dump(const char* filename) {
  ...
  for (Thread* thread : Runtime::Current()->GetThreadList()->GetList()) {
    ...
    thread->SetTraceBufferCurrentEntry(kAlwaysOnTraceBufSize);
  }
}
