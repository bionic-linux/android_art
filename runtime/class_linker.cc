...
mirror::Class* ClassLinker::FindClass(Thread* self,
  ...
  if (UNLIKELY(!descriptor_equals)) {
    ...
    // For backward compatibility, accept the class for target SDK version <= 24.
    // Bug: 34394972
    int32_t target_sdk_version = Runtime::Current()->GetTargetSdkVersion();
    if (target_sdk_version > 0 && target_sdk_version <= 24) {
      ...
      if (!reported) {
        ...
        LOG(WARNING)
            ...
            << " (forbidden for >=25), consequences are unpredictable.";
      }
