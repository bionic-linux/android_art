...
bool ClassLinker::FindClassInBaseDexClassLoaderHelper(ScopedObjectAccessAlreadyRunnable& soa,
  ...
  if (class_loader == nullptr) {
    ...
    return true;
  } else if (IsBootClassLoader(soa, class_loader.Get())) {
...
bool ClassLinker::FindClassInBootClassLoaderClassPath(Thread* self,
  ...
  if (pair.second != nullptr) {
    *result = DefineClass(self,
                          ...
                          *pair.first,
                          *pair.second);
    DCHECK(*result != nullptr || self->IsExceptionPending()) << descriptor;
  }
...
bool ClassLinker::FindClassInBaseDexClassLoaderClassPath(
  ...
  if (class_def != nullptr) {
    *result = DefineClass(soa.Self(), descriptor, hash, class_loader, *dex_file, *class_def);
    DCHECK(*result != nullptr || soa.Self()->IsExceptionPending()) << descriptor;
  }
...
ObjPtr<mirror::Class> ClassLinker::FindClassInAppClassLoader(Thread* self,
  ...
  if (self->IsExceptionPending()) {
    DCHECK(known_hierarchy);
    if (!MatchesDexFileCaughtExceptions(self->GetException(), this)) {
      // Not an exception due to class loading, propagate it up.
      return nullptr;
    }
    // Clear the expected exception, we are going to create our own.
    self->ClearException();
  }
...
ObjPtr<mirror::Class> ClassLinker::FindClass(Thread* self,
  ...
  if (descriptor[0] == '[') {
    ...
    result_ptr = FindClassInAppClassLoader(self, descriptor, hash, class_loader, &descriptor_equals);
  }
  ...
  if (descriptor_equals && result_ptr->GetClassLoader() == class_loader.Get()) {
    // The class has been inserted already, we can return.
    return result_ptr;
  }
...
void ClassLinker::RegisterClassLoader(ObjPtr<mirror::ClassLoader> class_loader) {
  ...
  if (kIsDebugBuild) {
    ...
    ScopedObjectAccessUnchecked soa(self);
    DCHECK(!IsBootClassLoader(soa, class_loader));
  }
...
ClassTable* ClassLinker::InsertClassTableForClassLoader(ObjPtr<mirror::ClassLoader> class_loader) {
  ...
  if (class_table == nullptr) {
    ScopedObjectAccessUnchecked soa(Thread::Current());
    if (IsBootClassLoader(soa, class_loader)) {
      return boot_class_table_.get();
    }
    RegisterClassLoader(class_loader);
