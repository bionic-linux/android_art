...
void ClassLinker::FinishInit(Thread* self) {
  ...
  ArtField* zombie = java_lang_ref_FinalizerReference->GetField(5);
...
void ClassLinker::LoadClass(Thread* self,
  ...
  Runtime* const runtime = Runtime::Current();
  {
    ...
    LengthPrefixedArray<ArtField>* fields = AllocArtFieldArray(self,
                                                               allocator,
...
bool ClassLinker::LinkFieldsHelper::LinkFields(ClassLinker* class_linker,
  ...
  if (kIsDebugBuild) {
    ...
    for (size_t i = 0; i < klass->NumFields(); i++) {
      ArtField* field = &fields->At(i);
      if (field->IsStatic() != is_static) continue;
