...
void ClassLinker::CreateProxyMethod(Handle<mirror::Class> klass, ArtMethod* prototype,
  ...
  DCHECK(!out->IsIntrinsic()) << "Removing kAccDefault from an intrinsic would be a mistake";
  ...
  for (size_t i = 0; i != num_new_copied_methods; ++i) {
    ...
    switch (record->GetState()) {
      ...
      case CopiedMethodRecord::State::kDefault: {
        ...
        static_assert((kAccDefault & kAccIntrinsicBits) != 0);
        DCHECK(!new_method.IsIntrinsic()) << "Setting kAccDefault from an intrinsic would be a mistake";
  ...
  if (klass->IsInterface()) {
    ...
    for (size_t i = 0; i < num_virtual_methods; ++i) {
      ...
      if (ArtMethod::IsInvokable(access_flags)) {
        ...
        static_assert((kAccDefault & kAccIntrinsicBits) != 0);
        DCHECK(!m->IsIntrinsic()) << "Setting kAccDefault from an intrinsic would be a mistake";
