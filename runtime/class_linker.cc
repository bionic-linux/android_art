...
bool ClassLinker::InitWithoutImage(std::vector<std::unique_ptr<const DexFile>> boot_class_path,
  ...
  {
    MutexLock bitstring_lock(Thread::Current(), *Locks::bitstring_lock_);
    InstanceOfTree::Lookup(java_lang_Object.Get()).EnsureInitialized();
    InstanceOfTree::Lookup(java_lang_Class.Get()).EnsureInitialized();
  }
...
bool ClassLinker::AddImageSpace(
  ...
  if (app_image) {
    ...
    // to recalculate the bitstring in the appimage.
    {
      MutexLock bitstring_lock(Thread::Current(), *Locks::bitstring_lock_);
      for (const ClassTable::TableSlot& root : temp_set) {
        InstanceOfTree::Lookup(root.Read()).EnsureInitialized();
      }
    }
  }
...
bool ClassLinker::EnsureInitialized(Thread* self,
  ...
  // InstanceOf::Initialized must happen-before any new-instance for that type.
  //
  ...
  {
    MutexLock bitstring_lock(Thread::Current(), *Locks::bitstring_lock_);
    InstanceOfTree::Lookup(c.Get()).EnsureInitialized();
