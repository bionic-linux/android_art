  ...
  for (size_t j = 0; j < super_vtable_length; ++j) {
    ...
    if (it != declared_virtual_signatures.end()) {
      ...
      if (initialized_methods.IsBitSet(*it)) {
        ...
        // We record that information and make sure the vtable index of the method is
        // the vtable index of the overridden method that is public.
  ...
  if (klass->IsInterface()) {
    ...
    for (ArtMethod& virtual_method : klass->GetVirtualMethodsSliceUnchecked(kPointerSize)) {
      ...
      if (UNLIKELY(vtable_index < same_signature_vtable_lists.size())) {
        ...
        // We really only expect one or two methods overriden, not more: one for
        // package-private visiblity, and one for public visiblity.
