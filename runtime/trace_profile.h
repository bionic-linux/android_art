...
// This class implements the low-overhead tracing. This feature is available only when
// always_enable_profile_code is enabled which is a build time flag. When this flag is enabled, AOT
// and JITed code has ability to record events on each method execution. When a profile is started,
// method entry / exit events are recorded in a per-thread circular buffer. When requested the
// recorded events in the buffer are dumped into a file. The buffers are released when the profile
// is stopped.
class TraceProfiler {
 public:
