...
OatFileAssistant::OatFileAssistant(const char* dex_location,
  ...
  if (pos == std::string::npos) {
    ...
    if (access(parent.c_str(), W_OK) == 0) {
      ...
      VLOG(oat) << "Dex parent is not writable because: " << strerror(errno);
    }
  }
}
...
OatFileAssistant::OatFileInfo& OatFileAssistant::GetBestInfo() {
  if (dex_parent_writable_) {
    ...
    // create the odex file. In this as we unconditionally pick it as
    // the best oat file. This corresponds to the regular use case when
