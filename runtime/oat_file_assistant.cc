...
OatFileAssistant::OatFileAssistant(const char* dex_location,
  ...
  // This is will be needed in most uses of OatFileAssistant and so it's OK to
...
bool OatFileAssistant::Lock(std::string* error_msg) {
  ...
  //
  // The only special case when in theory this could work for primaries is if
  // system server initiates the compilation of non system apps via class
  // loaders. This never happens!
  //
...
OatFileAssistant::MakeUpToDate(bool profile_changed, std::string* error_msg) {
  ...
  switch (info.GetDexOptNeeded(target, profile_changed)) {
    ...
    case kDex2OatForFilter:
      return GenerateOatFile(info, error_msg);
  }
  UNREACHABLE();
}
...
OatFileAssistant::ResultOfAttemptToUpdate
OatFileAssistant::GenerateOatFile(OatFileAssistant::OatFileInfo& info, std::string* error_msg) {
  CHECK(error_msg != nullptr);
