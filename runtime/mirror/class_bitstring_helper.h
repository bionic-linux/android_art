...
#ifndef ART_RUNTIME_MIRROR_CLASS_BITSTRING_HELPER_H_
#define ART_RUNTIME_MIRROR_CLASS_BITSTRING_HELPER_H_
...
constexpr uint32_t BitstringLength[kMaxBitstringDepth + 2] = {0, 12, 22, 32, 41, 49, 55, 64};

constexpr uint32_t kFirstLevelOffset = 52;
static_assert(kFirstLevelOffset + BitstringLength[1] == 64,
              "The first level offset doesn't fit.");
...
inline uint64_t GetUpdatedFirst56Bits(uint64_t old, uint64_t cur) {
  return GetLast8Bits(old) | GetFirst56Bits(cur);
}

inline uint64_t GetUpdatedLast8Bits(uint64_t old, uint64_t cur) {
...
inline uint64_t GetUpdatedBitsByDepth(uint64_t old, uint64_t cur, int dep) {
	return GetUpdatedRangedBits(old, BitstringLength[dep - 1], BitstringLength[dep], cur);
}
}  // namespace mirror
}  // namespace art

#endif  // ART_RUNTIME_MIRROR_CLASS_BITSTRING_HELPER_H_
