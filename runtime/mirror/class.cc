...
void Class::AssignSelfBitstring() {
  ...
  InstanceOfAndStatus now = GetInstanceOfAndStatus();
  DCHECK(parent.GetState(dep - 1) >= InstanceOfAndStatus::State::kBitstringAssigned);
  uint64_t inc = 0;
  bool suc = true;
...
void Class::EnsureInitializedInstanceOf() {
  if (!GetInstanceOfAndStatus().IsUninited()) {
    return;
  }
  ...
  super_class->EnsureInitializedInstanceOf();
