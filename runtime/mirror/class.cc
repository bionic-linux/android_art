...
bool Class::TrySetInstanceOfAndStatus(InstanceOfAndStatus old_value,
  ...
  if (Runtime::Current()->IsActiveTransaction()) {
    return (CasFieldWeakSequentiallyConsistent64<true>
            (StatusOffset(), old_value.GetData(), new_pair.GetData()));
  } else {
...
void Class::SetInstanceOfAndStatus(InstanceOfAndStatus new_value) {
  while (!TrySetInstanceOfAndStatus(GetInstanceOfAndStatus(), new_value)) {
  }
}
...
void Class::InitializeSelfBitstring() {
  ...
  DCHECK_GT(copy.GetState(Depth()), 0);
}
...
bool Class::CanAssignBitstring() {
  ...
  if (parent.IsOverflowed(dep)) {
    if (!now.IsAssigned(dep)) {
      now.MarkOverflowed();
  ...
  return parent.IsAssigned(dep)
         && !now.IsAssigned(dep)
         && !now.IsOverflowed(dep);
}
...
void Class::AssignSelfBitstring() {
  ...
  InstanceOfAndStatus now = GetInstanceOfAndStatus();
  DCHECK(parent.GetState(dep - 1) >= InstanceOfAndStatus::kBitstringAssigned);
  uint64_t inc = 0;
  bool suc = true;
  InstanceOfAndStatus temp = super_class->GetInstanceOfAndStatus();
  ...
  } while (!super_class->TrySetInstanceOfAndStatus(temp, parent));
  ...
  } while (!TrySetInstanceOfAndStatus(temp, now));
}
...
// at least kBitstringInitialized state.
void Class::InitializeBitstring() {
  ...
  if (super_class->GetInstanceOfAndStatus().IsUninited()) {
    super_class->InitializeBitstring();
  }
