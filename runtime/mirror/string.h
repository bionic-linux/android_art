...
class MANAGED String FINAL : public Object {
 public:
  ...
  int32_t GetLength() SHARED_REQUIRES(Locks::mutator_lock_) {
    return (GetCount<kVerifyFlags>() & INT32_MAX);
  }
  ...
  bool IsCompressed() SHARED_REQUIRES(Locks::mutator_lock_) {
    ...
    return (GetCount() & first_bit) != 0;
  }

  bool IsValueNull() SHARED_REQUIRES(Locks::mutator_lock_);

  static bool AllASCII(const uint16_t* const chars, const int length);

  static bool GetCompressionFlagFromCount(const int32_t count);

  static int32_t GetLengthFromCount(const int32_t count);

  static int32_t GetFlaggedCount(const int32_t count);
