...
class SetStringCountAndBytesVisitor {
 public:
  ...
  void operator()(Object* obj, size_t usable_size ATTRIBUTE_UNUSED) const
    ...
    String* string = down_cast<String*>(obj);
    string->SetCount(count_);
    DCHECK_EQ(string->IsCompressed(), kUseCompression);
...
class SetStringCountAndValueVisitorFromCharArray {
 public:
  ...
  void operator()(Object* obj, size_t usable_size ATTRIBUTE_UNUSED) const
    ...
    bool compressible = String::GetCompressionFlagFromCount(count_);
    if (compressible) {
      for (int i = 0; i < length; ++i) {
...
inline String* String::Alloc(Thread* self, int32_t utf16_length_with_flag,
  ...
  const bool compressible = String::GetCompressionFlagFromCount(utf16_length_with_flag);
...
inline String* String::AllocFromByteArray(Thread* self, int32_t byte_length,
  ...
  const int32_t length_with_flag = (compressible) ? String::GetFlaggedCount(byte_length) : byte_length;
...
inline String* String::AllocFromCharArray(Thread* self, int32_t count,
  ...
  const bool compressible = kUseCompression && String::AllASCII(array->GetData() + offset, count);
...
inline String* String::AllocFromString(Thread* self, int32_t string_length, Handle<String> string,
  ...
  const bool compressible = kUseCompression &&
                            ((string->IsCompressed()) ? true
                                                      : String::AllASCII(string->GetValue() + offset,
                                                                         string_length));
