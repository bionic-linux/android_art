...
class SetStringCountAndBytesVisitor {
 public:
  SetStringCountAndBytesVisitor(int32_t count, Handle<ByteArray> src_array, int32_t offset, int32_t high_byte ATTRIBUTE_UNUSED)
...
inline size_t String::SizeOf() {
  ...
  if (IsCompressed()) {
    size += 2 * (sizeof(uint8_t) * GetLength<kVerifyFlags>());
  } else {
