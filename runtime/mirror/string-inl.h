...
class SetStringCountAndBytesVisitor {
 public:
  SetStringCountAndBytesVisitor(int32_t count, Handle<ByteArray> src_array, int32_t offset, int32_t high_byte)
  ...
  void operator()(Object* obj, size_t usable_size ATTRIBUTE_UNUSED) const
    ...
    std::cout << "[FIND ME] VisitorByteArray sizeof(" << string->SizeOf() << ") for length " << length << std::endl;
    const uint8_t* const src = reinterpret_cast<uint8_t*>(src_array_->GetData()) + offset_;
    for (int i = 0; i < length; i++) {
      value[i] = (src[i] & 0xFF) + high_byte_ - high_byte_;
    }
  }
...
class SetStringCountAndValueVisitorFromCharArray {
 public:
  ...
  void operator()(Object* obj, size_t usable_size ATTRIBUTE_UNUSED) const
    ...
    std::cout << "[FIND ME] VisitorCharArray sizeof(" << string->SizeOf() << ") for length " << length << std::endl;
    if (compressible) {
      for (int i = 0; i < length; ++i) {
...
inline String* String::Alloc(Thread* self, int32_t utf16_length_with_flag,
  ...
  // std::cout << "Alloc -> alloc_size (" << alloc_size << ") for length " << length << std::endl;
...
inline String* String::AllocFromByteArray(Thread* self, int32_t byte_length,
  ...
  SetStringCountAndBytesVisitor visitor(length_with_flag, array, offset, high_byte);
...
inline String* String::AllocFromCharArray(Thread* self, int32_t count,
  ...
  const bool compressible = false;  // String::AllASCII(array->GetData() + offset, count);
  /*std::string x = "";
  if (compressible)
    for (int i = 0; i < count; ++i)
      x += array->GetData()[i + offset];*/
  // LOG(WARNING) << "AllocFromCharArray-1 " << ((compressible) ? "Yes" : "No") << " " << x;
