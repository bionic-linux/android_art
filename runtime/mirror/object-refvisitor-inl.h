...
inline size_t Object::TotalReferenceInstanceFields(ObjPtr<Class> klass) {
  if (klass->IsObjectArrayClass<kVerifyFlags, kReadBarrierOption>()) {
    return AsObjectArray<mirror::Object, kVerifyNone, kReadBarrierOption>()->GetLength();
  } else {
    size_t total_reference_instance_fields = 0;
    for (; klass != nullptr; klass = klass->GetSuperClass<kVerifyFlags, kReadBarrierOption>()) {
