...
String* String::AllocFromStrings(Thread* self, Handle<String> string, Handle<String> string2) {
  ...
  const bool compressible = kUseCompression && (string->IsCompressed() && string2->IsCompressed());
...
String* String::AllocFromUtf16(Thread* self, int32_t utf16_length, const uint16_t* utf16_data_in) {
  ...
  gc::AllocatorType allocator_type = Runtime::Current()->GetHeap()->GetCurrentAllocator();
  const bool compressible = kUseCompression && String::AllASCII(utf16_data_in, utf16_length);
...
String* String::AllocFromModifiedUtf8(Thread* self, int32_t utf16_length,
  ...
  gc::AllocatorType allocator_type = Runtime::Current()->GetHeap()->GetCurrentAllocator();
  const bool compressible = kUseCompression && (utf16_length == utf8_length);
...
int32_t String::CompareTo(String* rhs) {
  ...
  if (lhs->IsCompressed() && rhs->IsCompressed()) {
    int32_t comparison = memcmp(lhs, rhs, minCount * sizeof(uint8_t));
    if (comparison != 0) {
      return comparison;
    }
...
bool String::GetCompressionFlagFromCount(const int32_t count) {
  return (count & (1u << 31)) != 0;
}

int32_t String::GetLengthFromCount(const int32_t count) {
  return (count & INT32_MAX);
}

int32_t String::GetFlaggedCount(const int32_t count) {
  return (count | (1u << 31));
}
