...
String* String::AllocFromStrings(Thread* self, Handle<String> string, Handle<String> string2) {
  ...
  const bool compressible = false;  // (string->IsCompressed() && string2->IsCompressed());
  const int32_t length_with_flag = (compressible) ? ((length + length2) | (1u << 31)) : (length + length2);
  ...
  if (compressible) {
    ...
    if (string->IsCompressed()) {
      // memcpy(new_value, string->ToCharArray(self)->GetData(), length * sizeof(uint8_t));
    ...
    if (string2->IsCompressed()) {
      // memcpy(new_value + length, string2->ToCharArray(self)->GetData(), length2 * sizeof(uint8_t));
...
String* String::AllocFromUtf16(Thread* self, int32_t utf16_length, const uint16_t* utf16_data_in) {
  ...
  gc::AllocatorType allocator_type = Runtime::Current()->GetHeap()->GetCurrentAllocator();
  const bool compressible = false;  // String::AllASCII(utf16_data_in, utf16_length);
  int32_t length_with_flag = (compressible) ? (utf16_length | (1u << 31)) :
                                              utf16_length;
  SetStringCountVisitor visitor(length_with_flag);
...
String* String::AllocFromModifiedUtf8(Thread* self, int32_t utf16_length,
  ...
  gc::AllocatorType allocator_type = Runtime::Current()->GetHeap()->GetCurrentAllocator();
  const bool compressible = false;  // (utf16_length == utf8_length);
  const int32_t utf16_length_with_flag = (compressible) ? (utf16_length | (1u << 31)) :
                                                           utf16_length;
...
bool String::AllASCII(const char* chars) {
  int length = strlen(chars);
  for (int i = 0; i < length; ++i) {
    if (chars[i] <= 0 || chars[i] > 0x7f) {
      return false;
    }
  }
  return true;
...
bool String::AllASCII(const uint16_t* const chars, const int length) {
  for (int i = 0; i < length; ++i) {
    if (chars[i] <= 0 || chars[i] > 0xff) {
      return false;
    }
  }
  return true;
