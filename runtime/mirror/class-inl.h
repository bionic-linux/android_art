...
inline bool Class::IsSubClass(ObjPtr<Class> klass) {
  ...
  if (kIsDebugBuild) {
    SubtypeCheck this_tree = SubtypeCheck::Lookup(this);
    SubtypeCheck klass_tree = SubtypeCheck::Lookup(klass);

    SubtypeCheckInfo::Result result = this_tree.IsSubtypeOf(klass_tree);
    if (result != SubtypeCheckInfo::kUnknownSubtypeOf) {
      SubtypeCheckInfo::Result slow_result =
          slow_path ? SubtypeCheckInfo::kSubtypeOf : SubtypeCheckInfo::kNotSubtypeOf;
