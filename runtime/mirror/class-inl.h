...
inline void Class::FixupNativePointers(Class* dest,
  ...
  auto DestAddress = [dest](MemberOffset offset) {
  ...
  void** sfields_dest_address = DestAddress(OFFSET_OF_OBJECT_MEMBER(Class, sfields_));
  ...
  void** ifields_dest_address = DestAddress(OFFSET_OF_OBJECT_MEMBER(Class, ifields_));
  ...
  // Update method array.
  LengthPrefixedArray<ArtMethod>* methods = GetMethodsPtr();
  void** methods_dest_address = DestAddress(OFFSET_OF_OBJECT_MEMBER(Class, methods_));
  ...
  if (!IsTemp() && ShouldHaveEmbeddedVTable<kVerifyNone, kReadBarrierOption>()) {
    for (int32_t i = 0, count = GetEmbeddedVTableLength(); i < count; ++i) {
      ...
      void** method_dest_addr = DestAddress(EmbeddedVTableEntryOffset(i, pointer_size));
  ...
  if (!IsTemp() && ShouldHaveImt<kVerifyNone, kReadBarrierOption>()) {
    ...
    void** imt_dest_addr = DestAddress(ImtPtrOffset(pointer_size));
