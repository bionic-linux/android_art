...
class PtrCompression {
  ...
  static MirrorType* UnCompress(uint32_t ref) {
...
template<bool kPoisonReferences, class MirrorType>
class MANAGED ObjectReference : PtrCompression<kPoisonReferences, MirrorType> {
 public:
  MirrorType* AsMirrorPtr() const {
    return this->UnCompress(reference_);
  }

  void Assign(MirrorType* other) {
    reference_ = this->Compress(other);
  }
...
class MANAGED HeapReference : PtrCompression<kPoisonHeapReferences, MirrorType> {
 public:
  template <bool kIsVolatile = false>
  MirrorType* AsMirrorPtr() const REQUIRES_SHARED(Locks::mutator_lock_) {
    if (kIsVolatile) {
      return this->UnCompress(reference_.LoadSequentiallyConsistent());
    } else {
      return this->UnCompress(reference_.LoadJavaData());
    }
  }
  ...
  void Assign(MirrorType* other) REQUIRES_SHARED(Locks::mutator_lock_) {
    if (kIsVolatile) {
      reference_.StoreSequentiallyConsistent(this->Compress(other));
    } else {
      reference_.StoreJavaData(this->Compress(other));
    }
  }
  ...
  // uint32_t AsVRegValue() const {
  //   return reference_.LoadJavaData();
  // }
