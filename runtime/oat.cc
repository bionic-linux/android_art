...
uint32_t OatHeader::GetBCPInfoOffset() const {
  DCHECK(IsValid());
  DCHECK_GT(bcp_info_offset_, sizeof(OatHeader));
  return bcp_info_offset_;
}

void OatHeader::SetBCPInfoOffset(uint32_t bcp_info_offset) {
  DCHECK_GT(bcp_info_offset, sizeof(OatHeader));
  DCHECK(IsValid());
  DCHECK_EQ(bcp_info_offset_, 0u);

  bcp_info_offset_ = bcp_info_offset;
}
