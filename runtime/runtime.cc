...
void Runtime::StartSignalCatcher() {
  if (!is_zygote_) {
    signal_catcher_ = new SignalCatcher(stack_trace_file_, tombstoned_traces_);
  }
}
...
bool Runtime::Init(RuntimeArgumentMap&& runtime_options_in) {
  ...
  tombstoned_traces_ = runtime_options.GetOrDefault(Opt::TombstonedTraces);
#if !defined(ART_TARGET_ANDROID)
  if (tombstoned_traces_) {
    LOG(FATAL) << "-Xtombstonedtraces is only supported in an Android environment";
  }
#endif
