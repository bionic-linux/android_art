...
struct InstanceOfAndStatus {
  ...
  enum State {
    ...
    kBitstringOverflowuninitialized = 1,
    kBitstringInitialized = 2,
    kBitstringAssigned = 3,
    kBitstringOverflowed = 4,
  };
  ...
  State GetState(size_t dep) const {
    ...
    if (IsAssigned(dep)) {
      return kBitstringAssigned;
    }
    if (IsOverflowed(dep)) {
      if (GetBitstring() == 0) {
        return kBitstringOverflowuninitialized;
      } else {
    ...
    if (GetBitstring() == 0) {
      return kBitstringUninitialized;
    }
    return kBitstringInitialized;
  }
