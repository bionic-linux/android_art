...
void WellKnownClasses::InitFieldsAndMethodsOnly(JNIEnv* env) {
  ...
  Handle<mirror::Class> j_l_rl_Proxy =
      hs.NewHandle(FindSystemClass(class_linker, self, "Ljava/lang/reflect/Proxy;"));
...
void WellKnownClasses::LateInit(JNIEnv* env) {
  ...
  // is `java_lang_Runtime_nativeLoad` and, if so, returns the class loader overide.
  // However, this is valid only after the override has been set up by the `Runtime`,
  // so we cannot initialize `java_lang_Runtime_nativeLoad` in `Init()` because
  // the .
  // TODO: Clean up the initializaion steps.
  ...
  // confused.
  DCHECK_NE(
      java_lang_reflect_Proxy_init->GetEntryPointFromQuickCompiledCode(),
