...
struct InstanceOfAndStatus {
  ...
  explicit InstanceOfAndStatus(StorageType value) : data(value) {}

  StorageType GetData() const {
    return data;
  }

  void SetData(StorageType now) {
    data = now;
  }

  StorageType GetBitstring() const {
  ...
  void SetBitstring(StorageType now) {
    data = UpdateFirst56Bits(data, now);
  }

  void SetStatus(StorageType now) {
  ...
  void SetIncrementalValue(StorageType inc, size_t dep) {
  ...
  void InitializeBitstring(StorageType super, size_t dep) {
