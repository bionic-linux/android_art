...
class FuzzerCorpusTest : public CommonRuntimeTest {
 public:
  ...
  static void ClassVerification(const uint8_t* data,
    ...
    // We need this for the class loader since the test's runtime doesn't handle it. We also
    // register/unregister the dex files manually.
  ...
  void TestFuzzerHelper(
    ...
    while ((error = Next(cookie, &entry, &name)) >= 0) {
      ...
      verify_file(
          reinterpret_cast<const uint8_t*>(data.data()), data.size(), name, is_valid_dex_file);
    }
