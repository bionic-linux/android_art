...
class FuzzerCorpusTest : public CommonRuntimeTest {
 public:
  ...
  static void VerifyClassDexFile(const uint8_t* data,
                                 size_t size,
                                 const std::string& name,
                                 bool expected_success) {
    // Do not verify the checksum as we only care about the DEX file contents,
    ...
    ASSERT_EQ(success_dex, true) << " Failed dex file verification for " << name;
    ...
    ASSERT_EQ(success, expected_success) << " Failed class dex file verification for " << name;
  }

  void TestFuzzerHelper(const std::string& archive,
                        const std::unordered_set<std::string>& valid_dex_files,
                        void (*verifyCorpus)(const uint8_t*, size_t, const std::string&, bool)) {
    // Consistency checks.
    ...
    const std::string filename = folder + archive;
...
TEST_F(FuzzerCorpusTest, VerifyCorpusDexFiles) {
  ...
  const std::string archive = "/fuzzer_corpus.zip";

  TestFuzzerHelper(archive, valid_dex_files, VerifyDexFile);
}
...
TEST_F(FuzzerCorpusTest, VerifyCorpusClassDexFiles) {
  // These dex files are expected to pass verification. The others are regressions tests.
  const std::unordered_set<std::string> valid_dex_files = {"Main.dex", "hello_world.dex"};
  const std::string archive = "/class_fuzzer_corpus.zip";

  TestFuzzerHelper(archive, valid_dex_files, VerifyClassDexFile);
}
