  ...
  static void ClassVerification(const uint8_t* data,
    ...
    // Scope for the handles
    {
      ...
      for (ClassAccessor accessor : dex_file.GetClasses()) {
        ...
        const Handle<mirror::Class> h_klass(scope.NewHandle<mirror::Class>(
            class_linker->FindClass(soa.Self(), descriptor, h_loader)));
        const Handle<mirror::DexCache> h_dex_cache(
            scope.NewHandle<mirror::DexCache>(h_klass->GetDexCache()));
