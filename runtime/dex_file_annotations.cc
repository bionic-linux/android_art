...
static void DCheckNativeAnnotation(ScopedObjectAccess& soa, const char* descriptor, jclass cls)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  if (kIsDebugBuild) {
    ObjPtr<mirror::Class> klass = soa.Decode<mirror::Class>(cls);
    ClassLinker* linker = Runtime::Current()->GetClassLinker();
    // WellKnowClasses may not be initialized yet, so `klass` may be null.
    if (klass != nullptr) {
...
uint32_t GetNativeMethodAnnotationAccessFlags(const DexFile& dex_file,
  ...
  if (annotation_set != nullptr) {
    ...
    for (uint32_t i = 0; i < annotation_set->size_; ++i) {
      const DexFile::AnnotationItem* annotation_item =
          dex_file.GetAnnotationItem(annotation_set, i);
      ...
      if (strcmp(descriptor, "Ldalvik/annotation/optimization/FastNative;") == 0) {
        access_flags |= kAccFastNative;
        DCheckNativeAnnotation(soa,
                               descriptor,
                               WellKnownClasses::dalvik_annotation_optimization_FastNative);
      } else if (strcmp(descriptor, "Ldalvik/annotation/optimization/CriticalNative;") == 0) {
