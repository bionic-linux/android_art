...
static bool IsMethodBuildAnnotationPresent(Thread* self,
                                           const DexFile& dex_file,
                                           const DexFile::AnnotationSetItem& annotation_set,
                                           ObjPtr<mirror::Class> annotation_class)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  for (uint32_t i = 0; i < annotation_set.size_; ++i) {
    const DexFile::AnnotationItem* annotation_item = dex_file.GetAnnotationItem(&annotation_set, i);
...
uint32_t GetNativeMethodAnnotationAccessFlags(const DexFile& dex_file,
  ...
  if (annotation_set != nullptr) {
    ...
    ObjPtr<mirror::Class> fast_native =
        soa.Decode<mirror::Class>(WellKnownClasses::dalvik_annotation_optimization_FastNative);
    if (IsMethodBuildAnnotationPresent(soa.Self(), dex_file, *annotation_set, fast_native)) {
      access_flags |= kAccFastNative;
    }
    ObjPtr<mirror::Class> critical_native =
        soa.Decode<mirror::Class>(WellKnownClasses::dalvik_annotation_optimization_CriticalNative);
    if (IsMethodBuildAnnotationPresent(soa.Self(), dex_file, *annotation_set, critical_native)) {
