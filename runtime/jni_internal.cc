...
static jmethodID FindMethodID(ScopedObjectAccess& soa, jclass jni_class,
    ...
    REQUIRES_SHARED(Locks::mutator_lock_) {
  mirror::Class* c = EnsureInitialized(soa.Self(), soa.Decode<mirror::Class>(jni_class).Decode());
  if (c == nullptr) {
    return nullptr;
  }
...
static jfieldID FindFieldID(const ScopedObjectAccess& soa, jclass jni_class, const char* name,
  ...
  Handle<mirror::Class> c(
      hs.NewHandle(EnsureInitialized(soa.Self(), soa.Decode<mirror::Class>(jni_class).Decode())));
  if (c.Get() == nullptr) {
    return nullptr;
  }
 ...
 public:
  ...
  static jobject AllocObject(JNIEnv* env, jclass java_class) {
    ...
    mirror::Class* c = EnsureInitialized(soa.Self(),
                                         soa.Decode<mirror::Class>(java_class).Decode());
    if (c == nullptr) {
      return nullptr;
    }
    if (c->IsStringClass()) {
...
#define SET_STATIC_PRIMITIVE_FIELD(fn, value) \
  ...
  static const jchar* GetStringChars(JNIEnv* env, jstring java_string, jboolean* is_copy) {
    ...
    if (heap->IsMovableObject(s.Decode()) || s->IsCompressed()) {
  ...
  static const jchar* GetStringCritical(JNIEnv* env, jstring java_string, jboolean* is_copy) {
    CHECK_NON_NULL_ARGUMENT(java_string);
    ScopedObjectAccess soa(env);
    mirror::String* s = soa.Decode<mirror::String>(java_string).Decode();
    gc::Heap* heap = Runtime::Current()->GetHeap();
  ...
  static void ReleaseStringCritical(JNIEnv* env,
    ...
    gc::Heap* heap = Runtime::Current()->GetHeap();
    ObjPtr<mirror::String> s = soa.Decode<mirror::String>(java_string);
    if (heap->IsMovableObject(s.Decode())) {
      if (!kUseReadBarrier) {
  ...
  static void* GetPrimitiveArrayCritical(JNIEnv* env, jarray java_array, jboolean* is_copy) {
    ...
    gc::Heap* heap = Runtime::Current()->GetHeap();
    if (heap->IsMovableObject(array.Decode())) {
      if (!kUseReadBarrier) {
        heap->IncrementDisableMovingGC(soa.Self());
      } else {
        // For the CC collector, we only need to wait for the thread flip rather than the whole GC
