...
static inline void CheckUnattachedThread(LockLevel level) NO_THREAD_SAFETY_ANALYSIS {
  // The check below enumerates the cases where we expect not to be able to validity check locks
...
inline bool Mutex::IsExclusiveHeld(const Thread* self) const {
  ...
  if (kDebugLocking) {
    // Validity debug check that if we think it is locked we have it in our held mutexes.
...
inline bool ReaderWriterMutex::IsExclusiveHeld(const Thread* self) const {
  ...
  bool result = (GetExclusiveOwnerTid() == SafeGetTid(self));
  if (kDebugLocking) {
    // Validity that if the pthread thinks we own the lock the Thread agrees.
