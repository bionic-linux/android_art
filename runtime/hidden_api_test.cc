...
class HiddenApiTest : public CommonRuntimeTest {
 protected:
  ...
  void TestLocation(const std::string& location, hiddenapi::Domain expected_domain) {
    ...
    const std::string_view prefix = location.substr(0, location.length() - suffix.length());
    ...
    memcpy(unique_location.get(), prefix.data(), prefix.length());
    memcpy(unique_location.get() + prefix.length(),
           placeholder.data(),
           placeholder.length());
    memcpy(unique_location.get() + prefix.length() + placeholder.length(),
           prefix.data(),
           suffix.length());
