...
bool Instrumentation::PushDeoptContextIfNeeded(Thread* self,
  ...
  if (!ShouldDeoptimizeCurrentMethod(self, sp)) {
    return false;
  }
  ...
  self->PushDeoptimizationContext(
      return_value, is_ref, self->GetException(), /* from_code= */ false, deopt_type);
...
void Instrumentation::DeoptimizeIfNeeded(Thread* self,
  ...
  if (self->IsAsyncExceptionPending() || ShouldDeoptimizeCurrentMethod(self, sp)) {
    JValue return_value;
    return_value.SetJ(0);
...
bool Instrumentation::ShouldDeoptimizeCurrentMethod(Thread* self, ArtMethod** sp) {
