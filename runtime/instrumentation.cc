...
void Instrumentation::DeoptimizeIfNeeded(Thread* self, DeoptimizationMethodType type) {
  ...
  // Check if we need to deoptimize the method. If there is a pending async
  // exception switch to interpreter to handle it.
  if (self->IsAsyncExceptionPending() || ShouldDeoptimizeMethod(self, visitor)) {
    JValue return_value;
    return_value.SetJ(0);
