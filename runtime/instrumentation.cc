...
bool Instrumentation::PushDeoptContextIfNeeded(Thread* self,
  ...
  if (!ShouldDeoptimizeMethod(self, sp)) {
    return false;
  }
...
void Instrumentation::DeoptimizeIfNeeded(Thread* self,
                                         ...
                                         JValue return_value,
                                         bool is_reference) {
  if (self->IsAsyncExceptionPending() || ShouldDeoptimizeMethod(self, sp)) {
...
bool Instrumentation::ShouldDeoptimizeMethod(Thread* self, ArtMethod** sp) {
  ...
  bool method_needs_deopt = NeedsSlowInterpreterForMethod(self, caller);
