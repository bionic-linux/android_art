...
void InstrumentationInstallStack(Thread* thread, void* arg, bool deopt_all_frames)
    ...
    bool VisitFrame() override REQUIRES_SHARED(Locks::mutator_lock_) {
      ...
      if (return_pc == instrumentation_exit_pc_) {
        ...
        InstrumentationStackFrame instrumentation_frame(
            GetThisObject().Ptr(), m, return_pc, GetFrameId(), false, force_deopt_id_);
        if (kVerboseInstrumentation) {
  ...
  if (visitor.has_quick_frames_) {
    instrumentation->SetImmediateDeoptRequired(true);
  }
...
static void InstrumentationRestoreStack(Thread* thread, void* arg)
  ...
  stack->clear();
  if (!visitor.has_quick_frames_) {
    instrumentation->SetImmediateDeoptRequired(false);
  }
}
...
bool Instrumentation::PushDeoptContextIfNeeded(Thread* self, DeoptimizationMethodType deopt_type)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  if (LIKELY(!IsImmediateDeoptRequired())) {
    return false;
  }
  if (self->IsExceptionPending()) {
    return false;
  ...
  DCHECK(sp != nullptr);
  // A few runtime methods like artLockObjectFromCode can be called both from native and compiled
  // code. If it is called from native code no further checks are required.
  if ((*sp)->IsNative()) {
    return false;
  }
  DCHECK((*sp)->IsRuntimeMethod());
  if (!ShouldDeoptimize(self, sp)) {
    return false;
  }

  // self->SetDeoptRequired(false);
  ...
  JValue return_value;
  return_value.SetJ(0);
  self->PushDeoptimizationContext(return_value, false, self->GetException(), false, deopt_type);
...
void Instrumentation::DeoptimizeIfNeeded(Thread* self,
  ...
  if (self->IsAsyncExceptionPending() || ShouldDeoptimize(self, sp)) {
    JValue return_value;
    return_value.SetJ(0);
...
bool Instrumentation::ShouldDeoptimizeMethod(Thread* self, ArtMethod* method) {
  return (method != nullptr) &&
         (InterpreterStubsInstalled() || IsDeoptimized(method) || self->IsForceInterpreter() ||
...
bool Instrumentation::ShouldDeoptimize(Thread* self, ArtMethod** sp) {
  ...
  if (caller == nullptr || caller->IsNative() ||
      caller_pc == reinterpret_cast<uintptr_t>(GetQuickInstrumentationExitPc()) ||
      !Runtime::Current()->IsAsyncDeoptimizeable(caller_pc)) {
    return false;
  }

  bool method_needs_deopt = ShouldDeoptimizeMethod(self, caller);
  if (!Runtime::Current()->IsJavaDebuggable() || method_needs_deopt) {
    return method_needs_deopt;
  }
  ...
  return method_needs_deopt || should_deoptimize_frame;
}

bool Instrumentation::ShouldDeoptimize(Thread* self, const NthCallerVisitor& visitor) {
  bool should_deoptimize_frame = false;
  ...
  return ShouldDeoptimizeMethod(self, visitor.caller) || should_deoptimize_frame;
}
...
TwoWordReturn Instrumentation::PopInstrumentationStackFrame(Thread* self,
  ...
  *return_pc_addr = instrumentation_frame.return_pc_;
  ...
  bool deoptimize = ShouldDeoptimize(self, visitor) || should_deoptimize_frame;
