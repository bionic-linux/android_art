...
bool Instrumentation::ShouldDeoptimizeCaller(Thread* self, ArtMethod** sp) {
  ...
  if (caller == nullptr ||
    ...
    // instrumentation exit trampoline so we don't need to handle deoptimizations here. For the
    // other cases (native and when caller is nullptr) we don't need to deoptimize.
    return false;
  }
