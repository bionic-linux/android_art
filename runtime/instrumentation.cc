...
bool Instrumentation::ShouldDeoptimizeCaller(Thread* self, ArtMethod** sp) {
  ...
  if (caller == nullptr ||
    ...
    // not safe to use any quick code because the field offsets might change. For native
    // methods, we don't embed any field offsets so no need to check for a deoptimization.
    // If the caller is null we don't need to do anything. This can happen during shutdown
    // or early startup.
    return false;
  }
