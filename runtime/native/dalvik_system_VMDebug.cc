...
static jobject VMDebug_locateJavaMethodNative(JNIEnv* env, jclass, jobject javaMethod) {
  ...
  const uint64_t* oatMethodQuickCode =
      reinterpret_cast<const uint64_t*>(artMethod->GetOatMethodQuickCode(kRuntimePointerSize));

  auto odexOffset = -1;  // TODO: get the offset of the odex itself like from proc/<pid>/map
  ...
  auto methodOffset = oatMethodQuickCode == 0 ?
                          0 :
                          oatMethodQuickCode - reinterpret_cast<const uint64_t*>(elfBegin);
