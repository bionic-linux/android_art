...
static jobjectArray VMDebug_getVmFeatureList(JNIEnv* env, jclass) {
  ...
  Handle<mirror::ObjectArray<mirror::String>> array = hs.NewHandle(
      mirror::ObjectArray<mirror::String>::Alloc(
          self, GetClassRoot<mirror::ObjectArray<mirror::String>>(), arraysize(features)));
  if (array == nullptr) {
    DCHECK(self->IsExceptionPending());
    return nullptr;
  }
  for (size_t i = 0; i < arraysize(features); ++i) {
    ObjPtr<mirror::String> ofeature = mirror::String::AllocFromModifiedUtf8(self, features[i]);
    if (ofeature == nullptr) {
      DCHECK(self->IsExceptionPending());
      return nullptr;
    }
