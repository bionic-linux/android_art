...
// Delete dex files associated with this dalvik.system.DexFile that are
// not currently in use. Returns true if all dex files were deleted, false if
// some dex files could not be deleted.
static bool CloseDexFiles(DexFileCookie* cookie) {
  ...
  if (all_deleted && cookie->oat_file != nullptr) {
...
static void FreeDexFileCookie(void* address) {
  DexFileCookie* cookie = reinterpret_cast<DexFileCookie*>(address);
  if (!CloseDexFiles(cookie)) {
...
static jboolean DexFile_closeDexFile(JNIEnv*, jclass, jlong cookie_addr) {
  DexFileCookie* cookie = DexFileCookieFromAddr(cookie_addr);
  CHECK(cookie != nullptr);
  return CloseDexFiles(cookie) ? JNI_TRUE : JNI_FALSE;
}
...
static JNINativeMethod gMethods[] = {
  NATIVE_METHOD(DexFile, closeDexFile, "(J)Z"),
