...
static jobjectArray Executable_getParameterAnnotationsNative(JNIEnv* env, jobject javaMethod) {
  ...
  // If the method is not a constructor has parameter annotations for
  // each parameter then we can return those annotations unmodified.
  // Otherwise, we need to look at whether the constructor has
  // implicit parameters that need empty annotations.
