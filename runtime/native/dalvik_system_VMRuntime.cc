...
static void VMRuntime_registerAppInfo(JNIEnv* env,
  ...
  int profile_fd = open(ref_profile_file_str.c_str(), O_RDONLY);
  if (profile_fd < 0) {
    PLOG(ERROR) << "CALIN: Can't open " << ref_profile_file_str;
    return;
  }
  LOG(ERROR) << "CALIN: I CAN open " << ref_profile_file_str;
