...
const DexFile::ProtoId* DexFile::FindProtoId(dex::TypeIndex return_type_idx,
  ...
  while (hi >= lo) {
    ...
    int compare = return_type_idx.index - proto.return_type_idx_.index;
    if (compare == 0) {
      DexFileParameterIterator it(*this, proto);
      size_t i = 0;
      while (it.HasNext() && i < signature_length && compare == 0) {
        compare = signature_type_idxs[i].index - it.GetTypeIdx().index;
        it.Next();
        i++;
      }
      if (compare == 0) {
...
std::string DexFile::PrettyType(dex::TypeIndex type_idx) const {
  if (type_idx.index >= NumTypeIds()) {
    return StringPrintf("<<invalid-type-idx-%d>>", type_idx.index);
  }
