...
void VdexFile::UnquickenDexFile(const DexFile& target_dex_file,
  ...
  for (ClassAccessor class_accessor : target_dex_file.GetClasses()) {
    auto methods = class_accessor.GetMethods();
    for (ClassAccessor::DataIterator<ClassAccessor::Method> it = methods.begin();
        it != methods.end();
        ++it) {
      const DexFile::CodeItem* code_item = it->GetCodeItem();
      if (code_item != nullptr && unquickened_code_item.emplace(code_item).second) {
        const uint32_t offset = accessor.GetOffset(it->GetIndex());
        ...
        it.UnHideAccessFlags();
      }
    }
    auto fields = class_accessor.GetMethods();
