...
public class DominatorsComputation {
  ...
  private static class NodeS {
    ...
    // If foo.id > this.maxid, then foo is not reachable from this node.
    public long maxid;
  ...
  private static boolean isReachableAscending(NodeS srcS, NodeS dstS) {
    if (dstS.id < srcS.id) {
      ...
      return dstS.inRefIds.hasIdInRange(srcS.id, srcS.maxid);
    }
