...
public class CollectionContentsHelper {
  /**
   * A DocString summarizing the old value if the value has changed, or nothing if the value is the
   * same.
   */
  public static DocString baselineValue(Value nowValue, Value wasValue) {
    if (Objects.equals(nowValue, wasValue)) {
      return DocString.text("");
    }
    return DocString.text("was ").append(Summarizer.summarize(wasValue));
  }
