...
public class ListHandler implements KnownTypesHandler {
  ...
  */
  private int maxContentsChars = 200;
  ...
  /**
   * Set the maximum number of visible characters used when summarizing the contents of the list.
   */
  public ListHandler setMaxContentsCharactersInSummary(int maxChars) {
    maxContentsChars = maxChars;
    return this;
  }
  ...
  public DocString summarize(AhatInstance instance) {
    ...
    try {
      ...
      int remaining = maxContentsChars;
  ...
  private static void printDiffTable(
    ...
    doc.table(new Column("Value Now"), new Column("Size"), new Column("Δ"),
        new Column("Retained Size"), new Column("Δ"), new Column("Old Value"));
    ...
    for (int i = 0; i < count; i++) {
      ...
      boolean isDifferent = !Objects.equals(nowValue, wasValue);

      doc.row(Summarizer.summarize(nowValue), CollectionContentsHelper.size(nowValue),
          ...
          CollectionContentsHelper.baselineValue(nowValue, wasValue));
    }
    doc.end();
