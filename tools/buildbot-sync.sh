...
if [[ "$(build/soong/soong_ui.bash --dumpvar-mode TARGET_FLATTEN_APEX)" != "true" ]]; then
  ...
  echo -e "${magenta}You can force APEX flattening by setting the environment variable" \
    "\`PRODUCT_FLATTEN_APEX\` to \"true\" before starting the build and running this script.${nc}"
  exit 1
fi
...
# As a workaround, one can run `export PRODUCT_FLATTEN_APEX=true` before building
