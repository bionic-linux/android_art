...
def add_builder(mode,
    ...
    name += '.debug' if debug else '.ndebug'
    name += '.' + str(bitness)
...
def add_builders():
  for mode, arch in [("target", "arm"), ("host", "x86")]:
    for bitness in [32, 64]:
      ...
      add_builder(mode, arch, bitness, debug=True)
      if mode == "host" and bitness == 64:
        add_builder(mode, arch, bitness, ngen=True, debug=True)
      add_builder(mode, arch, bitness, ngen=True, cmc=True, debug=True)
      if mode == "target":
        add_builder(mode, arch, bitness, poison=True)
      add_builder(mode, arch, bitness, poison=True, debug=True)
      if mode == "target":
        add_builder(mode, arch, bitness, gcstress=True)
      add_builder(mode, arch, bitness, gcstress=True, debug=True)
