...
public class Jdwp {
  ...
  public long suspendForExecution() throws IOException {
    ...
    // to avoid this case.
    final int EVENT_TYPE = 40; // method entry event request
    final int SUSPEND_ALL = 2;
    int responseId = command(Command.EventRequest.Set, data -> {
      data.writeByte(EVENT_TYPE);
      data.writeByte(SUSPEND_ALL);
  ...
  /**
   * Creates a string and returns the created string id.
   */
