...
for arch in ${archs}; do
  ...
  for jar in ${device_bcp_list//:/ }; do
    ...
    for ext in art oat vdex; do
      check_artifact "/system/framework/${arch}/boot-${stem}.${ext}" "${arch_output}/boot-$stem.$ext"
    done
  done
done
...
for jar in ${SYSTEMSERVERCLASSPATH//:/ }; do
  ...
  for ext in odex vdex; do
    check_artifact "/system/framework/oat/${systemserver_arch}/${stem}.${ext}" "${arch_output}/${stem}.${ext}"
  done
