...
function usage() {
  cat <<EOF
Usage: $0 [OPTION]
Compile boot class path and system server components present in the system image.

  -c, --check  check artifacts against those already in the system image.
...
echo "CHECK_ARTIFACTS is $check_artifacts"
...
for jar in ${SYSTEMSERVERCLASSPATH//:/ }; do
  ...
  # Add profile if it exists. Profiles are only used for larger jar files.
  stem=$(basename $jar .jar)
