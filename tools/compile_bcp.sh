...
function compare_file() {
  # compare_file <old_file> <new_file>
  local old_file="$1"
  local new_file="$2"
...
function usage() {
  cat <<EOF
Usage: $0 [OPTION]
Compile boot class path and system server jars in the system image.

  -c, --compare-files   Compare generated files against those already in the
                        system image.
...
while [ $# -gt 0 ] ; do
  case "$1" in
    (-c|--compare-files) compare_files=true ;;
    (-h|--help) usage() ;;
    (*)
  ...
  if $compare_files ; then
    for jar in ${device_bcp_list//:/ }; do
      ...
      for ext in art oat vdex; do
        compare_file "/system/framework/${arch}/boot-${stem}.${ext}" \
...
for jar in ${SYSTEMSERVERCLASSPATH//:/ }; do
  ...
  if $compare_files ; then
    for ext in odex vdex; do
      compare_file "/system/framework/oat/${systemserver_arch}/${stem}.${ext}" \
