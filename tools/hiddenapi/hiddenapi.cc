...
class HiddenApi final {
 ...
 private:
  ...
  static constexpr std::string_view kDependencyStubApi{"dependency-stub-api"};
  ...
  void ListApi() {
    ...
    dependency_classpath.ForEachDexMember([&](const DexMember& boot_member) {
      boot_members[boot_member.GetApiEntry()] = {kDependencyStubApi};
    });
    ...
    for (const auto& entry : boot_members) {
      ...
      } else if (flags.find(kDependencyStubApi) == flags.end()) {
