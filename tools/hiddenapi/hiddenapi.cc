...
class HiddenapiItemBuilder final {
 public:
  explicit HiddenapiItemBuilder(const DexFile& dex_file, uint32_t dex_modifiers_)
      : num_classdefs_(dex_file.NumClassDefs()),
        ...
        data_(sizeof(DexFile::HiddenapiItem) + (sizeof(uint32_t) * num_classdefs_), 0u) {
    // Assume malloc() aligns allocated memory to at least uint32_t.
    CHECK(IsAligned<sizeof(uint32_t)>(data_.data()));

    DexFile::HiddenapiItem* header = GetHeader();
    header->size_ = GetCurrentDataSize();
    header->header_size_ = header->class_data_offset_ = sizeof(DexFile::HiddenapiItem);
 ...
 private:
  ...
  DexFile::HiddenapiItem* GetHeader() { return GetPointer<DexFile::HiddenapiItem>(0); }
...
class DexFileEditor final {
 public:
  ...
  void Encode() {
    ...
    size_t size_delta =
        RoundUp(hiddenapi_item_.size(), kMapListAlignment) + sizeof(DexFile::MapItem);
      ...
      if (command == "encode") {
        for (int i = 1; i < argc; ++i) {
          ...
          if (option.starts_with("--input-dex=")) {
            ...
            dex_modifiers_ |= DexFile::HiddenapiItem::kMod_CorePlatform;
          } else {
