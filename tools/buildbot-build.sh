...
if [[ $build_target == "yes" ]]; then
  ...
  # Use a fake libartpalette implmentations to prevent chroot tests from talking to the platform
  # through libartpalette.
  for l in lib lib64; do
    if [ ! -d "$ANDROID_PRODUCT_OUT/system/$l" ]; then
      continue
    fi
    cmd="cp -p \"$ANDROID_PRODUCT_OUT/system/$l/libartpalette_fake.so\" \"$ANDROID_PRODUCT_OUT/system/$l/libartpalette-system.so\""
