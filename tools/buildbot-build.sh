...
if [[ $mode == "host" ]]; then
  ...
  # Create a copy of the ICU .dat prebuilt file in /system/etc/icu on target,
  ...
  # Create a copy of the time zone data module prebuilt files in
  # /system/etc/tzdata_module/etc on target, so that it can be found even if
