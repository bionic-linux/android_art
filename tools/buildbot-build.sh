...
if [[ $mode == "host" ]]; then
  ...
  # TODO(rpl): No longer generate ART artifacts under the `system` directory now
  # that we use ART artifacts from the Runtime APEX.
  ...
  # Build the bootstrap Bionic libraries (libc, libdl, libm). These targets also
  # create the symlinks from /system/lib(64)/$lib to /bionic/lib(64)/$lib (see
  # also comments in art/tools/setup-buildbot-device.sh, for the ART
  # chroot-based testing case).
  #
  # TODO(rpl): Remove this when Bionic boostrap artifacts are gone (see b/125549215).
