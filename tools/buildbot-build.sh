...
if [[ $mode == "host" ]]; then
  ...
  # Build the bootstrap Bionic libraries (libc, libdl, libm). These targets are
  # required as they are the ones which create the symlinks from
  # /system/lib(64)/$lib to /bionic/lib(64)/$lib (see also comments in
  # art/tools/setup-buildbot-device.sh, for the case of ART chroot-based
  # testing).
