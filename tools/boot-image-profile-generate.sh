...
if [[ "$#" -lt 4 ]]; then
  ...
  echo "         $0 output-dir boot.zip frameworks/base/config/preloaded-classes-denylist android.prof --boot-image-profman-arg --upgrade-startup-to-hot=true"
...
while [[ "$#" -ge 1 ]] && [[ ! "$1" = '--boot-image-profman-arg' ]] && [[ ! "$1" = '--system-server-profman-arg' ]]; do
...
while [[ "$#" -ge 2 ]] && [[ "$1" = '--boot-image-profman-arg' ]]; do
  boot_image_profman_args+=("$2")
  shift 2
done

system_server_profman_args=()
while [[ "$#" -ge 2 ]] && [[ "$1" = '--system-server-profman-arg' ]]; do
