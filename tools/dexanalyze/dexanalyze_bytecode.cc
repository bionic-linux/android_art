...
void NewRegisterInstructions::Dump(std::ostream& os, uint64_t total_size) const {
  ...
  for (auto&& pair : instruction_freq_) {
    pairs.emplace_back(pair.second * pair.first.size(), pair.first);
  }
  std::sort(pairs.rbegin(), pairs.rend());
  os << "Top instruction bytecode sizes and hex dump" << "\n";
  uint64_t top_instructions_sum = 0u;
  uint64_t top_instructions_savings = 0u;
  for (size_t i = 0; i < 128 && i < pairs.size(); ++i) {
    top_instructions_sum += pairs[i].first;
    top_instructions_savings += pairs[i].first - pairs[i].first / pairs[i].second.size();
    os << Percent(pairs[i].first, total_size) << " "
       << Instruction::Name(static_cast<Instruction::Code>(pairs[i].second[0]))
       << "(" << pairs[i].second << ")\n";
  }
  os << "Top instructions sum " << Percent(top_instructions_sum, total_size) << "\n";
  os << "Top instructions savings " << Percent(top_instructions_savings, total_size) << "\n";
}
  ...
  for (auto inst = code_item.begin(); ; ++inst) {
    ...
    switch (opcode) {
      ...
      case Instruction::IPUT_SHORT: {
        ...
        if (count_types_ || !Enabled(kExperimentInstanceField)) {
      ...
      case Instruction::CONST_STRING: {
        const uint16_t str_idx = inst->VRegB_21c();
        if (!count_types_ && Enabled(kExperimentString) && (false)) {
          const uint32_t out_reg = inst->VRegA_21c();
          if (InstNibbles(opcode, {out_reg, static_cast<uint32_t>(str_idx) & 0xF})) {
            continue;
          }
        }
        break;
      ...
      case Instruction::SPUT_SHORT: {
        ...
        CHECK_LT(dex_field_idx, dex_file.NumFieldIds());
        dex::TypeIndex holder_type = dex_file.GetFieldId(dex_field_idx).class_idx_;
        if (count_types_ || !Enabled(kExperimentStaticField)) {
          // ++type_use_counts_[holder_type.index_];
        } else {
      ...
      case Instruction::INVOKE_SUPER: {
        ...
        if (count_types_ || !Enabled(kExperimentInvoke)) {
      ...
      case Instruction::NEW_INSTANCE: {
        ...
        const uint32_t out_reg = inst->VRegA_21c();
        if (count_types_ || !Enabled(kExperimentLocalType)) {
      ...
      case Instruction::RETURN_VOID: {
        if (Enabled(kExperimentImpliedReturn) && !count_types_ && (false)) {
