...
jdwp_path=$PWD/prebuilts/jdk/jdk8/linux-x86/jre/lib/amd64/libjdwp.so
if [ ! -f $jdwp_path ]; then
...
echo running command: LD_LIBRARY_PATH=\"$(dirname $jdwp_path):$LD_LIBRARY_PATH\" ./art/tools/run-jdwp-tests.sh $args -Xplugin:$plugin --agent-wrapper ${wrapper}=${props_path} --jdwp-path $jdwp_path --expectations $expect_path
LD_LIBRARY_PATH="$(dirname $jdwp_path):$LD_LIBRARY_PATH" ./art/tools/run-jdwp-tests.sh $args -Xplugin:$plugin --agent-wrapper ${wrapper}=${props_path} --jdwp-path $jdwp_path --expectations $expect_path
