...
class Item {
 public:
  virtual ~Item() { }
  Item() : offset_(0) { }
  explicit Item(uint32_t offset) : offset_(offset) { }
...
typedef std::vector<const TypeId*> TypeIdVector;

class ProtoId : public Item {
 public:
  ProtoId(const StringId* shorty,
          const TypeId* return_type,
          std::unique_ptr<std::vector<const TypeId*>> parameters) :
    shorty_(shorty),
...
typedef std::vector<std::unique_ptr<FieldItem>> FieldItemVector;
...
typedef std::vector<std::unique_ptr<MethodItem>> MethodItemVector;
...
class ArrayItem : public Item {
 public:
  ...
  ArrayItem(uint8_t type) : type_(type) { }
  ~ArrayItem() OVERRIDE { }
...
typedef std::vector<std::unique_ptr<ArrayItem>> ArrayItemVector;

class ClassData : public Item {
 public:
  ClassData(uint32_t offset,
            std::unique_ptr<FieldItemVector> static_fields,
            std::unique_ptr<FieldItemVector> instance_fields,
            std::unique_ptr<MethodItemVector> direct_methods,
            std::unique_ptr<MethodItemVector> virtual_methods) :
    Item(offset),
    static_fields_(std::move(static_fields)),
    instance_fields_(std::move(instance_fields)),
    direct_methods_(std::move(direct_methods)),
    virtual_methods_(std::move(virtual_methods)) {
  }

  ~ClassData() OVERRIDE = default;
  FieldItemVector& StaticFields() { return *static_fields_; }
  FieldItemVector& InstanceFields() { return *instance_fields_; }
  MethodItemVector& DirectMethods() { return *direct_methods_; }
  MethodItemVector& VirtualMethods() { return *virtual_methods_; }
...
class ClassDef : public Item {
 public:
  ClassDef(const TypeId* class_type,
           ...
           std::unique_ptr<TypeIdVector> interfaces,
           uint32_t interfaces_offset,
           const StringId* source_file,
           std::unique_ptr<AnnotationsDirectoryItem> annotations,
           std::unique_ptr<ArrayItemVector> static_values,
    ...
    static_values_(std::move(static_values)),
    class_data_(std::move(class_data)) {
  }
...
typedef std::vector<std::unique_ptr<const CatchHandler>> CatchHandlerVector;

class TryItem : public Item {
 public:
  TryItem(uint32_t start_addr, uint16_t insn_count, std::unique_ptr<CatchHandlerVector> handlers) :
    start_addr_(start_addr), insn_count_(insn_count), handlers_(std::move(handlers)) { }
  ~TryItem() OVERRIDE { }
...
typedef std::vector<std::unique_ptr<const TryItem>> TryItemVector;
...
class CodeItem : public Item {
 public:
  CodeItem(uint16_t registers_size,
           uint16_t ins_size,
           uint16_t outs_size,
           std::unique_ptr<DebugInfoItem> debug_info,
           uint32_t insns_size,
           std::unique_ptr<uint16_t[]> insns,
           std::unique_ptr<TryItemVector> tries) :
...
typedef std::vector<std::unique_ptr<AnnotationItem>> AnnotationItemVector;

class AnnotationSetItem : public Item {
 public:
  AnnotationSetItem(std::unique_ptr<AnnotationItemVector> items) :
...
typedef std::vector<std::unique_ptr<AnnotationSetItem>> AnnotationSetItemVector;
...
typedef std::vector<std::unique_ptr<FieldAnnotation>> FieldAnnotationVector;
...
typedef std::vector<std::unique_ptr<MethodAnnotation>> MethodAnnotationVector;

class ParameterAnnotation {
 public:
  ParameterAnnotation(MethodId* method_id,
                      std::unique_ptr<AnnotationSetItemVector> annotations) :
...
typedef std::vector<std::unique_ptr<ParameterAnnotation>> ParameterAnnotationVector;

class AnnotationsDirectoryItem : public Item {
 public:
  AnnotationsDirectoryItem(std::unique_ptr<AnnotationSetItem> class_annotation,
                           std::unique_ptr<FieldAnnotationVector> field_annotations,
                           std::unique_ptr<MethodAnnotationVector> method_annotations,
                           std::unique_ptr<ParameterAnnotationVector> parameter_annotations) :
    class_annotation_(std::move(class_annotation)),
    field_annotations_(std::move(field_annotations)),
    method_annotations_(std::move(method_annotations)),
    parameter_annotations_(std::move(parameter_annotations)) {
  }
