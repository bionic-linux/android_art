...
void ReadPayloadUnion(Header& header,
  ...
  switch (type) {
    ...
    case DexFile::kDexAnnotationArray: {
      item->annotation_array_val_ = new std::vector<std::unique_ptr<ArrayItem>>();
      // Decode all elements.
...
CodeItem* ReadCodeItem(const DexFile& dex_file,
  ...
  if (tries_size > 0) {
    tries = new std::vector<std::unique_ptr<const TryItem>>();
    for (uint32_t i = 0; i < tries_size; ++i) {
      ...
      TryItem* try_item = new TryItem(start_addr,
                                      insn_count,
                                      std::unique_ptr<CatchHandlerVector>(handlers));
      tries->push_back(std::unique_ptr<const TryItem>(try_item));
    }
  }

  return new CodeItem(registers_size,
                      ins_size,
                      outs_size,
...
AnnotationSetItem* ReadAnnotationSetItem(const DexFile& dex_file,
  ...
  return new AnnotationSetItem(std::unique_ptr<AnnotationItemVector>(items));
}
...
ParameterAnnotation* ReadParameterAnnotation(
  ...
  return new ParameterAnnotation(method_id, std::unique_ptr<AnnotationSetItemVector>(annotations));
}
...
AnnotationsDirectoryItem* ReadAnnotationsDirectoryItem(
  ...
  return new AnnotationsDirectoryItem(
      std::unique_ptr<AnnotationSetItem>(class_annotation),
...
Header* DexIrBuilder(const DexFile& dex_file) {
  ...
  for (uint32_t i = 0; i < dex_file.NumProtoIds(); ++i) {
    ...
    typedef std::vector<const TypeId*> ParamVector;
    ParamVector* parameters = new ParamVector();
    ...
    ProtoId* proto_id = new ProtoId(header->StringIds()[disk_proto_id.shorty_idx_].get(),
                                    ...
                                    std::unique_ptr<ParamVector>(parameters));
