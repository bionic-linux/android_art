...
void Collections::CreateProtoId(const DexFile& dex_file, uint32_t i) {
  ...
  ProtoId* proto_id = new ProtoId(GetStringId(disk_proto_id.shorty_idx_),
                                  GetTypeId(disk_proto_id.return_type_idx_.index),
                                  parameter_type_list);
...
void Collections::CreateFieldId(const DexFile& dex_file, uint32_t i) {
  ...
  FieldId* field_id = new FieldId(GetTypeId(disk_field_id.class_idx_.index),
                                  GetTypeId(disk_field_id.type_idx_.index),
...
void Collections::CreateMethodId(const DexFile& dex_file, uint32_t i) {
  ...
  MethodId* method_id = new MethodId(GetTypeId(disk_method_id.class_idx_.index),
...
void Collections::CreateClassDef(const DexFile& dex_file, uint32_t i) {
  ...
  const TypeId* class_type = GetTypeId(disk_class_def.class_idx_.index);
  uint32_t access_flags = disk_class_def.access_flags_;
  const TypeId* superclass = GetTypeIdOrNullPtr(disk_class_def.superclass_idx_.index);
...
TypeList* Collections::CreateTypeList(const DexFile::TypeList* dex_type_list, uint32_t offset) {
  ...
  for (uint32_t index = 0; index < size; ++index) {
    type_vector->push_back(GetTypeId(dex_type_list->GetTypeItem(index).type_idx_.index));
  }
    ...
    for (uint32_t i = 0; i < tries_size; ++i) {
      ...
      if (handlers == nullptr) {
        ...
        for (CatchHandlerIterator it(disk_code_item, *disk_try_item); it.HasNext(); it.Next()) {
          ...
          const TypeId* type_id = GetTypeIdOrNullPtr(type_index.index);
