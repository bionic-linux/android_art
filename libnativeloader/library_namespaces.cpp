...
Result<NativeLoaderNamespace*> LibraryNamespaces::Create(JNIEnv* env, uint32_t target_sdk_version,
  ...
  // art_public_libraries() return empty list normally but contains extra lib from
  // ANDROID_ADDITIONAL_PUBLIC_LIBRARIES in debuggable platform builds.
  if (art_ns.ok() && !art_public_libraries().empty()) {
