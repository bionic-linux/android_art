...
Result<NativeLoaderNamespace*> LibraryNamespaces::Create(JNIEnv* env, uint32_t target_sdk_version,
  ...
  if (!is_shared) {
    if (apk_origin == APK_ORIGIN_SYSTEM) {
      // System apps commonly get access to system libs through shared
      // namespaces (i.e. is_shared is true) and hence don't need this. In
      // practice it's necessary for shared system libraries (i.e. JARs rather
      // than actual APKs) that are loaded by ordinary apps which don't get
      // shared namespaces.
