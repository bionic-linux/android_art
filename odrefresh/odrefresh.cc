...
class OnDeviceRefresh final {
 ...
 public:
  ...
  void WriteCacheInfo() const {
    ...
    std::optional<art_apex::CacheProvenanceType> cache_provenance =
    ...
    art_apex::ApexCacheInfo info({cache_provenance.value()});
    art_apex::write(out, info);
  }

  // Returns cache provenance information based on the current ART APEX version and filesystem
  // information.
  std::optional<art_apex::CacheProvenanceType> GenerateCacheProvenance()
      const {
    ...
    return art_apex::CacheProvenanceType{
