...
OnDeviceRefresh::RunDex2oatForBootClasspath(const std::string& staging_dir,
  ...
  if (input_boot_images.empty()) {
    ...
    std::string art_preloaded_classes_file(GetAndroidRoot() + "/etc/preloaded-classes-art");
    ...
    if (art_file != nullptr) {
      ...
      LOG(WARNING) << ART_FORMAT("Missing art preloaded classes file '{}'", art_preloaded_classes_file);
    }
  } else {
    // Mainline extension.
