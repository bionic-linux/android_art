...
class OnDeviceRefresh final {
 ...
 public:
  ...
  std::optional<art_apex::ApexCacheInfo> ReadCacheInfo() {
  ...
  void WriteCacheInfo() const {
    ...
    std::optional<art_apex::CacheProvenance> cache_provenance = GenerateCacheProvenance();
    ...
    const std::vector<art_apex::CacheProvenance> cache_provenances { cache_provenance.value() };
  ...
  // information.
  std::optional<art_apex::CacheProvenance> GenerateCacheProvenance() const {
    ...
    return art_apex::CacheProvenance{
        art_apex_info->getVersionCode(), art_apex_info->getVersionName(), xmlDateTime};
  }
  ...
  ExitCode CheckArtifactsAreUpToDate() {
    ...
    std::optional<art_apex::ApexCacheInfo> apex_cache_info = ReadCacheInfo();
    ...
    if (actual_provenance->getArtModuleVersionCode() !=
      ...
      LOG(INFO) << "ART APEX version code mismatch ("
                << actual_provenance->getArtModuleVersionCode()
                << " != " << expected_provenance->getArtModuleVersionCode() << ")";
      RemoveArtifactsOrDie();
      return ExitCode::kCompilationRequired;
    }

    if (actual_provenance->getArtModuleVersionName() !=
        expected_provenance->getArtModuleVersionName()) {
      LOG(INFO) << "ART APEX version code mismatch ("
                << actual_provenance->getArtModuleVersionName()
                << " != " << expected_provenance->getArtModuleVersionName() << ")";
      RemoveArtifactsOrDie();
      return ExitCode::kCompilationRequired;
    }

    // Check boot class components. These may unexpectedly change: for example an OTA could update
    // framework.jar.
    const std::vector<art_apex::Component> expected_bcp_components =
        GenerateBootExtensionComponents();
    if (expected_bcp_components.size() != 0 &&
        (!apex_cache_info->hasDex2oatBootClasspath() ||
         !apex_cache_info->getFirstDex2oatBootClasspath()->hasComponent())) {
    ...
    const std::vector<art_apex::Component>& bcp_components =
        apex_cache_info->getFirstDex2oatBootClasspath()->getComponent();
    ...
    // Check system server components. These may unexpectedly change: for example an OTA could
    // update services.jar.
    const std::vector<art_apex::Component> expected_system_server_components =
        GenerateSystemServerComponents();
    if (expected_system_server_components.size() != 0 &&
        (!apex_cache_info->hasSystemServerClasspath() ||
         !apex_cache_info->getFirstSystemServerClasspath()->hasComponent())) {
    ...
    const std::vector<art_apex::Component>& system_server_components =
        apex_cache_info->getFirstSystemServerClasspath()->getComponent();
