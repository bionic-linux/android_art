...
std::string OnDeviceRefresh::GetSystemServerImagePath(bool on_system,
  ...
  if (on_system) {
    std::string system_file_for_apex =
        GetSystemOdexFilenameForApex(jar_path, config_.GetSystemServerIsa());
    if (!system_file_for_apex.empty()) {
      return system_file_for_apex;
    }
