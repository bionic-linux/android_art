...
TEST_F(OdRefreshTest, ContinueWhenBcpCompilationFailed) {
  ...
  EXPECT_CALL(*mock_exec_utils_,
              DoExecAndReturnCode(Contains(Concatenate({"--dex-file=", location_provider_jar_}))))
      .WillOnce(Return(0));
  EXPECT_CALL(*mock_exec_utils_,
              DoExecAndReturnCode(Contains(Concatenate({"--dex-file=", services_jar_}))))
      .WillOnce(Return(0));
  EXPECT_CALL(*mock_exec_utils_,
              DoExecAndReturnCode(Contains(Concatenate({"--dex-file=", services_foo_jar_}))))
      .WillOnce(Return(0));
  EXPECT_CALL(*mock_exec_utils_,
              DoExecAndReturnCode(Contains(Concatenate({"--dex-file=", services_bar_jar_}))))
      .WillOnce(Return(0));
