...
class OdRefreshTest : public CommonArtTest {
 ...
 protected:
  void SetUp() override {
    ...
    delete OS::CreateEmptyFile(framework_jar.c_str());
    delete OS::CreateEmptyFile(location_provider_jar.c_str());
    delete OS::CreateEmptyFile(services_jar.c_str());
    delete OS::CreateEmptyFile(services_jar_prof.c_str());
    ...
    odrefresh_ = std::make_unique<OnDeviceRefresh>(config_, dalvik_cache_dir + "/cache-info.xml");
  }
