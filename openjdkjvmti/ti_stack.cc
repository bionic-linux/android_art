...
jvmtiError StackUtil::NotifyFramePop(jvmtiEnv* env,
                                     jthread thread,
                                     jint depth) {
  if (depth < 0) {
    return ERR(ILLEGAL_ARGUMENT);
  ...
  do {
    ...
    if (method->IsNative()) {
      // TODO Check this error return value.
      return ERR(OPAQUE_FRAME);
    }
    // From here we are sure to succeed.
    // TODO Record that this jvmtienv is waiting for this specific shadow frame.
