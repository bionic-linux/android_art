...
class ArtdProfileSaveNotificationTest : public ArtdTest {
 protected:
  void SetUp() override {
    ...
    notification_file_ = ART_FORMAT(
        "{}/misc/profiles/cur/{}/{}/pid_{}.tmp", android_data_, user_id_, package_name_, pid_);
    std::filesystem::create_directories(android::base::Dirname(notification_file_));
  }

  const std::string package_name_ = "com.android.foo";
  const int user_id_ = 0;
...
TEST_F(ArtdProfileSaveNotificationTest, initAndWaitSuccess) {
  ...
  ASSERT_STATUS_OK(
      artd_->initProfileSaveNotification(package_name_, user_id_, pid_, &notification));
  ...
  std::filesystem::remove(notification_file_);
...
TEST_F(ArtdProfileSaveNotificationTest, initAndWaitProcessGone) {
  ...
  EXPECT_FALSE(std::filesystem::exists(notification_file_));
}
...
TEST_F(ArtdProfileSaveNotificationTest, initFileGone) {
  // Remove the file before inotify_add_watch.
  EXPECT_CALL(mock_inotify_add_watch_, Call)
      .WillOnce([this](int fd, const char* pathname, uint32_t mask) {
        std::filesystem::remove(notification_file_);
