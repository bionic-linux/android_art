...
ScopedAStatus Artd::initProfileSaveNotification(const std::string& in_packageName,
  ...
  unique_fd inotify_fd(inotify_init1_(IN_NONBLOCK | IN_CLOEXEC));
  if (inotify_fd < 0) {
    return NonFatal(ART_FORMAT("Failed to inotify_init1: {}", strerror(errno)));
  }

  // Use the FD path to make sure we watch the file that we just created.
  std::string fd_path = ART_FORMAT("/proc/self/fd/{}", file->Fd());
  int wd = inotify_add_watch_(inotify_fd, fd_path.c_str(), IN_DELETE_SELF);
  if (wd < 0) {
    return NonFatal(ART_FORMAT(
        "Failed to inotify_add_watch '{}' for '{}': {}", fd_path, path, strerror(errno)));
  }

  unique_fd pidfd = pidfd_open_(in_pid, /*flags=*/0);
  if (pidfd < 0) {
    if (errno == ESRCH) {
...
ScopedAStatus ArtdNotification::wait(int in_timeoutMs, bool* _aidl_return) {
  ...
  int ret = TEMP_FAILURE_RETRY(poll_(pollfds, /*nfds=*/2, in_timeoutMs));
  if (ret < 0) {
    return NonFatal(
...
void ArtdNotification::CleanUp() {
  std::lock_guard<std::mutex> lock(mu_);
  // This deletion takes place if the process has exited or a timeout occurred. It may be
  // interpreted by another listener as a notification, but that's fine because in either case, it's
  // rarely useful for another listener to wait longer.
  DeleteFile(path_);
  inotify_fd_.reset();
  pidfd_.reset();
}
