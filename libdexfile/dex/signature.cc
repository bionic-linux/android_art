...
std::string Signature::ToString() const {
  ...
  if (params == nullptr) {
    ...
    for (uint32_t i = 0; i < params->Size(); ++i) {
      result += dex_file_->GetTypeDescriptor(params->GetTypeItem(i).type_idx_);
    }
    result += ")";
  }
  result += dex_file_->GetTypeDescriptor(proto_id_->return_type_idx_);
  return result;
}
...
bool Signature::operator==(std::string_view rhs) const {
  ...
  if (params != nullptr) {
    for (uint32_t i = 0; i < params->Size(); ++i) {
      std::string_view param(dex_file_->GetTypeDescriptor(params->GetTypeItem(i).type_idx_));
  ...
  tail.remove_prefix(1);  // ")";
  return tail == dex_file_->GetTypeDescriptor(proto_id_->return_type_idx_);
}
