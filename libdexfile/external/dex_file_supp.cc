...
#define DEFINE_DLFUNC_PTR(DLFUNC) decltype(DLFUNC)* g_##DLFUNC = DLFUNC;
#else
#define DEFINE_DLFUNC_PTR(DLFUNC) decltype(DLFUNC)* g_##DLFUNC = nullptr;
#endif
...
#define RESOLVE_DLFUNC_PTR(DLFUNC) \
    if ((g_##DLFUNC = reinterpret_cast<decltype(DLFUNC)*>(dlsym(handle, #DLFUNC))) == nullptr) { \
      *err_msg = dlerror(); \
      return false; \
    }
