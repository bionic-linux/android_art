...
TEST(DexFileTest, get_method_info_callback) {
  ...
  auto run_test = [&dex_file](ExtDexFileMethodFlags flags, const char* expected_name) {
  ...
  run_test(kExtDexFileFlags_none, "");
  run_test(kExtDexFileFlags_nameOnly, "main");
  run_test(kExtDexFileFlags_nameWithClass, "Main.main");
  run_test(kExtDexFileFlags_nameWithParameters, "void Main.main(java.lang.String[])");
}
...
TEST(DexFileTest, error_msg) {
  ...
  for (size_t i = 0; i < kNumErrors; i++) {
    ...
    ASSERT_TRUE(!std::string_view(err).empty());
  }
