...
TEST(DexFileTest, get_method_info_callback) {
  ...
  auto run_test = [&dex_file](ADexFile_Flags flags, const char* expected_name) {
    size_t num_callbacks = 0;
    auto check = [&num_callbacks, expected_name](ADexFile_MethodInfo* info) {
  ...
  run_test(ADEXFILE_FLAGS_NONE, "");
  run_test(ADEXFILE_FLAGS_NAME_ONLY, "main");
  run_test(ADEXFILE_FLAGS_NAME_WITH_CLASS, "Main.main");
  run_test(ADEXFILE_FLAGS_NAME_WITH_PARAMETERS, "void Main.main(java.lang.String[])");
}
