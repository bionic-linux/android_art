...
enum ADexFile_Error : uint32_t {
  ADEXFILE_ERROR_OK = 0,
  ADEXFILE_ERROR_INVALID_DEX = 1,  // Unspecified loading error.
...
enum ADexFile_Flags : uint32_t {
  ...
  ADEXFILE_FLAGS_NAME_ONLY = 1 << 0,            // E.g. Main
  ADEXFILE_FLAGS_NAME_WITH_CLASS = 1 << 1,       // E.g. MyClass.Main
  ADEXFILE_FLAGS_NAME_WITH_PARAMETERS = 1 << 2,  // E.g. void MyClass.Main(String[])
};
...
//             ADEXFILE_ERROR_NOT_ENOUGH_DATA and sets size to a new size to try again with.
...
// @param flags Specifies the method name formatting in the 'name' field.
