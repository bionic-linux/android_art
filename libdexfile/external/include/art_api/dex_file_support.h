...
class DexFile {
 public:
  ...
  void GetMethodInfoForOffset(int64_t dex_offset,
    ...
    g_ExtDexFile_FindMethodAtOffset(ext_dex_file_, dex_offset, flags, cb, &callback);
  }
  ...
  void GetAllMethodInfos(T& callback, ExtDexFileFlags flags = ExtDexFileFlags_NameWithClass) {
    auto cb = [](void* ctx, ExtDexFileMethodInfo* info) { (*reinterpret_cast<T*>(ctx))(info); };
    g_ExtDexFile_ForEachMethod(ext_dex_file_, flags, cb, &callback);
  }
 ...
 private:
  ...
  static decltype(ExtDexFile_FindMethodAtOffset)* g_ExtDexFile_FindMethodAtOffset;
  static decltype(ExtDexFile_ForEachMethod)* g_ExtDexFile_ForEachMethod;
