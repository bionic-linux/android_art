...
class DexFile {
 public:
  ...
  MethodInfo GetMethodInfoForOffset(int64_t dex_offset, bool with_signature) {
    MethodInfo res{};
    auto set_method = [](void* ctx, char* name, size_t len, uint32_t addr, uint32_t size) {
      MethodInfo* res = reinterpret_cast<MethodInfo*>(ctx);
      *res = MethodInfo{.offset = addr, .len = size, .name = std::string(name, len)};
    };
    g_ExtDexFileGetMethodInfoForOffset(ext_dex_file_, dex_offset, with_signature, set_method, &res);
    return res;
  }
  ...
  std::vector<MethodInfo> GetAllMethodInfos(bool with_signature) {
    ...
    auto add_method = [](void* ctx, char* name, size_t len, uint32_t addr, uint32_t size) {
      std::vector<MethodInfo>* res = reinterpret_cast<std::vector<MethodInfo>*>(ctx);
      res->emplace_back(MethodInfo{.offset = addr, .len = size, .name = std::string(name, len)});
    };
    g_ExtDexFileGetAllMethodInfos(ext_dex_file_, with_signature, add_method, &res);
    return res;
  }
