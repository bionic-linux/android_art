 ...
 public:
  struct Method {
    size_t GetCodeOffset(size_t* out_size = nullptr) const {
      return g_ADexFile_Method_getCodeOffset(self_, out_size);
    }

    const char* GetName(size_t* out_size = nullptr) const {
      return g_ADexFile_Method_getName(self_, out_size);
    }

    const char* GetQualifiedName(bool with_params = false, size_t* out_size = nullptr) const {
      return g_ADexFile_Method_getQualifiedName(self_, with_params, out_size);
    }

    const char* GetClassDescriptor(size_t* out_size = nullptr) const {
      return g_ADexFile_Method_getClassDescriptor(self_, out_size);
    }

    const ADexFile_Method* self_;
  };

  struct Error {
    const char* ToString() const {
      return g_ADexFile_Error_toString(self_);
    }

    operator bool() const {
      return self_ == ADEXFILE_ERROR_OK;
    }

    ADexFile_Error self_;
  };
