...
struct ADexFile {
  ...
  inline bool GetMethodDefIndex(uint32_t dex_offset, /*out*/ ADexFile_Method* result) {
...
size_t ADexFile_findMethodAtOffset(ADexFile* self,
  ...
  if (!self->GetMethodDefIndex(dex_offset, &info)) {
    return 0;
  }
...
const char* ADexFile_Method_getQualifiedName(const ADexFile_Method* self,
                                             bool with_params,
                                             size_t* out_size) {
  std::string& temp = self->adex->temporary_name_;
  temp.clear();
...
const char* ADexFile_Error_toString(ADexFile_Error self) {
  switch (self) {
    ...
    case ADEXFILE_ERROR_INVALID_DEX: return "Can not open dex file.";
    ...
    case ADEXFILE_ERROR_MISSING_LIBRARY: return "Can not dlopen required libraries";
  }
  return nullptr;
}
