...
ExtDexFileError ExtDexFile_Create(const void* addr,
  ...
  if (*size < sizeof(art::DexFile::Header)) {
    *size = sizeof(art::DexFile::Header);
    return ExtDexFileError_NotEnoughData;
  }
  ...
  if (art::CompactDexFile::IsMagicValid(header->magic_)) {
    ...
    if (__builtin_add_overflow(header->data_off_, header->data_size_, &computed_file_size)) {
      return ExtDexFileError_InvalidHeader;
    }
  ...
  if (dex_file == nullptr) {
    ...
    return ExtDexFileError_Error;
  }

  *ext_dex_file = new ExtDexFile(std::move(dex_file));
  return ExtDexFileError_Ok;
}

uint32_t ExtDexFile_GetMethodInfoForOffset(ExtDexFile* self,
                                           uint32_t dex_offset,
                                           ExtDexFileFlags flags,
                                           ExtDexFile_MethodInfoCallback* method_info_cb,
                                           void* user_data) {
...
uint32_t ExtDexFile_GetAllMethodInfos(ExtDexFile* self,
                                      ExtDexFileFlags flags,
                                      ExtDexFile_MethodInfoCallback* method_info_cb,
                                      void* user_data) {
  uint32_t count = 0;
...
const char* ExtDexFileError_ToString(ExtDexFileError self) {
  switch (self) {
    case ExtDexFileError_Ok: return "Ok";
    case ExtDexFileError_Error: return "Can not open dex file.";
    case ExtDexFileError_NotEnoughData: return "Not enough data. Incomplete dex file.";
    case ExtDexFileError_InvalidHeader: return "Invalid dex file header.";
  }
  return nullptr;
}
